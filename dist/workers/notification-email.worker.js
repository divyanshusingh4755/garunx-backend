...
export const notificationEmailWorker = new Worker(NOTIFICATION_QUEUE_NAMES.EMAIL, async (job) => {
    try {
        return await NotificationService
            .deliverEmail(job.data.notificationId);
    }
    catch (error) {
        /*
         * attemptsMade contains the
         * number of previous failed
         * attempts.
         *
         * Therefore current attempt =
         * attemptsMade + 1.
         */
        ...
        if (currentAttempt >=
            ...
            await NotificationService
                .markEmailDeliveryFailed(job.data.notificationId, error);
        }
        /*
         * BullMQ requires the worker
         * processor to throw for its
         * retry mechanism.
         */
...
notificationEmailWorker.on("completed", (job) => {
    console.log(`[NOTIFICATION EMAIL] Completed: ${job.id}`);
});
notificationEmailWorker.on("failed", (job, error) => {
    console.error(`[NOTIFICATION EMAIL] Failed: ${job?.id}`, error.message);
});
notificationEmailWorker.on("error", (error) => {
    console.error("[NOTIFICATION EMAIL] Worker error:", error);
});
