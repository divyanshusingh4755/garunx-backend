...
export const register = async (req, res) => {
    try {
        const { role, password, idToken } = req.body;
        // Call service to handle logic
        const user = await AuthService.registerUser(role, idToken, password);
        res.status(201).send({
            success: true,
...
export const getGetAllUser = async (req, res) => {
    try {
        ...
        const { users, pagination } = await AuthService.GetAllUser(Number(page) || 1, Number(limit) || 40);
...
export const GetUserByEmailorPhone = async (req, res) => {
    try {
        ...
        const data = await AuthService.GetUserByEmailorPhone(identifier);
        res.status(200).json({ success: true, data });
    }
    catch (error) {
