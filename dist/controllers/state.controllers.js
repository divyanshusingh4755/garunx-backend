...
export const getAllState = async (req, res) => {
    try {
        const { searchTerm, stateFilter, countryFilter, limit, page } = req.query;
        const { data, total, page: CurrentPage, totalPages } = await StateService.FindState(searchTerm, stateFilter, countryFilter, Number(limit) || 40, Number(page) || 1);
