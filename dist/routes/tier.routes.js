...
const createTierValidation = [
    body("name").notEmpty().withMessage("name is required").isString().withMessage("name must be string").trim(),
    body("tierReference").optional({ checkFalsy: true }).isString().withMessage("tierReference must be string").trim(),
...
const updateTierValidation = [
    ...
    body("tierReference").optional({ nullable: true }).isString().withMessage("tierReference must be string").trim(),
    body().custom((payload) => {
        const allowedFields = ["name", "tierReference",];
...
const adminTierListValidation = [
    ...
    query("sortBy").optional().isIn(["name", "tierReference", "isActive", "createdAt", "updatedAt", "relevance",]).withMessage("Invalid sortBy value"),
    query("sortOrder").optional().isIn(["asc", "desc"]).withMessage("sortOrder must be asc or desc"),
    validate,
];
const exportTiersValidation = [
    body("tierIds").isArray({ min: 1, max: 1000 }).withMessage("tierIds must contain between 1 and 1000 tier IDs"),
    body("tierIds.*").isMongoId().withMessage("Each tierId must be a valid MongoDB ID"),
    validate,
];
// PUBLIC
