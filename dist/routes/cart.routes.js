...
const mergeValidation = [
    body("guestCartIds")
        .exists()
        .isArray()
        .withMessage("guestCartIds must be an array of IDs"),
    body("guestCartIds.*")
        .isString()
        .withMessage("Each guestCartId must be a string"),
    validate,
];
const customerDetailsValidation = [
    body("customerDetails")
        .exists()
        .isObject()
        .withMessage("customerDetails is required"),
    validate,
];
const router = Router();
...
router.put("/:cartId/customer", authenticate, customerDetailsValidation, cartController.updateCustomerDetails);
router.delete("/", authenticate, cartController.clearCart);
router.post("/validate", cartController.validateCart);
router.post("/checkout/:cartId", authenticate, cartController.prepareCheckout);
