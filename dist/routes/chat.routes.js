...
const getMessagesValidation = [
    param("conversationId").isMongoId().withMessage("Invalid conversation ID"),
    query("cursor").optional().isMongoId().withMessage("Invalid message cursor"),
    query("limit").optional().isInt({ min: 1, max: 100 }).withMessage("Limit must be between 1 and 100").toInt(),
    validate,
];
const sendMessageValidation = [
    param("conversationId").isMongoId().withMessage("Invalid conversation ID"),
    body("clientMessageId").exists({ checkFalsy: true, }).withMessage("Client message ID is required").bail().isString().withMessage("Client message ID must be string").bail().trim().notEmpty().withMessage("Client message ID cannot be empty").isLength({ max: 128, }).withMessage("Client message ID cannot exceed 128 characters"),
    body("type").exists({ checkFalsy: true, }).withMessage("Message type is required").bail().isIn([ChatMessageType.TEXT, ChatMessageType.IMAGE,]).withMessage("Message type must be TEXT or IMAGE"),
    body("replyToMessageId").optional().isMongoId().withMessage("Invalid reply message ID"),
    body("text").optional().isString().withMessage("Message text must be a string").trim().isLength({ max: 5000, }).withMessage("Message text cannot exceed 5000 characters"),
    body("images").optional().isArray({ max: 5, }).withMessage("Images must be an array with maximum 5 images"),
    body("images.*").optional().isString().withMessage("Each image must be a string").bail().trim().isLength({ min: 1, max: 2000, }).withMessage("Each image URL must be between 1 and 2000 characters").bail().isURL({ protocols: ["http", "https",], require_protocol: true, }).withMessage("Each image must be a valid HTTP or HTTPS URL"),
    body().custom((value) => {
