...
export class SubServiceComponentService {
    static async invalidateSubServiceComponentCache(subServiceComponentId) {
        const operations = [RedisCacheService.deleteByPattern(CacheKeys.subServiceComponentListPattern())];
    ...
    static async updateSubServiceComponent(subServiceComponentId, updateData) {
        ...
        const affectedServiceIds = [existing.serviceId.toString(), ...(updatedSubServiceComponent.serviceId ? [typeof updatedSubServiceComponent.serviceId === "object" && "_id" in updatedSubServiceComponent.serviceId ? updatedSubServiceComponent.serviceId._id.toString() : updatedSubServiceComponent.serviceId] : [])];
        await Promise.all([
