...
export class WalletService {
    ...
    static normalizedObjectId(value, fieldName) {
    ...
    static validateOwnerRole(ownerRole) {
    ...
    static async getOrCreateWallet(params) {
    ...
    static async reservedForWithdrawal(params) {
        const amount = this.validateAmount(params.amount);
        const ownerId = this.normalizedObjectId(params.ownerId, "owner ID");
        this.validateOwnerRole(params.ownerRole);
        const wallet = await this.getOrCreateWallet({ ownerId, ownerRole: params.ownerRole, session: params.session });
    ...
    static async releaseWithdrawalReservation(params) {
        const amount = this.validateAmount(params.amount);
        const ownerId = this.normalizedObjectId(params.ownerId, "owner ID");
        this.validateOwnerRole(params.ownerRole);
        const updatedWallet = await Wallet.findOneAndUpdate({
    ...
    static async adminDebit(params) {
        const amount = this.validateAmount(params.amount);
        const ownerId = this.normalizedObjectId(params.ownerId, "owner ID");
        ...
        if (existringTransaction) {
            return existringTransaction;
        }
        const wallet = await Wallet.findOneAndUpdate({
            ownerId,
            ownerRole: params.ownerRole,
    ...
    static async creditCoordinatorEarning(params) {
        const bookingId = this.normalizedObjectId(params.bookingId, "booking ID");
        return this.credit({
    ...
    static async getMyWallet(ownerId, ownerRole) {
    ...
    static async getMyTransactions(ownerId, ownerRole, options = {}) {
        this.validateOwnerRole(ownerRole);
        const ownerObjectId = this.normalizedObjectId(ownerId, "owner ID");
