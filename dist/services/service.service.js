...
export class ServiceService {
    static async invalidateServiceCache(serviceId) {
        ...
        if (serviceId) {
            operations.push(RedisCacheService.delete(CacheKeys.serviceDetail(serviceId)), RedisCacheService.delete(CacheKeys.serviceFull(serviceId)), RedisCacheService.deleteByPattern(CacheKeys.serviceFullByCitiesPattern(serviceId)));
        }
        await Promise.all(operations);
    }
    static async invalidatePackageCaches() {
        await Promise.all([
    ...
    static async buildFullServiceData(service, options) {
         ...
         *
         * PUBLIC:
         * only globally active tiers.
         *
         * ADMIN:
         * every tier attached to Service.
         * -------------------------------------------------
         */
        const serviceTierIds = (service.tiers ??
            []).map((tier) => tier.tierId);
        let validTierIds = serviceTierIds;
        if (publicView) {
            const activeTierDocs = await Tier.find({
                _id: {
                    $in: serviceTierIds,
                },
                isActive: true,
                ...
                .lean();
            validTierIds =
                activeTierDocs.map((tier) => tier._id);
        }
         ...
         *
         * PUBLIC:
         * embedded location must be active
         * AND global Location must be active.
         *
         * ADMIN:
         * every attached location.
         * -------------------------------------------------
         */
        const candidateLocations = publicView
            ? (service.locations ??
                []).filter((location) => location.isActive)
            : (service.locations ??
                []);
        const serviceLocationIds = candidateLocations.map((location) => location.locationId);
        let validLocationIds = serviceLocationIds;
        if (publicView) {
            const activeLocationDocs = await Location.find({
                _id: {
                ...
                .lean();
            validLocationIds =
                activeLocationDocs.map((location) => location._id);
        }
        ...
        if (publicView) {
            serviceComponentQuery.tierId = {
                $in: validTierIds,
            };
            pricingQuery.tierId = {
                $in: validTierIds,
            };
            pricingQuery.locationId = {
                $in: validLocationIds,
            };
            pricingQuery.isActive =
                true;
        }
        const pricingBuilder = ServicePricing.find(pricingQuery)
            .select([
            "tierId",
            "componentId",
            "locationId",
        ...
        /*
         * PUBLIC:
         * only currently active TaxProfile.
         *
         * ADMIN:
         * populate even inactive TaxProfile so
         * configuration can be diagnosed.
         */
         ...
         */
        const componentIds = [
            ...new Set(serviceComponents.map((component) => component.componentId
                .toString())),
        ];
        /*
         * PUBLIC:
        ...
        const componentQuery = {
            _id: {
                $in: componentIds.map((id) => new Types.ObjectId(id)),
            },
        };
        if (publicView) {
            componentQuery.isActive =
                true;
        }
        const componentDocs = componentIds.length > 0
            ? await Component.find(componentQuery).lean()
            : [];
         ...
         */
        const itemIds = [
            ...new Set(serviceComponents.flatMap((component) => (component.items ??
        ...
        const itemDocs = itemIds.length > 0
            ? await ComponentItem.find(itemQuery).lean()
            : [];
        ...
        for (const price of pricing) {
            const tierId = price.tierId
                .toString();
            const locationId = price.locationId
                .toString();
            /*
             * Public defensive filtering.
             */
            if (publicView &&
                (!validTierIdSet.has(tierId) ||
                    !validLocationIdSet.has(locationId) ||
                    !price.isActive)) {
                continue;
            }
        ...
        for (const serviceComponent of serviceComponents) {
            const tierId = serviceComponent
                .tierId
                .toString();
            /*
             * Public:
             * globally inactive Tier is hidden.
             */
            if (publicView &&
                !validTierIdSet.has(tierId)) {
                continue;
            }
            const componentId = serviceComponent
                .componentId
                .toString();
            const componentDetails = componentMap.get(componentId);
            /*
             * Public:
             * inactive/missing Component is hidden.
             *
             * Admin:
             * preserve mapping even if canonical
            ...
            const componentPricing = pricingMap.get(pricingKey) ??
                [];
            /*
             * Public configuration without active
             * pricing is unusable.
             */
            if (publicView &&
                componentPricing.length ===
                    0) {
                continue;
            }
            /*
             ...
             * -------------------------------------------------
             */
            const hydratedItems = (serviceComponent.items ??
                [])
                .filter((item) => !publicView ||
                itemMap.has(item.itemId
                    .toString()))
                .map((item) => {
                ...
                return {
                    ...
                    itemDetails: itemDetails ??
                        null,
                    /*
                     * Helpful to admin when referenced
                     * item was removed/inactivated.
                     */
                ...
                component: componentDetails
                    ? {
                        ...
                        name: componentDetails
                            .name,
                        image: componentDetails
                            .imageUrl,
                        isRemovable: componentDetails
                            .isRemovable,
                        isBundled: componentDetails
                            .isBundled,
                        isActive: componentDetails
                            .isActive,
                    }
                    : null,
                /*
         ...
         * -------------------------------------------------
         */
        const filteredLocations = publicView
            ? (service.locations ??
                []).filter((location) => location.isActive &&
         ...
         *
         * PUBLIC:
         * active global Tier AND must contain
         * usable components.
         *
         * ADMIN:
        ...
        const filteredTiers = (service.tiers ??
            ...
            const tierId = tier.tierId
                .toString();
            return (validTierIdSet.has(tierId) &&
                Boolean(grouped[tierId]));
        });
        /*
         * -------------------------------------------------
         * Final response.
        ...
        return {
            ...
            tiers: filteredTiers.map((tier) => ({
                tierId: tier.tierId,
                name: tier.name,
                components: grouped[tier.tierId
                    .toString()]?.components ??
                    [],
            })),
            components: grouped,
    ...
    static async toggleServiceStatus(serviceId, isActive, confirmed = false) {
        ...
        try {
            ...
            await this.invalidateServiceCache(serviceId);
            if (!isActive) {
                await this.invalidatePackageCaches();
            }
            return {
                success: true,
    ...
    static async getServicesByLocation(params) {
        ...
        const safeSortBy = allowedSortFields.has(sortBy)
            ? sortBy
            : "createdAt";
        const cacheKey = CacheKeys.serviceByLocationList({
            cityIds,
            categoryIds,
            limit: safeLimit,
            page: safePage,
    ...
    static async getFullService(serviceId) {
        if (!Types.ObjectId.isValid(serviceId)) {
            throw createHttpError("Invalid serviceId", 400);
        }
        return RedisCacheService.getOrSet({
            ...
            loader: async () => {
                ...
                return this.buildFullServiceData(service, {
                    publicView: true,
                });
            },
        });
    }
    static async getFullServiceAdmin(serviceId) {
        if (!Types.ObjectId.isValid(serviceId)) {
            throw createHttpError("Invalid serviceId", 400);
        }
        /*
         * Admin version intentionally does NOT use
         * the public serviceFull cache.
         */
        const service = await Service.findById(serviceId)
            .populate({
            path: "subServiceComponents",
             ...
             * No isActive filter.
             *
             * Admin should be able to inspect inactive
             * sub-services as configuration data.
             */
            select: "name description image isActive",
            options: {
                sort: {
    ...
    static async getFullServiceByCities(serviceId, cityIds) {
        ...
        return RedisCacheService.getOrSet({
            ...
            loader: async () => {
                 ...
                 * belonging to the requested cities.
                 */
                const locations = await Location.find({
                    cityId: {
                        $in: cityIds.map((id) => new Types.ObjectId(id)),
                ...
                /*
                 * Public full-service response
                 * must never expose inactive or
                 * incomplete services.
                 */
                if (!service.isActive ||
                    !service.isComplete) {
                ...
                /*
                 * Find globally active tiers that
                 * are currently attached to this service.
                 */
                ...
                /*
                 * Keep only service locations that
                 * correspond to active locations from
                 ...
                 * active pricing and active components.
                 */
                const [components, pricing, componentDetails,] = await Promise.all([
                    ...
                    Component.find({
                        ...
                        .select("name imageUrl isRemovable isBundled isActive")
                        .lean(),
                ]);
                /*
                 * Collect all component-item IDs used
                 * by the selected service components.
                 */
                const itemIds = components.flatMap((component) => component.items?.map((item) => item.itemId) ?? []);
                /*
                 * Only active component items should
                 * be exposed publicly.
                 */
                ...
                for (const component of components) {
                    ...
                    /*
                     * Defensive check.
                     * Never expose inactive tiers.
                     */
                    if (!activeTierIdSet.has(tierId)) {
                        continue;
                    }
                    const componentInfo = componentMap.get(component.componentId
                        .toString());
                    /*
                    ...
                    const componentPricing = pricingMap.get(pricingKey) ?? [];
                    /*
                     * If there is no active pricing for
                     * this component in the requested
                     * locations, don't expose it.
                     */
                    if (componentPricing.length ===
                        0) {
                        continue;
                    }
                    /*
                     * Remove inactive component items
                     * while keeping details for active ones.
                     */
                    const activeItems = (component.items ?? [])
                        ...
                        .map((item) => ({
                        ...item,
                        itemDetails: itemMap.get(item.itemId
                            .toString()),
                    }));
                    if (!grouped[tierId]) {
                        grouped[tierId] = {
                ...
                /*
                 * Return only active tiers that actually
                 * contain usable components/pricing.
                 */
                const filteredTiers = service.tiers.filter((tier) => {
                    const tierId = tier.tierId
                        .toString();
                    return (activeTierIdSet.has(tierId) &&
                        Boolean(grouped[tierId]));
                });
                return {
                    service: {
    ...
    static async validateServiceConfiguration(serviceId) {
        const evaluation = await ServiceCascadingEngine
            .evaluateConfiguration(serviceId);
        /*
         * Persist the derived completeness state,
         * but never automatically activate.
         *
         * If configuration has become invalid,
        ...
        const updated = await Service.findByIdAndUpdate(serviceId, {
            $set: update,
        }, {
            new: true,
            runValidators: true,
    ...
    static async exportServicesToCsv(serviceIds) {
        ...
        if (services.length ===
            0) {
            throw createHttpError("No services found for export", 404);
        }
        /*
         * Preserve the order supplied by
         * the admin/frontend.
         */
        ...
        const escapeCsv = (value) => {
            ...
            /*
             * Prevent CSV / spreadsheet formula
