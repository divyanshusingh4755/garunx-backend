...
export class ServiceService {
    ...
    static async buildFullServiceData(service, options) {
        ...
        const candidateLocations = publicView ? (service.locations ?? []).filter((location) => location.isActive) : (service.locations ?? []);
        ...
        const serviceComponentQuery = { serviceId: service._id };
        const pricingQuery = { serviceId: service._id };
        if (publicView) {
        ...
        const componentQuery = { _id: { $in: componentIds.map((id) => new Types.ObjectId(id)) } };
        if (publicView) {
            componentQuery.isActive = true;
        }
        ...
        const itemQuery = { _id: { $in: itemIds.map((id) => new Types.ObjectId(id)) } };
        if (publicView) {
        ...
        for (const price of pricing) {
            ...
            if (publicView && (!validTierIdSet.has(tierId) || !validLocationIdSet.has(locationId) || !price.isActive)) {
                continue;
            }
        ...
        for (const serviceComponent of serviceComponents) {
            ...
            if (publicView && !validTierIdSet.has(tierId)) {
                continue;
            }
            ...
            if (publicView && !componentDetails) {
                continue;
            }
            ...
            if (publicView && componentPricing.length === 0) {
                continue;
            }
            // Hydrate component items.
            const hydratedItems = (serviceComponent.items ?? []).filter((item) => !publicView || itemMap.has(item.itemId.toString())).map((item) => {
        ...
        // Locations.
        const filteredLocations = publicView ? (service.locations ?? []).filter((location) => location.isActive && validLocationIdSet.has(location.locationId.toString())) : (service.locations ?? []);
        ...
        return {
            ...
            tiers: filteredTiers.map((tier) => ({ tierId: tier.tierId, name: tier.name, components: grouped[tier.tierId.toString()]?.components ?? [] })),
            components: grouped,
        };
    }
    ...
    static async getFullServiceByCities(serviceId, cityIds) {
        ...
        return RedisCacheService.getOrSet({
            ...
            loader: async () => {
                ...
                const locationMap = new Map(locations.map((location) => [location._id.toString(), { locationId: location._id, locationName: location.name, city: location.cityId }]));
                // Fetch the service.
                const service = await Service.findById(serviceId)
                    .populate({
                    path: "subServiceComponents",
    ...
    static async validateServiceConfiguration(serviceId) {
        ...
        const update = { isComplete: evaluation.isComplete };
    ...
    static async exportServicesToCsv(serviceIds) {
        ...
        const orderedServices = uniqueServiceIds.map((id) => serviceMap.get(id)).filter((service) => Boolean(service));
