...
export class CouponService {
    static async validateCouponReferences(coupon) {
        ...
        if (coupon.applicableOn === "PACKAGE") {
            ...
            if (existingPackageCount !== packageIds.length) {
    ...
    static async invalidateCouponCache(couponId) {
        const operations = [RedisCacheService.deleteByPattern(CacheKeys.couponListPattern())];
        if (couponId) {
    ...
    static async findCoupons(searchTerm, limit = 20, page = 1, isActive, assignedUserId, applicableOn, sortBy = "createdAt", sortOrder = "desc") {
        ...
        return RedisCacheService.getOrSet({
            ...
            loader: async () => {
                ...
                if (applicableOn) {
                    const values = (Array.isArray(applicableOn) ? applicableOn : applicableOn.split(",")).map((item) => item.trim().toUpperCase()).filter(Boolean);
                    if (assignedUserId) {
