...
class CartService {
    ...
    static normalizeCommissionPercentage(value) {
    ...
    static getCommissionBaseFromLine(line) {
        ...
        if (line.tax) {
            return this.round(line.tax.taxableAmount);
        }
        return this.round(Math.max(0, line.amount - line.discountAmount));
    }
    static async applyCommissionResults(cart, totals, session) {
        if (cart.serviceId) {
            ...
            if (!service)
                throw new Error("Service not found while calculating commission");
        ...
        if (!pkg) {
            throw new Error("Package not found while calculating commission");
        }
            ...
            return {
                ...
                components: components.map((component) => {
                    if (!component.componentId) {
                        throw new Error(`Invalid ServiceComponent for service "${service.name}": componentId is missing`);
                    }
        ...
        for (const serviceId of serviceIds) {
            ...
            selectedServices.push({
                ...
                components: (serviceComponentMap.get(serviceIdString) ?? []).map((component) => {
                    if (!component.componentId) {
                        throw new Error(`Invalid ServiceComponent for service "${matchedService.name}": componentId is missing`);
                    }
                    if (!Types.ObjectId.isValid(component.componentId.toString())) {
                        throw new Error(`Invalid ServiceComponent for service "${matchedService.name}": componentId is invalid`);
                    }
                    const items = (component.items ?? []).map((item) => {
                        if (!item.itemId) {
    ...
    static async recalculateCart(owner, cartId, options) {
        ...
        if (oldValues.commissionPercentage !== cart.commissionPercentage) {
            changes.push(`Commission percentage changed from ${oldValues.commissionPercentage} to ${cart.commissionPercentage}`);
        }
        if (oldValues.commissionBaseAmount !== cart.commissionBaseAmount) {
            changes.push(`Commission base changed from ${oldValues.commissionBaseAmount} to ${cart.commissionBaseAmount}`);
        }
        if (oldValues.commissionAmount !== cart.commissionAmount) {
            changes.push(`Commission amount changed from ${oldValues.commissionAmount} to ${cart.commissionAmount}`);
        }
    ...
    static async rebuildPackageServiceComponents(cart, session) {
        if (!cart.packageId) {
            return false;
        }
        ...
        const rebuildServices = (services) => {
            for (const service of services) {
                const serviceId = service.serviceId.toString();
                const currentComponents = serviceComponentMap.get(serviceId) ?? [];
                const rebuiltComponents = currentComponents.map((component) => {
                    if (!component.componentId) {
    ...
    static async validateCart(owner, cartId, persist, session) {
        ...
        if (cart.serviceId) {
            ...
            for (const [index, component,] of (cart.selectedComponents ?? []).entries()) {
                ...
                if (!Types.ObjectId.isValid(component.componentId.toString())) {
                    errors.push(`Invalid selected componentId at index ${index}`);
                    continue;
                }
            ...
            if ((cart.selectedComponents ??
                []).length === 0) {
                errors.push("No components selected for service");
            }
            // Service addon components
            for (const [index, component,] of (cart.addonComponents ??
                []).entries()) {
                if (!component?.componentId) {
        ...
        if (cart.packageId) {
            ...
            try {
                const repaired = await this.rebuildPackageServiceComponents(cart, session);
                if (repaired) {
                    changes.push("Package service component snapshots were refreshed");
                }
                if (persist) {
            ...
            // Package
            const packageQuery = Package.findById(cart.packageId);
            if (session) {
                packageQuery.session(session);
            }
            const pkg = await packageQuery.lean();
            if (!pkg) {
                ...
                const tierExists = pkg.tiers.some((tier) => tier.tierId.toString() ===
                    cart.tierId.toString());
                if (!tierExists) {
                    errors.push("Selected package tier is no longer available");
                }
                const location = pkg.locations.find((location) => location.locationId.toString() ===
                ...
                // ---------------------------------------------------
                const mappingQuery = PackageTierMap.findOne({
                ...
                const packageTierMap = await mappingQuery.lean();
                if (!packageTierMap) {
                    ...
                    // INCLUDED SERVICES
                    // ===============================================
                    for (const [serviceIndex, service,] of (cart.selectedServices ??
                        []).entries()) {
                        if (!service?.serviceId) {
                        ...
                        const mappedService = packageTierMap.services?.find((mapped) => mapped.serviceId
                            ?.toString() ===
                            serviceId);
                        if (!mappedService) {
                        ...
                        // Components
                        // ---------------------------------------------
                        for (const [componentIndex, component,] of (service.components ??
                            []).entries()) {
                            if (!component?.componentId) {
                            ...
                            for (const [itemIndex, item,] of (component.items ??
                                []).entries()) {
                                if (!item?.itemId) {
                        ...
                        // Sub-services
                        // ---------------------------------------------
                        for (const [subServiceIndex, subService,] of (service.subServices ??
                            []).entries()) {
                    ...
                    for (const mappedService of packageTierMap.services ??
                        ...
                        if (mappedService.isRequired &&
                            ...
                            errors.push(`Missing required package service: ${mappedService.name}`);
                        }
                    }
                    // ===============================================
                    // ADDON SERVICES
                    // ===============================================
                    for (const [serviceIndex, service,] of (cart.addonServices ??
                        []).entries()) {
                        if (!service?.serviceId) {
                        ...
                        for (const [componentIndex, component,] of (service.components ??
                            ...
                            for (const [itemIndex, item,] of (component.items ??
                                ...
                                if (!Types.ObjectId.isValid(item.itemId.toString())) {
                                    errors.push(`Invalid itemId at index ${itemIndex} in component "${component.name}" for addon service "${service.name}"`);
                                }
                            }
                        }
                        // ---------------------------------------------
