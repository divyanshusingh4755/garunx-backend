...
class CartService {
    ...
    static async getCartById(userId, cartId) {
        ...
        if (cart.serviceId) {
            ...
            const componentIds = service.subServiceComponents
                ?.map((c) => c.componentId)
                ?.filter(Boolean) || [];
            const componentMap = new Map((await Component.find({
                _id: { $in: componentIds },
            }).lean()).map((c) => [c._id.toString(), c]));
            const itemIds = service.subServiceComponents
                ?.flatMap((c) => c.items?.map((i) => i.itemId))
                ?.filter(Boolean) || [];
            const itemMap = new Map((await ComponentItem.find({
                _id: { $in: itemIds },
    ...
    static async updateSelectedComponents(userId, cartId, payload) {
        ...
        const missingRequired = serviceComponents
            ...
            .filter((c) => {
            return !selectedComponents?.some((sc) => sc.componentId === c.componentId.toString());
        });
