...
export class PolicyService {
    ...
    static async getAllPolicies(page = 1, limit = 20, isActive, type, userType) {
        ...
        const safeLimit = Number.isInteger(limit) &&
            ...
            ? Math.min(limit, 100)
            : 20;
        const cacheKey = CacheKeys.policyList({
            page: safePage,
            limit: safeLimit,
            isActive,
            type,
    ...
    static async exportPoliciesToCsv(policyIds) {
        ...
        const policies = await Content.find({
            _id: {
                $in: policyObjectIds,
            },
        })
        ...
        const orderedPolicies = uniquePolicyIds
        ...
        const rows = orderedPolicies.map((policy) => [
            policy._id
                .toString(),
            policy.type,
            policy.userType,
