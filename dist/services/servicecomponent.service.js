...
export class ServiceComponentService {
    static async bulkUpsertComponents(payload) {
        ...
        const componentMap = new Map(dbComponents.map((c) => [c._id.toString(), c]));
        ...
        for (const comp of components) {
            ...
            if (dbComp.isBundled) {
                ...
                comp.items.forEach((id) => allItemIds.add(id));
            }
            else {
                if (comp.items && comp.items.length > 0) {
        ...
        const newComponentIds = components.map((c) => c.componentId);
    ...
    static async replaceComponents(payload) {
        ...
        try {
            ...
            const componentObjectIds = componentIds.map((id) => {
                if (!Types.ObjectId.isValid(id)) {
                    throw new Error(`Invalid componentId: ${id}`);
                }
                return new Types.ObjectId(id);
    ...
    static async patchComponent(payload) {
        ...
        const tierExists = service.tiers.some((t) => t.tierId.toString() === tierId);
        if (!tierExists)
            throw new Error("Tier not found in service");
        ...
        await ServiceComponent.updateOne({ serviceId, tierId, componentId }, { $set: updateData });
        await ServiceCascadingEngine.run(serviceId);
        return {
            success: true,
...
//# sourceMappingURL=servicecomponent.service.js.map