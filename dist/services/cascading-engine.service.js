...
export class ServiceCascadingEngine {
    ...
    static async cleanupPricing(service, session) {
        ...
        const components = await ServiceComponent.find({ serviceId: service._id }).session(session).select("tierId componentId").lean();
        const validComponentKeys = new Set(components.map((component) => `${component.tierId.toString()}_${component.componentId.toString()}`));
        const pricing = await ServicePricing.find({ serviceId: service._id }).session(session).select("_id tierId locationId componentId").lean();
        const deleteIds = [];
    ...
    static async evaluateConfigurationForService(service, session) {
        ...
        const requiredComponents = await componentQuery.lean();
        ...
        if (requiredComponents.length > 0) {
            ...
            if (activeComponentCount !== requiredComponentIds.length) {
        ...
        for (const tier of service.tiers) {
            ...
            if (!tierComponents || tierComponents.length === 0) {
        ...
        const pricing = await pricingQuery.lean();
    ...
    static async computeStartingPrice(service, session) {
        const requiredComponents = await ServiceComponent.find({ serviceId: service._id, isRequired: true }).session(session).select("tierId componentId").lean();
        if (requiredComponents.length === 0) {
            return 0;
        }
        ...
        const pricing = await ServicePricing.find({ serviceId: service._id, isActive: true }).session(session).select("tierId locationId componentId price").lean();
        const pricingMap = new Map();
        for (const row of pricing) {
            const tierId = row.tierId.toString();
            const locationId = row.locationId.toString();
            if (!validTierIds.has(tierId) || !activeLocationIds.has(locationId)) {
                continue;
            }
