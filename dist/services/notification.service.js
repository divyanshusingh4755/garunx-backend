...
export class NotificationService {
    static isCategoryEnabled(params) {
        ...
        switch (category) {
            case "BOOKING":
                return preferences.categories.booking;
            case "PAYMENT":
                return preferences.categories.payment;
            case "QUERY":
                return preferences.categories.query;
            case "REVIEW":
                return preferences.categories.review;
            case "PROMOTIONAL":
                return preferences.categories.promotional;
            case "APP_UPDATE":
                return preferences.categories.appUpdate;
            case "NEW_FEATURE":
                return preferences.categories.newFeature;
            case "SYSTEM":
                return false;
        }
    }
    static async emitUnreadCount(userId) {
        ...
        const recipientId = typeof userId === "string"
        ...
        io.to(getUserRoom(recipientId.toString())).emit("notification:unread-count", {
            unreadCount,
        });
    }
    ...
    static async markAsRead(params) {
        ...
        if (io && notification.readAt) {
            io.to(getUserRoom(userId)).emit("notification:read", {
    ...
    static async markAllAsRead(userId) {
        ...
        if (io && result.modifiedCount > 0) {
            io.to(getUserRoom(userId)).emit("notification:read-all", {
    ...
    static async deleteNotification(params) {
        ...
        const io = getSocketServer();
        if (io) {
            io.to(getUserRoom(userId)).emit("notification:deleted", {
                notificationId: notification._id.toString(),
            });
            await this.emitUnreadCount(notification.recipientId);
        }
    ...
    static async createFromTemplate(params) {
         ...
         */
        const rendered = await NotificationTemplateService
            .renderTemplate({
            templateCode,
            variables,
        });
        ...
        if (rendered.preferenceMode ===
            "OPTIONAL") {
            const preferences = await NotificationPreferenceService
                .getOrCreatePreferences(recipientId.toString());
        ...
        /*
         * 2. Determine requested delivery channels
         *
        ...
        const { notification, created, } = await this.createNotification({
            ...
            ...(pushRequested && {
                ...
                pushMessage: rendered.pushMessage,
            }),
        });
        /*
         * 4. Duplicate notification
         *
         * createNotification() returns created:false
         * when the dedupeKey already exists.
         *
        ...
        if (emailRequested) {
            try {
                ...
                if (!user?.email) {
                    ...
                    await Notification.updateOne({
                        ...
                        $set: {
                            ...
                            "emailDelivery.messageId": emailResult.messageId,
                        },
                        $unset: {
                ...
                console.error(`Failed to send notification email to user ${recipientId.toString()}:`, error);
            }
        }
        /*
         * ------------------------------------------------
        ...
        if (pushRequested) {
            try {
                const pushResult = await NotificationDeviceService
                    .sendToUser({
                    userId: recipientId.toString(),
                    title: rendered.pushTitle,
                    message: rendered.pushMessage,
                    data: {
                        notificationId: notification._id.toString(),
                ...
                if (pushResult.attempted === 0) {
                    ...
                    const setData = {
                        ...
                        "pushDelivery.attemptedCount": pushResult.attempted,
                        "pushDelivery.successCount": pushResult.successCount,
                        "pushDelivery.failureCount": pushResult.failureCount,
                    };
                    /*
                     * At least one device received it.
                     */
                    if (pushResult.successCount > 0) {
                        setData["pushDelivery.sentAt"] = now;
                    }
                    /*
                     * All attempts failed.
                     */
                    if (pushStatus === "FAILED") {
                        setData["pushDelivery.failedAt"] = now;
                    }
                    const unsetData = {
                        "pushDelivery.error": "",
                    };
                    if (pushStatus !== "FAILED") {
                        unsetData["pushDelivery.failedAt"] = "";
                    }
                    await Notification.updateOne({
                        _id: notification._id,
                    }, {
                ...
                console.error(`Failed to send push notification to user ${recipientId.toString()}:`, error);
            }
        }
    ...
    static async retryEmail(notificationId) {
        ...
        if (notification.emailDelivery.status !==
            "FAILED") {
            throw new Error("Only failed notification emails can be retried");
        }
    ...
    static async retryPush(notificationId) {
        ...
        if (![
            "FAILED",
            "PARTIAL",
        ].includes(notification.pushDelivery.status)) {
        ...
        try {
            const result = await NotificationDeviceService
                .sendToUser({
            ...
            await Notification.updateOne({
                ...
                $set: {
                    ...
                    ...(result.successCount > 0 && {
                        "pushDelivery.sentAt": now,
                    }),
                    ...(status === "FAILED" && {
                        "pushDelivery.failedAt": now,
                    }),
                    ...(errorMessage && {
                        "pushDelivery.error": errorMessage,
                    }),
