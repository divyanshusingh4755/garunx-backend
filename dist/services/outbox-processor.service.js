...
export class OutboxProcessorService {
    ...
    static getRetryDelay(attempts) {
         ...
         * Maximum 15 minutes.
         */
        return Math.min(10_000 *
            Math.pow(2, Math.max(attempts - 1, 0)), 15 * 60 * 1000);
    }
    static async claimNextEvent() {
        const now = new Date();
        const staleBefore = new Date(now.getTime() -
        ...
        return OutboxEvent
            ...
            $or: [
                ...
                /*
                 * PROCESSING event whose
                 * worker probably died.
                 */
                {
                    status: "PROCESSING",
                    attempts: {
    ...
    static async processOne() {
        ...
        try {
            await NotificationEventHandler
                .handle({
