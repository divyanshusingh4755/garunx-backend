...
export class PackageCascadingEngine {
    static async evaluateConfiguration(packageId) {
        ...
        try {
            let result = { isComplete: false, issues: [], startingPrice: 0 };
    ...
    static async cleanupPricing(packageDocument, session) {
        ...
        const mappings = await PackageTierMap.find({ packageId: packageDocument._id }).session(session).select("tierId services.serviceId").lean();
        ...
        const pricing = await PackageTierPricing.find({ packageId: packageDocument._id }).session(session).select("_id tierId locationId serviceId").lean();
        const deleteIds = [];
    ...
    static async computeStartingPrice(packageDocument, session) {
        ...
        const [mappings, pricingRows,] = await Promise.all([
            PackageTierMap.find({ packageId: packageDocument._id }).session(session).select("tierId services.serviceId services.isRequired").lean(),
            PackageTierPricing.find({ packageId: packageDocument._id }).session(session).select("tierId locationId serviceId finalPrice").lean(),
        ]);
        ...
        for (const pricing of pricingRows) {
            ...
            if (typeof pricing.finalPrice !== "number" || !Number.isFinite(pricing.finalPrice)) {
                continue;
            }
    ...
    static async computeIsComplete(packageDocument, session) {
        ...
        if (globalActiveLocationCount !== activeLocationIds.length) {
            return false;
        }
        ...
        if (globalActiveTierCount !== tierIds.length) {
            return false;
        }
        ...
        if (activeServiceCount !== requiredServiceIds.length) {
            return false;
        }
        // 9. Load PackageTierPricing.
        const pricing = await PackageTierPricing.find({ packageId: packageDocument._id }).session(session).select("tierId locationId serviceId").lean();
