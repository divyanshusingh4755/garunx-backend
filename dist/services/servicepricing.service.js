...
export class ServicePricingService {
    static async bulkUpsertTierPricing(payload) {
        ...
        const serviceLocationIds = new Set(service.locations.map((location) => location.locationId.toString()));
        ...
        for (const locationPricing of pricing) {
            ...
            if (locationIdsSeen.has(locationId)) {
                throw new Error(`Duplicate location in pricing: ${locationId}`);
            }
            locationIdsSeen.add(locationId);
            if (!serviceLocationIds.has(locationId)) {
                throw new Error(`Location ${locationId} does not belong to service`);
            }
            if (!Array.isArray(locationPricing.components) ||
                locationPricing.components.length ===
                    0) {
                throw new Error(`Location ${locationId} must contain components`);
            }
            const componentIdsSeen = new Set();
            for (const componentPricing of locationPricing.components) {
                ...
                if (componentIdsSeen.has(componentId)) {
                    throw new Error(`Duplicate component ${componentId} for location ${locationId}`);
                }
                ...
                if (typeof componentPricing.price !==
                    ...
                    throw new Error(`Invalid price for component ${componentId}`);
                }
                const taxProfileId = componentPricing.taxProfileId;
                if (taxProfileId) {
                    if (!Types.ObjectId.isValid(taxProfileId)) {
                        throw new Error(`Invalid taxProfileId: ${taxProfileId}`);
                    }
                    allTaxProfileIds.add(taxProfileId);
                }
                const taxPriceMode = componentPricing.taxPriceMode ??
                    "EXCLUSIVE";
                if (taxPriceMode !== "EXCLUSIVE" &&
                    taxPriceMode !== "INCLUSIVE") {
                    throw new Error(`Invalid taxPriceMode for component ${componentId}`);
                }
            }
        }
        /*
         ...
         */
        const componentObjectIds = Array.from(allComponentIds).map((id) => new Types.ObjectId(id));
        ...
        const invalidComponentIds = Array.from(allComponentIds).filter((componentId) => !validComponentSet.has(componentId));
        if (invalidComponentIds.length > 0) {
            throw new Error(`Components do not belong to this service tier: ${invalidComponentIds.join(", ")}`);
        }
        /*
         * Validate all tax profiles in one query.
         */
        const taxProfileIds = Array.from(allTaxProfileIds);
        if (taxProfileIds.length > 0) {
            ...
            const validTaxProfiles = await TaxProfile.find({
                ...
                .select("_id")
                .lean();
            const validTaxProfileSet = new Set(validTaxProfiles.map((profile) => profile._id.toString()));
            const invalidTaxProfileIds = taxProfileIds.filter((id) => !validTaxProfileSet.has(id));
            if (invalidTaxProfileIds.length > 0) {
                throw new Error(`Inactive, expired, future, or invalid tax profiles: ${invalidTaxProfileIds.join(", ")}`);
            }
        }
        const bulkOps = [];
        const requestedPricingKeys = new Set();
        /*
            ...
            for (const componentPricing of locationPricing.components) {
                ...
                const taxPriceMode = taxProfileId
                    ? componentPricing.taxPriceMode ??
                        "EXCLUSIVE"
                    : "EXCLUSIVE";
                requestedPricingKeys.add(`${locationId}_${componentId}`);
                bulkOps.push({
                    updateOne: {
                        filter: {
         ...
         * are deleted.
         */
        const requestedPairs = Array.from(requestedPricingKeys).map((key) => {
            const separatorIndex = key.indexOf("_");
            const locationId = key.slice(0, separatorIndex);
            const componentId = key.slice(separatorIndex + 1);
            return {
                locationId: new Types.ObjectId(locationId),
                componentId: new Types.ObjectId(componentId),
            };
    ...
    `)
      ...
      `,
        ...
        const resolvedComponents = components.map((component) => {
            ...
            const taxProfile = pricingRecord?.taxProfileId;
            return {
                ...
                tax: pricingRecord
                    ? {
                        ...
                        priceMode: pricingRecord.taxProfileId
