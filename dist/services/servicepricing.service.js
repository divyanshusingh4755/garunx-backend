    ...
    static async bulkUpsertTierPricing(payload) {
        ...
        for (const locationPricing of pricing) {
            ...
            if (locationIdsSeen.has(locationId)) {
                throw createHttpError(`Duplicate location in pricing: ${locationId}`, 400);
            }
            locationIdsSeen.add(locationId);
            if (!serviceLocationIds.has(locationId)) {
                throw createHttpError(`Location ${locationId} does not belong to service`, 400);
            }
            ...
            for (const componentPricing of locationPricing.components) {
                ...
                if (componentIdsSeen.has(componentId)) {
                    throw createHttpError(`Duplicate component ${componentId} for location ${locationId}`, 400);
                }
        ...
        if (taxProfileIds.length > 0) {
            ...
            if (invalidTaxProfileIds.length > 0) {
                throw createHttpError(`Inactive or invalid tax profiles: ${invalidTaxProfileIds.join(", ")}`, 400);
            }
        }
