...
export class ComponentItemService {
    static async invalidateComponentItemCache(componentItemId) {
        const operations = [
            RedisCacheService.deleteByPattern(CacheKeys.componentItemListPattern()),
        ];
        if (componentItemId) {
    ...
    static async invalidateAffectedServiceCaches(componentItemId) {
        const mappings = await ServiceComponent.find({
            items: {
                $elemMatch: {
                    itemId: new Types.ObjectId(componentItemId),
                },
            },
        })
            .select("serviceId")
            .lean();
        const serviceIds = [
            ...new Set(mappings.map((mapping) => mapping.serviceId
                .toString())),
        ];
    ...
    static async getAllComponentItems(params) {
        ...
        const safeSortBy = isTextSearch &&
            ...
            : allowedSortFields.has(sortBy)
                ? sortBy
                : "createdAt";
        const cacheKey = CacheKeys.componentItemList({
            searchTerm: term,
            limit: safeLimit,
            page: safePage,
            isActive,
    ...
    static async exportComponentItemsToCsv(componentItemIds) {
        const uniqueIds = [
            ...new Set(componentItemIds),
        ];
        const componentItems = await ComponentItem.find({
            _id: {
                $in: uniqueIds,
            },
        })
            .select("_id name price isActive createdAt updatedAt")
            .lean();
        ...
        const rows = componentItems.map((item) => [
            item._id.toString(),
            item.name,
            item.price ?? "",
            item.isActive
