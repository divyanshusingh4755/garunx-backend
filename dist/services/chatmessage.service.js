...
export class ChatMessageService {
    ...
    static async sendMessage(params) {
        ...
        if (normalizedClientMessageId.length > 128) {
        ...
        const messageData = { conversationId: conversation._id, senderId: new Types.ObjectId(senderId), type, clientMessageId: normalizedClientMessageId, };
        if (normalizedText) {
            messageData.text = normalizedText;
        }
        if (normalizedImages.length > 0) {
            messageData.images = normalizedImages;
        }
        if (replyToMessageId) {
        ...
        try {
            try {
                await session.withTransaction(async () => {
                    ...
                    const conversationUpdate = await ChatConversation.updateOne({ _id: conversation._id, status: ChatConversationStatus.ACTIVE, "participants.userId": new Types.ObjectId(senderId), }, { $max: { lastMessageAt: message.createdAt, }, }, { session, });
                    if (conversationUpdate.matchedCount === 0) {
    ...
    static async getMessages(params) {
        ...
        const replyMessageMap = new Map(replyMessages.map((message) => [message._id.toString(), message,]));
