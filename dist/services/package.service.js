...
export class PackageService {
    ...
    static async buildFullPackageData(pkg, options) {
        ...
        const services = serviceIds.length > 0
            ? await Service.find(serviceQuery)
                .select([
                "_id",
                "name",
                "categoryId",
        ...
        const categories = categoryIds.length > 0
            ? await Category.find({
                _id: {
                    $in: categoryIds.map((categoryId) => new Types.ObjectId(categoryId)),
                },
            })
        ...
        for (const mapping of mappings) {
            ...
            for (const mappedService of mapping.services ??
                ...
                const category = serviceDetails
                    ?.categoryId
                    ? categoryMap.get(serviceDetails
                        .categoryId
                        .toString())
                    : null;
                grouped[tierId].services.push({
                    ...
                    name: serviceDetails
                        ...
                        mappedService
                            .name,
                    shortDescription: serviceDetails
                        ?.shortDescription ??
                        null,
                     ...
                     */
                    isActive: serviceDetails
                        ?.isActive ??
                        false,
                    isComplete: serviceDetails
                        ?.isComplete ??
                        false,
                    category: category
                        ? {
        ...
        const packageCategory = pkg.categoryId
            ? categoryMap.get(pkg.categoryId
                .toString())
            : null;
        /*
         * -------------------------------------------------
    ...
    static async findPackages(searchTerm, categoryId, locationId, tierId, limit = 20, page = 1, isActive, isComplete, sortBy = "createdAt", sortOrder = "desc") {
        ...
        const useTextSearch = Boolean(term &&
            term.length > 4);
        /*
        ...
        const safeSortBy = useTextSearch &&
            sortBy === "relevance"
            ? "relevance"
         ...
         */
        const cacheKey = CacheKeys.packageList({
            searchTerm: term,
            categoryId,
            locationId,
            tierId,
    ...
    static async removePackageLocation(packageId, locationId) {
        ...
        try {
            await session.withTransaction(async () => {
                ...
                const exists = pkg.locations.some((location) => location.locationId
                    .toString() ===
                    locationId);
                if (!exists) {
                    alreadyMissing =
                ...
                pkg.locations =
                    pkg.locations.filter((location) => location.locationId
                        .toString() !==
                        locationId);
                await pkg.save({
                    session,
    ...
    static async removePackageTier(packageId, tierId) {
        ...
        try {
            await session.withTransaction(async () => {
                ...
                const exists = pkg.tiers.some((tier) => tier.tierId
                    .toString() ===
                    tierId);
                if (!exists) {
                    alreadyMissing =
                ...
                pkg.tiers =
                    pkg.tiers.filter((tier) => tier.tierId
                        .toString() !==
                        tierId);
                await pkg.save({
                    session,
    ...
    static async getFullPackage(packageId) {
        ...
        return RedisCacheService.getOrSet({
            ...
            loader: async () => {
                ...
                return this.buildFullPackageData(pkg, {
                    publicView: true,
                });
            },
        });
    ...
    static async getRelatedPackageService(packageId, tierId, locationId) {
        ...
        return RedisCacheService.getOrSet({
            ...
            loader: async () => {
                ...
                const packageLocation = pkg.locations.find((item) => item.locationId
                    .toString() ===
                    locationId);
                if (!packageLocation) {
                ...
                const relatedServices = (mapping.services ??
                    []).filter((service) => service.isRelated);
                const serviceIds = relatedServices.map((service) => service.serviceId
                    .toString());
                /*
                ...
                const services = await Service.find({
                    ...
                    .select("_id categoryId thumbnailImage tiers locations")
                    .lean();
                /*
                ...
                const validServices = services.filter((service) => {
                    const hasTier = service.tiers.some((serviceTier) => serviceTier.tierId
                        .toString() ===
                        tierId);
                    const hasLocation = service.locations.some((location) => location.locationId
                        .toString() ===
                        locationId &&
                        location.isActive);
                    return (hasTier &&
                ...
                const validServiceIds = new Set(validServices.map((service) => service._id
                    .toString()));
                const filteredRelatedServices = relatedServices.filter((service) => validServiceIds.has(service.serviceId
                    .toString()));
                const categoryIds = [
                    ...new Set([
                        pkg.categoryId
                ...
                const serviceMap = new Map(validServices.map((service) => [
                    service._id
                        .toString(),
                    {
                ...
                const pricingMap = new Map(pricing
                    .filter((price) => validServiceIds.has(price.serviceId
                    .toString()))
                    .map((price) => [
                    price.serviceId
                        .toString(),
                ...
                const hydratedRelatedServices = filteredRelatedServices.map((service) => {
                    const serviceDetails = serviceMap.get(service.serviceId
                        .toString());
                    const category = serviceDetails
                        ?.categoryId
                        ? categoryMap.get(serviceDetails
                    ...
                    return {
                        ...
                        category: category
                            ...
                            : null,
                        pricing: pricingMap.get(service.serviceId
                            .toString()) ??
                            null,
                    };
                });
    ...
    static async validatePackageConfiguration(packageId) {
        const evaluation = await PackageCascadingEngine
            .evaluateConfiguration(packageId);
        /*
        ...
        const updated = await Package.findByIdAndUpdate(packageId, {
            $set: update,
        }, {
            new: true,
            runValidators: true,
        ...
        return RedisCacheService.getOrSet({
            ...
            loader: async () => {
                ...
                const filteredLocations = pkg.locations
                    .filter((location) => location.isActive &&
                    locationIds.some((id) => id.toString() ===
                        location.locationId
                            .toString()))
                    .map((loc) => ({
                    ...loc,
    ...
    static async exportPackagesToCsv(packageIds) {
        ...
        const packages = await Package.find({
            _id: {
                $in: uniquePackageIds.map((packageId) => new Types.ObjectId(packageId)),
            },
        })
            .select([
            "packageReference",
