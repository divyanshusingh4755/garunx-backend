...
export class NotificationQueueService {
    static async enqueueEmail(notificationId) {
        return notificationEmailQueue.add("send-email", {
            notificationId,
        }, {
    ...
    static async enqueuePush(notificationId) {
        return notificationPushQueue.add("send-push", {
            notificationId,
        }, {
    ...
    static async enqueueEmailRetry(notificationId) {
        return notificationEmailQueue.add("send-email", {
            ...
            jobId: `email-retry-${notificationId}-${Date.now()}`,
        });
    }
    static async enqueuePushRetry(notificationId) {
        return notificationPushQueue.add("send-push", {
            ...
            jobId: `push-retry-${notificationId}-${Date.now()}`,
        });
    }
}
