...
export class BannerService {
    static async invalidateBannerCache(bannerId) {
        const operations = [
            RedisCacheService.deleteByPattern(CacheKeys.bannerListPattern()),
        ];
        if (bannerId) {
    ...
    static async findBanners(searchTerm, placement, format, redirectType, limit = 20, page = 1, isActive, sortBy = "displayOrder", sortOrder = "asc") {
         ...
         */
        const cacheKey = CacheKeys.bannerList({
    ...
    static async exportBannersToCsv(bannerIds) {
        ...
        const bannerMap = new Map(banners.map((banner) => [
            banner._id
                .toString(),
            banner,
        ]));
         ...
         */
        const orderedBanners = uniqueBannerIds
            .map((bannerId) => bannerMap.get(bannerId))
        ...
        const rows = orderedBanners.map((banner) => [
            banner._id
                .toString(),
            banner.version,
            banner.name,
