...
export class ReviewService {
    ...
    static async getAllReviews(params) {
        ...
        const safeSortBy = allowedSortFields.has(sortBy)
            ? sortBy
            : "createdAt";
        const cacheKey = CacheKeys.reviewList({
            searchTerm,
            direction,
            visibility,
            moderationStatus,
        ...
        try {
            ...
            await session.withTransaction(async () => {
                ...
                if (hasRating &&
                    ...
                    const contributesToRating = reviewDocument.visibility === "PUBLISHED" &&
    ...
    static async moderateReviewService(input) {
        ...
        try {
            ...
            await session.withTransaction(async () => {
                ...
                const wasContributing = reviewDocument.visibility === "PUBLISHED" &&
                    !reviewDocument.isDeleted;
                switch (action) {
                    case "HIDE":
                ...
                const isContributing = reviewDocument.visibility === "PUBLISHED" &&
                    !reviewDocument.isDeleted;
                ...
                if (action === "DELETE") {
                    const booking = await Booking.findById(reviewDocument.bookingId)
                    ...
                    await OutboxService.createEvent({
                        ...
                        payload: {
                            ...
                            direction: reviewDocument.direction,
                            reason: reviewDocument.deletionReason ??
    ...
    static async getMyReviews(params) {
        ...
        const safeSortBy = allowedSortFields.has(sortBy)
            ? sortBy
            : "createdAt";
        const cacheKey = CacheKeys.myReviewList({
            userId,
            rating,
            direction,
            limit: safeLimit,
    ...
    static async getCoordinatorReviews(params) {
        ...
        const safeSortBy = allowedSortFields.has(sortBy)
            ? sortBy
            : "createdAt";
        const cacheKey = CacheKeys.coordinatorReviewList({
            coordinatorId,
            rating,
            limit: safeLimit,
            page: safePage,
    ...
    static async exportReviewsToCsv(reviewIds) {
        ...
        const orderedReviews = uniqueReviewIds
