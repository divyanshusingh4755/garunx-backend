...
import { TaxProfileService, type TaxProfileFilters, type CreateTaxProfilePayload, type UpdateTaxProfilePayload } from "../services/taxprofile.service.js";
...
export class TaxProfileController {
  ...
  static async list(req: Request, res: Response) {
    try {
      ...
      if (typeof req.query.treatment === "string") { filters.treatment = req.query.treatment as TaxTreatment; }
      ...
      if (Number.isInteger(limit) && limit > 0) { filters.limit = Math.min(limit, 100); }
  ...
  static async getById(req: Request, res: Response) {
    try {
      const taxProfile = await TaxProfileService.getTaxProfileById(req.params.taxProfileId as string);
  ...
  static async update(req: Request, res: Response) {
    try {
      ...
      const payload: UpdateTaxProfilePayload = { updatedBy: adminId };

      if (Object.prototype.hasOwnProperty.call(req.body, "name")) { payload.name = req.body.name; }
      if (Object.prototype.hasOwnProperty.call(req.body, "treatment")) { payload.treatment = req.body.treatment; }
      if (Object.prototype.hasOwnProperty.call(req.body, "totalRate")) { payload.totalRate = req.body.totalRate; }
      if (Object.prototype.hasOwnProperty.call(req.body, "description")) { payload.description = req.body.description; }

      const taxProfile = await TaxProfileService.updateTaxProfile(req.params.taxProfileId as string, payload);
  ...
  static async updateStatus(req: Request, res: Response) {
    try {
      ...
      const taxProfile = await TaxProfileService.updateTaxProfileStatus(req.params.taxProfileId as string, req.body.isActive, adminId);
  ...
  static async exportCsv(req: Request, res: Response,) {
    try {
      const { taxProfileIds }: { taxProfileIds: string[] } = req.body;
