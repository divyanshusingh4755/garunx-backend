...
const getErrorStatus = (error: unknown,): number => {
  ...
  if (error.message === "Policy not found" || error.message.includes("policy not found") || error.message === "No policies found for export") { return 404; }
  return 400;
};
...
export const createPolicy = async (req: Request, res: Response) => {
  try {
    ...
    const data = await PolicyService.createPolicy({ type, userType, title, content });
...
export const updatePolicy = async (req: Request, res: Response) => {
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(req.body, "content")) { payload.content = req.body.content; }

    const data = await PolicyService.updatePolicy(req.params.id as string, payload);
...
export const togglePolicyStatus = async (req: Request, res: Response) => {
  try {
    const data = await PolicyService.togglePolicyStatus(req.params.id as string, req.body.isActive);
...
export const getPolicyByType = async (req: Request, res: Response) => {
  try {
    const data = await PolicyService.getPolicyByType(req.params.type as PolicyType, req.query.userType as UserType);
...
export const exportPoliciesCsv = async (req: Request, res: Response) => {
  try {
    const { policyIds }: { policyIds: string[] } = req.body;
