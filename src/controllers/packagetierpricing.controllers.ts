...
const getStatusCode = (error: unknown): number => {
  ...
  if (typeof error === "object" && error !== null && "name" in error && (error as { name?: unknown; }).name === "ValidationError") { return 400; }
...
export const bulkUpsertPackageTierPricing = async (req: Request, res: Response) => {
  try {
...
export const resolvePackagePricing = async (req: Request, res: Response) => {
  try {
    ...
    const data = await PackageTierPricingService.resolvePricing(packageId as string, tierId as string, locationId as string);
