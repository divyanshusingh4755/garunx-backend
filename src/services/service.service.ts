...
export class ServiceService {
  private static async invalidateServiceCache(
    serviceId?:
      string,
  ): Promise<void> {
    const operations:
  ...
  private static async invalidatePackageCaches():
    Promise<void> {
    await Promise.all([
      RedisCacheService.deleteByPattern(
  ...
  private static async buildFullServiceData(
    service: any,
    options?: {
      publicView?: boolean;
    },
    ...
    if (publicView) {
      ...
      validLocationIds =
        activeLocationDocs.map(
          (location) =>
            location._id,
        );
    }

    const validLocationIdSet =
      new Set(
        validLocationIds.map(
          (id) =>
            id.toString(),
        ),
      );
    ...
    for (
      ...
      const key =
        `${tierId}_${price.componentId.toString()}`;
  ...
  static async toggleServiceStatus(
    ...
    if (isActive) {
      ...
      if (!validation.isComplete) {
        throw createHttpError(
          "Service configuration incomplete. Cannot activate.",
          400,
        );
      }
    }
  ...
  static async getServicesByLocation(params: {
    ...
    const cacheKey =
      CacheKeys.serviceByLocationList({
        cityIds,
        categoryIds,
        limit: safeLimit,
        page: safePage,
  ...
  static async removeServiceTier(serviceId: string, tierId: string) {
    ...
    const exists = service.tiers.some(
    ...
    service.tiers = service.tiers.filter(
      (tier) => tier.tierId.toString() !== tierId,
    );
    ...
    return RedisCacheService.getOrSet({
      ...
      loader:
        async () => {
          ...
          return this.buildFullServiceData(
            service,
            {
              publicView:
                true,
  ...
  static async getFullServiceAdmin(
    serviceId: string,
  ) {
    if (
      !Types.ObjectId.isValid(
  ...
  static async getFullServiceByCities(
    serviceId: string,
    cityIds: string[],
  ) {
    const cacheKey =
      CacheKeys.serviceFullByCities(
        serviceId,
        cityIds,
      );
        ...
        const [
          ...
          await Promise.all([
            ...
            Component.find({
              ...
              .select(
                "name imageUrl isRemovable isBundled isActive",
              )
              .lean(),
          ]);
        ...
        for (
          ...
          grouped[
            ...
            component: {
              ...
              image:
                componentInfo.imageUrl,

              isRemovable:
                componentInfo.isRemovable,

              isBundled:
                componentInfo.isBundled,

              isActive:
                componentInfo.isActive,
            },
  ...
  static async validateServiceConfiguration(
    serviceId: string,
  ) {
    const evaluation =
      await ServiceCascadingEngine
  ...
  static async exportServicesToCsv(
    serviceIds: string[],
  ) {
