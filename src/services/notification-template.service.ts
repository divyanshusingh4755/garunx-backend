...
import { NotificationTemplate, type NotificationCategory, type NotificationPreferenceMode } from "../models/notification-template.model.js";
import type { NotificationType } from "../models/notification.model.js";

export class NotificationTemplateService {
    private static renderText(text: string, variables: Record<string, string | number | boolean | Date | null | undefined>): string {
    ...
    private static getRequiredVariables(texts: Array<string | undefined>): string[] {
    ...
    private static validateVariables(requiredVariables: string[], variables: Record<string, string | number | boolean | Date | null | undefined>): void {
    ...
    static async renderTemplate(params: {
        templateCode: string; variables?: Record<string, string | number | boolean | Date | null | undefined>;
        ...
        if (!template) { throw new Error("Notification template not found or inactive"); }

        const textsToValidate: Array<string | undefined> = [template.title, template.message,];

        if (includeEmail) { textsToValidate.push(template.emailSubject, template.emailBody); }
        if (includePush) { textsToValidate.push(template.pushTitle, template.pushMessage); }
    ...
    static async createTemplate(params: { code: string; type: NotificationType; category: NotificationCategory; preferenceMode?: NotificationPreferenceMode; title: string; message: string; emailSubject?: string; emailBody?: string; pushTitle?: string; pushMessage?: string; isActive?: boolean; }) {
        ...
        if (existing) { throw new Error("Notification template code already exists"); }

        return NotificationTemplate.create({ ...params, code, preferenceMode });
    }

    static async getTemplates(params: { page?: number; limit?: number; type?: NotificationType; isActive?: boolean; category?: NotificationCategory; }) {
    ...
    static async updateTemplate(params: { templateId: string; code?: string; type?: NotificationType; title?: string; message?: string; emailSubject?: string; emailBody?: string; pushTitle?: string; pushMessage?: string; isActive?: boolean; category?: NotificationCategory; preferenceMode?: NotificationPreferenceMode; }) {
        ...
        if (!currentTemplate) { throw new Error("Notification template not found"); }
        ...
        if (preferenceMode !== undefined) { updateData.preferenceMode = preferenceMode; }
