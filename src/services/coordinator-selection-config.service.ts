import { CoordinatorSelectionConfig, type CoordinatorSortBy, type CoordinatorSortOrder } from "../models/coordinator-selection-config.model.js";
...
const DEFAULT_SETTINGS: CoordinatorSelectionSettings = {
    matchCaste: false,
    matchGotra: false,
    minRating: 0,
...
export class CoordinatorSelectionConfigService {
    static async getEffectiveConfig(): Promise<CoordinatorSelectionSettings> {
    ...
    static async updateConfig(params: { matchCaste: boolean; matchGotra: boolean; minRating: number; minCompletedBookings: number; autoAssignmentEnabled: | boolean | null; sortBy: CoordinatorSortBy; sortOrder: CoordinatorSortOrder; isActive: boolean; updatedBy: string; }) {
        return CoordinatorSelectionConfig.findOneAndUpdate(
            { key: "DEFAULT" },
            {
                $set: {
