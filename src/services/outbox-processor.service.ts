...
export class OutboxProcessorService {
    private static readonly workerId = `${hostname()}-${process.pid}`;

    private static getRetryDelay(attempts: number) {
    ...
    static async processOne() {
        ...
        try {
            ...
            await OutboxEvent.updateOne(
                { _id: event._id, status: "PROCESSING", lockedBy: this.workerId },
                {
            ...
            await OutboxEvent.updateOne(
                { _id: event._id, lockedBy: this.workerId },
                {
                    $set: {
                        status: "FAILED",
                        error: errorMessage,
