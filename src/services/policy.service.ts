...
export class PolicyService {
  ...
  static async togglePolicyStatus(
    id: string,
    isActive: boolean,
  ) {
    this.ensureValidId(
    ...
    try {
      ...
      if (!updatedPolicy) {
        throw new Error(
          "Policy status update failed",
        );
      }

      await this.invalidatePolicyCache(
        updatedPolicy.type,
        updatedPolicy.userType,
      );
  ...
  static async exportPoliciesToCsv(
    policyIds: string[],
  ) {
    if (
      !Array.isArray(
    ...
    if (
      ...
      throw new Error(
        "A maximum of 1000 policies can be exported at once",
      );
    }
    ...
    /*
     * Preserve the same selection order
