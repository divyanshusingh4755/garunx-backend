...
export class PolicyService {
  private static async invalidatePolicyCache(type?: PolicyType, userType?: UserType): Promise<void> {
  ...
  private static ensureValidId(id: string): void { if (!Types.ObjectId.isValid(id)) { throw new Error("Invalid policy id"); } }
  ...
  static async updatePolicy(id: string, payload: UpdatePolicyPayload) {
    ...
    if (payload.title !== undefined) { policy.title = payload.title; }
    if (payload.content !== undefined) { policy.content = payload.content; }
  ...
  static async getAllPolicies(page = 1, limit = 20, isActive?: boolean, type?: PolicyType, userType?: UserType) {
  ...
  static async togglePolicyStatus(id: string, isActive: boolean) {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        if (isActive) {
          await Content.updateMany(
            { _id: { $ne: policy._id }, type: policy.type, userType: policy.userType, isActive: true },
            { $set: { isActive: false } },
            { session },
          );
      ...
      if (!updatedPolicy) { throw new Error("Policy status update failed"); }
  ...
  static async getPolicyByType(type: PolicyType, userType: UserType) {
    return RedisCacheService.getOrSet({
      ...
      loader: async () => {
        ...
        if (!policy) { throw new Error(`${type} policy not found`); }
        return policy;
      },
    });
  }

  static async exportPoliciesToCsv(policyIds: string[]) {
    if (!Array.isArray(policyIds) || policyIds.length === 0) { throw new Error("At least one policy ID is required"); }
    if (policyIds.length > 1000) { throw new Error("A maximum of 1000 policies can be exported at once"); }

    const uniquePolicyIds = [...new Set(policyIds)];

    for (const policyId of uniquePolicyIds) {
      if (!Types.ObjectId.isValid(policyId)) { throw new Error("Invalid policy ID"); }
    }
    ...
    if (policies.length === 0) { throw new Error("No policies found for export"); }
    ...
    const orderedPolicies = uniquePolicyIds.map((policyId) => policyMap.get(policyId)).filter((policy): policy is NonNullable<typeof policy> => Boolean(policy));
    const escapeCsv = (value: unknown): string => {
      if (value === null || value === undefined) { return ""; } const stringValue = String(value);
