...
export class CartService {
  ...
  async addServiceEntry(cartId: string, payload: AddServiceEntryPayload) {
    try {
      ...
      let subtotal = 0;
      const components = serviceComponents.map((serviceComponent: any) => {
      ...
      cart.entries.push(entry as any);

      let cartSubtotal = 0;
      cart.entries.forEach((entry: any) => {
  ...
  async recalculateCart(cartId: string, session?: mongoose.ClientSession) {
    try {
      ...
      for (const entry of cart.entries as any[]) {
        if (entry.entryType === "SERVICE" && entry.serviceConfiguration) {
          ...
          for (const component of serviceConfig.components) {
            ...
            if (component.selectedItems?.length) {
              itemsTotal = component.selectedItems.reduce(
        ...
        if (entry.entryType === "PACKAGE" && entry.packageConfiguration) {
          ...
          for (const serviceConfig of packageConfig.services) {
            ...
            for (const component of serviceConfig.components) {
              ...
              const itemsTotal = component.selectedItems.reduce(
                (sum: number, item: any) => sum + (item.price || 0),
                0,
              );
  ...
  async updateComponent(
    ...
    try {
      ...
      const entry = (cart.entries as any[]).find(
      ...
      if (entry.entryType === "PACKAGE" && entry.packageConfiguration) {
        ...
        for (const service of allServices) {
          const foundComponent = service.components.find(
            (component: any) =>
              component.componentId.toString() === componentId,
          );
      ...
      const component = components.find(
        (component: any) => component.componentId.toString() === componentId,
      );
      if (!component) {
  ...
  async updateComponentItems(
    ...
    try {
      ...
      if (entry.entryType === "SERVICE" && entry.serviceConfiguration) {
        component = entry.serviceConfiguration.components.find(
          (component: any) => component.componentId.toString() === componentId,
        );
      }
      ...
      serviceComponent = await ServiceComponent.findById(
        component.serviceComponentId,
      );
      ...
      const allowedItemIds = new Set(
        serviceComponent.items.map((item: any) => item.itemId.toString()),
      );
  ...
  async addAddonComponent(
    ...
    try {
      ...
      if (!serviceConfig) {
        throw new Error(
          "Addon component currently supported only for SERVICE entries",
        );
      }

      const existingComponent = serviceConfig.components.find(
        (component: any) => component.componentId.toString() === componentId,
      );
  ...
  async removeAddonComponent(
    ...
    try {
      ...
      if (!serviceConfig) {
        throw new Error(
          "Addon component removal currently supported only for SERVICE entries",
        );
      }
      const componentIndex = serviceConfig.components.findIndex(
        (component: any) => component.componentId.toString() === componentId,
      );
  ...
  async addAddonService(
    ...
    try {
      ...
      ].find((service: any) => service.serviceId.toString() === serviceId);
      ...
      const tier = service.tiers.find(
        (tier: any) => tier.tierId.toString() === tierId,
      );
      ...
      const location = service.locations.find(
        (location: any) => location.locationId.toString() === locationId,
      );
      ...
      const components = await Promise.all(
        serviceComponents.map(async (serviceComponent: any) => {
          const pricing = await ServicePricing.findOne({
            serviceId,
  ...
  async removeAddonService(cartId: string, entryId: string, serviceId: string) {
    try {
      const cart = await Cart.findById(cartId);
      if (!cart) {
        throw new Error("Cart not found");
      ...
      if (entry.entryType !== "PACKAGE" || !entry.packageConfiguration) {
        throw new Error(
          "Addon service removal only supported for package entries",
        );
      }

      const packageConfig = entry.packageConfiguration;

      const addonServiceIndex = packageConfig.addonServices.findIndex(
  ...
  async validateCart(cartId: string, session?: ClientSession) {
    try {
      const cart = await Cart.findById(cartId).session(session || null);
  ...
  private async validateServiceConfiguration(
    ...
    const tierExists = service.tiers.some(
      (tier: any) =>
        tier.tierId.toString() === serviceConfig.tier.tierId.toString(),
    );
    ...
    const componentIds = serviceConfig.components.map(
      (component: any) => component.componentId,
    );
    ...
    const componentMap = new Map(
      components.map((component: any) => [component._id.toString(), component]),
    );
  ...
  async checkoutCart(cartId: string, userId: string) {
    ...
    try {
      ...
      const bookingEntries = cart.entries.map((entry: any) => ({
      ...
      const bookingPayload: Partial<IBooking> = {
        ...
        cartSnapshot: cart.toObject() as unknown as Record<string, unknown>,
  ...
  async updateCart(cartId: string, userId: string, payload: any) {
    try {
      const cart = await Cart.findOne({
        _id: cartId,
        userId,
  ...
  async getCartById(cartId: string, userId: string) {
    try {
      const cart = await Cart.findOne({
        _id: cartId,
        userId,
  ...
  async clearCartEntries(cartId: string, userId: string) {
    try {
      const cart = await Cart.findOne({
        _id: cartId,
        userId,
  ...
  async getEntryById(cartId: string, entryId: string, userId: string) {
    try {
      const cart = await Cart.findOne({
        _id: cartId,
        userId,
  ...
  private async buildServiceConfiguration(payload: {
    serviceId: string;
    tierId: string;
    locationId: string;
    subServiceId?: string;
    serviceRole?: "PRIMARY" | "INCLUDED" | "ADDON";
  }) {
    const {
      serviceId,
      tierId,
    ...
    const subtotal = components.reduce(
      (sum, component) => sum + component.pricing.total,
      0,
    );
    ...
    return {
      ...
      tier: {
        tierId,
        name:
          service.tiers.find((tier: any) => tier.tierId.toString() === tierId)
            ?.name || "",
      },
    ...
    try {
      ...
      for (const packageService of packageDoc.services) {
        const selectedService = payload.services.find(
          (service) =>
            service.serviceId === packageService.serviceId.toString(),
        );

        if (!selectedService) {
          throw new Error(
            `Missing configuration for service ${packageService.name}`,
          );
        }

        const tierId =
          selectedService.tierId || packageService.defaultTierId?.toString();

        if (!tierId) {
          throw new Error(
            `Tier is required for service ${packageService.name}`,
          );
        }
      ...
      const subtotal = services.reduce(
        (sum, service) => sum + service.pricing.grandTotal,
        0,
      );
  ...
  private resolveCartType(entries: any[]) {
  ...
  async removeEntry(cartId: string, entryId: string, userId: string) {
    try {
      const cart = await Cart.findOne({
        _id: cartId,
        userId,
      ...
      const entryIndex = (cart.entries as any[]).findIndex(
        (entry) => entry.entryId.toString() === entryId,
      );
      ...
      const removedEntry = (cart.entries as any[])[entryIndex];

      cart.entries.splice(entryIndex, 1);

      cart.cartType = this.resolveCartType(cart.entries) as any;
  ...
  async updateEntry(
    ...
    try {
      ...
      const locationId =
        payload.locationId || existingConfig.location.locationId.toString();
  ...
  async updateIncludedService(
    ...
    try {
      ...
      if (entry.entryType !== "PACKAGE" || !entry.packageConfiguration) {
        throw new Error(
          "Only package entries support included service updates",
        );
      }
      ...
      const locationId =
        payload.locationId || targetService.location.locationId.toString();
      ...
      const includedIndex = packageConfig.services.findIndex(
        (service: any) => service.serviceId.toString() === serviceId,
      );

      if (includedIndex !== -1) {
        packageConfig.services[includedIndex] = rebuiltConfiguration as any;
      }

      const addonIndex = packageConfig.addonServices.findIndex(
        (service: any) => service.serviceId.toString() === serviceId,
      );

      if (addonIndex !== -1) {
        packageConfig.addonServices[addonIndex] = rebuiltConfiguration as any;
      }
