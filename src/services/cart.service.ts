...
import { Cart, type IAddonService, type ICart, type ICartSubService, type ISelectedComponent, type ISelectedComponentItem, type ISelectedService } from "../models/cart.model.js";
...
import { CartPricingEngine, type CartTotals } from "./cart-pricing.engine.js";
...
class CartService {
  private static applyLineTax(target: { tax?: ILineTax }, tax: ILineTax | undefined): void {
    if (tax) { target.tax = tax; }
    else { delete target.tax; }
  }

  private static async getSubServiceSnapshotsByServiceIds(serviceIds: Types.ObjectId[]): Promise<Map<string, ICartSubService[]>> {
    ...
    const subServiceMap = new Map<string, ICartSubService[]>();
  ...
  private static clearLineDiscounts(cart: ICart): void {
    for (const component of cart.selectedComponents ?? []) { component.discountAmount = 0; }
    for (const component of cart.addonComponents ?? []) { component.discountAmount = 0; }
    for (const service of cart.selectedServices ?? []) { service.discountAmount = 0; }
    for (const service of cart.addonServices ?? []) { service.discountAmount = 0; }
  }

  private static allocateDiscountToCartLines(cart: ICart, totals: CartTotals, totalDiscount: number): void {
    this.clearLineDiscounts(cart);
    if (totalDiscount <= 0 || totals.subtotal <= 0) { return; }

    const lines: Array<{ amount: number; applyDiscount: (discount: number) => void; }> = [];
    ...
    lines.forEach((line, index) => {
      ...
      if (index === lines.length - 1) { lineDiscount = this.round(totalDiscount - allocatedDiscount); }
      else {
  ...
  static ensureCartEditable(cart: ICart) {
    if (!["ACTIVE", "SCHEDULED"].includes(cart.status)) { throw new Error(`Cart cannot be modified in ${cart.status} state`); }
  }
  ...
  static async createPackageCart(owner: CartOwner, payload: any) {
    ...
    if (!pkg || !pkg.isActive || !pkg.isComplete) { throw new Error("Package is not available"); }
    ...
    if (!location.isActive) { throw new Error("Location is inactive for this package"); }
    ...
    if (requiredServices.length === 0) { throw new Error("Package has no required services configured"); }
    ...
    for (const service of requiredServices) {
      const serviceId = service.serviceId.toString();
      if (!pricingMap.has(serviceId)) { throw new Error(`Pricing not found for required service: ${service.name}`); }
    }
  ...
  static async getCartById(owner: CartOwner, cartId: string) {
    ...
    // Package Cart
    if (cart.packageId) {
      const pkg = (await Package.findById(cart.packageId).populate("tierMappings").lean({ virtuals: true })) as any;
      if (!pkg) { throw new Error("Package not found"); }

      // Get services for this cart's tier
      const packageTierMap = pkg.tierMappings?.find((m: any) => m.tierId.toString() === cart.tierId.toString());

      const serviceIds = packageTierMap?.services?.map((s: { serviceId: mongoose.Types.ObjectId; name: string; isRequired: boolean; }) => s.serviceId) || [];
      ...
      for (const sc of serviceComponents) {
        ...
        if (!components) { components = []; serviceComponentMap.set(key, components); }
        components.push(sc);
      }
      ...
      const addonServiceIds = cart.addonServices?.map((s: any) => s.serviceId) || [];
      ...
      const hydratedAddonServices = (cart.addonServices || []).map((addon: any) => ({ ...addon, service: addonServicesMap.get(addon.serviceId.toString()) }));
      const selectedServiceIds = cart.selectedServices?.map((s: any) => s.serviceId) || [];
      ...
      const hydratedSelectedServices = (cart.selectedServices || []).map((s: any) => ({ ...s, service: selectedServicesMap.get(String(s.serviceId)) }));
  ...
  static async updateSelectedComponents(owner: CartOwner, cartId: string, payload: any) {
    const { selectedComponents } = payload;

    if (!Array.isArray(selectedComponents)) { throw new Error("selectedComponents must be an array"); }
    this.ensureUniqueIds(selectedComponents.map((item: any) => item?.componentId), "componentIds");
    if (!mongoose.Types.ObjectId.isValid(cartId)) { throw new Error("Invalid cartId"); }
    ...
    if (!cart.serviceId) { throw new Error("This operation is only allowed for service carts"); }
    ...
    const missingRequired = serviceComponents.filter((c) => c.isRequired).filter((c) => { return !selectedComponents?.some((sc: any) => sc.componentId.toString() === c.componentId.toString()); });

    if (missingRequired.length > 0) { throw new Error(`Missing required components: ${missingRequired.map((c) => c.name).join(", ")}`); }
    ...
    for (const sc of selectedComponents || []) {
      ...
      if (!componentConfig) { throw new Error(`Invalid component`); }
      ...
      if (!pricing) { throw new Error(`Pricing not found for ${componentConfig.name}`); }
      ...
      for (const selectedItem of sc.items || []) {
        ...
        if (!selectedItemId) { throw new Error(`Invalid item format in ${componentConfig.name}`); }

        const matchedItem = allowedItems.find((item) => item.itemId.toString() === selectedItemId.toString());
        if (!matchedItem) { throw new Error(`Invalid item in ${componentConfig.name}`); }
  ...
  static async updateAddonComponents(owner: CartOwner, cartId: string, payload: any) {
    const { addonComponents } = payload;

    if (!Array.isArray(addonComponents)) { throw new Error("addonComponents must be an array"); }
    this.ensureUniqueIds(addonComponents.map((item: any) => item?.componentId), "componentIds");
    ...
    for (const ac of addonComponents || []) {
      ...
      if (!component) { throw new Error("Invalid addon component for this service"); }
      ...
      if (!pricing) { throw new Error(`Pricing not found for ${component.name}`); }
      ...
      for (const selectedItem of requestedItems) {
        const selectedItemId = typeof selectedItem === "string" ? selectedItem : selectedItem?.itemId;
        if (!selectedItemId) { throw new Error(`Invalid item format in ${component.name}`); }

        const matchedItem = allowedItems.find((item) => item.itemId.toString() === selectedItemId.toString());
        if (!matchedItem) { throw new Error(`Invalid item selected for ${component.name}`); }
  ...
  static async updateSelectedServices(owner: CartOwner, cartId: string, payload: any) {
    ...
    this.ensureCartEditable(cart);
    if (!cart.packageId) { throw new Error("This operation is only allowed for package carts"); }
    ...
    const selectedServiceIdSet = new Set(serviceIds.map((serviceId: any) => serviceId.toString()));
    const missingRequiredServices = requiredServiceIds.filter((serviceId) => !selectedServiceIdSet.has(serviceId));

    if (missingRequiredServices.length > 0) { throw new Error("All required package services must be selected"); }
    ...
    const requestedServiceObjectIds = serviceIds.map((serviceId: any) => new Types.ObjectId(serviceId.toString()));
    ...
    for (const serviceId of serviceIds) {
      ...
      if (!matchedService) { throw new Error("Invalid service selected"); }
      if (matchedService.isRelated) { throw new Error(`${matchedService.name} is an addon service. Use updateAddonServices instead.`); }

      const price = pricingMap.get(serviceIdString);
      if (price === undefined) { throw new Error(`Pricing not found for service ${matchedService.name}`); }
      ...
      selectedServices.push({
        ...
        components: (serviceComponentMap.get(serviceIdString) ?? []).map((component) => {
          if (!component.componentId) { throw new Error(`Invalid ServiceComponent for service "${matchedService.name}": componentId is missing`); }
          if (!Types.ObjectId.isValid(component.componentId.toString())) { throw new Error(`Invalid ServiceComponent for service "${matchedService.name}": componentId is invalid`); }

          const items = (component.items ?? []).map((item) => {
            if (!item.itemId) {
              throw new Error(`Invalid item in component "${component.name}" for service "${matchedService.name}": itemId is missing`);
            }

            if (!Types.ObjectId.isValid(item.itemId.toString())) {
              throw new Error(`Invalid item in component "${component.name}" for service "${matchedService.name}": itemId is invalid`);
            }
  ...
  static async updateAddonServices(owner: CartOwner, cartId: string, payload: any) {
    const { serviceIds } = payload;
    if (!mongoose.Types.ObjectId.isValid(cartId)) { throw new Error("Invalid cartId"); }
    ...
    for (const serviceId of serviceIds) {
      if (!Types.ObjectId.isValid(serviceId)) { throw new Error(`Invalid serviceId: ${serviceId}`); }
    }
    ...
    for (const serviceId of serviceIds) {
      const serviceIdString = serviceId.toString();
      const matchedService = allowedServices.find((service) => service.serviceId.toString() === serviceIdString);
      if (!matchedService) { throw new Error("Invalid addon service selected"); }

      // Only related non-required services can be addons.
      if (matchedService.isRequired || !matchedService.isRelated) { throw new Error(`${matchedService.name} is not an addon service.`); }
      ...
      addonServices.push({
        ...
        components: (serviceComponentMap.get(serviceIdString) ?? []).map((component) => {
          ...
          const items = (component.items ?? []).map((item) => {
            if (!item.itemId) { throw new Error(`Invalid item in component "${component.name}" for service "${matchedService.name}": itemId is missing`); }
            if (!Types.ObjectId.isValid(item.itemId.toString())) { throw new Error(`Invalid item in component "${component.name}" for service "${matchedService.name}": itemId is invalid`); }
  ...
  static async updateSchedule(owner: CartOwner, cartId: string, payload: any) {
    ...
    if (!mongoose.Types.ObjectId.isValid(cartId)) { throw new Error("Invalid cartId"); }
    if (typeof scheduledDate !== "string" || !scheduledDate.trim()) { throw new Error("scheduledDate is required"); }
    if (typeof scheduledTime !== "string" || !/^([01]\d|2[0-3]):[0-5]\d$/.test(scheduledTime)) { throw new Error("scheduledTime is required in HH:mm format"); }

    // Supports: 2026-08-03 2026-08-03T00:00:00.000Z
    const datePart = scheduledDate.split("T")[0];
    if (!datePart || !/^\d{4}-\d{2}-\d{2}$/.test(datePart)) { throw new Error("scheduledDate must be in YYYY-MM-DD format"); }
    ...
    if (Number.isNaN(scheduledAt.getTime())) { throw new Error("Invalid scheduled date or time"); }
    if (scheduledAt <= new Date()) { throw new Error("Scheduled date and time cannot be in the past"); }
  ...
  static async updateCustomerDetails(owner: CartOwner, cartId: string, payload: any) {
    ...
    if (bookingFor && !["MYSELF", "OTHER"].includes(bookingFor)) { throw new Error("Invalid bookingFor value"); }
    ...
    if (bookingFor) { cart.bookingFor = bookingFor; }
  ...
  static async updateCartNotes(owner: CartOwner, cartId: string, payload: any) {
    ...
    if (notes !== undefined && typeof notes !== "string") { throw new Error("notes must be a string"); }
    if (!mongoose.Types.ObjectId.isValid(cartId)) { throw new Error("Invalid cartId"); }
  ...
  static async recalculateCart(owner: CartOwner, cartId: string, options?: { session?: mongoose.ClientSession; persist?: boolean; }) {
    if (!mongoose.Types.ObjectId.isValid(cartId)) { throw new Error("Invalid cartId"); }
    ...
    if (cart.status === "CHECKOUT_PENDING" && options?.persist) { throw new Error("Cannot recalculate a cart during checkout"); }
  ...
  private static async rebuildPackageServiceComponents(cart: ICart, session?: mongoose.ClientSession): Promise<boolean> {
    if (!cart.packageId) { return false; }
    ...
    for (const service of allServices) {
      if (!service?.serviceId) {
        throw new Error(`Invalid package service "${service?.name ?? "Unknown"}": serviceId is missing`);
      }

      if (!Types.ObjectId.isValid(service.serviceId.toString())) {
        throw new Error(`Invalid package service "${service.name}": serviceId is invalid`);
      }

      serviceIds.push(new Types.ObjectId(service.serviceId.toString()));
    }

    // Remove duplicates
    const uniqueServiceIds = [...new Map(serviceIds.map((id) => [id.toString(), id,])).values(),];

    // Load current ServiceComponent configuration
    const query = ServiceComponent.find({ serviceId: { $in: uniqueServiceIds, }, tierId: cart.tierId, });

    if (session) { query.session(session); }
    const serviceComponents = await query.lean();

    // Group ServiceComponents by serviceId
    const serviceComponentMap = new Map<string, typeof serviceComponents>();

    for (const component of serviceComponents) {
      if (!component.serviceId) { continue; }
      const key = component.serviceId.toString();
      const list = serviceComponentMap.get(key) ?? [];
      list.push(component);
      serviceComponentMap.set(key, list);
    ...
    // Build canonical component snapshots
    const rebuildServices = (
      services: | ISelectedService[] | IAddonService[]) => {
      for (const service of services) {
        const serviceId = service.serviceId.toString();
        const currentComponents = serviceComponentMap.get(serviceId) ?? [];
        const rebuiltComponents: ISelectedComponent[] = currentComponents.map((component) => {
          if (!component.componentId) {
            throw new Error(`Invalid ServiceComponent "${component.name}" for service "${service.name}": componentId is missing`);
          }

          if (!Types.ObjectId.isValid(component.componentId.toString())) {
            throw new Error(`Invalid ServiceComponent "${component.name}" for service "${service.name}": componentId is invalid`);
          }

          const items: ISelectedComponentItem[] = (component.items ?? []).map((item) => {
            if (!item.itemId) {
              throw new Error(`Invalid item in component "${component.name}" for service "${service.name}": itemId is missing`);
            }

            if (!Types.ObjectId.isValid(item.itemId.toString())) {
              throw new Error(`Invalid item in component "${component.name}" for service "${service.name}": itemId is invalid`);
            }
          ...
          // Preserve existing pricing/tax data
          // where possible.
          const existing = (service.components ?? []).find((existingComponent) => existingComponent.componentId && existingComponent.componentId.toString() === component.componentId.toString());

          return {
            componentId: component.componentId,
            name: component.name,
            items,
            priceBeforeDiscount: existing?.priceBeforeDiscount ?? 0,
        ...
        // Detect whether repair/update is needed
        const existingComponents = service.components ?? [];
        const existingIds = existingComponents.map((component) => component.componentId?.toString() ?? "");
        const rebuiltIds = rebuiltComponents.map((component) => component.componentId.toString());
        const needsUpdate = existingIds.length !== rebuiltIds.length || existingIds.some((id, index) => !id || id !== rebuiltIds[index]);

        if (needsUpdate) { changed = true; }
        service.components = rebuiltComponents;
      }
    };

    rebuildServices(selectedServices);
    rebuildServices(addonServices);
  ...
  static async validateCart(owner: CartOwner, cartId: string, persist: boolean, session?: mongoose.ClientSession): Promise<CartValidationResult> {
    ...
    if (cart.serviceId) {
      ...
      for (const [index, component,] of (cart.selectedComponents ?? []).entries()) {
        ...
        if (
          ...
          errors.push(
            `Invalid selected componentId at index ${index}`
          );
        ...
        for (
          ...
          if (!item?.itemId) {
            errors.push(
              `Invalid item at index ${itemIndex} in component "${component.name}": itemId is missing`
            );
          ...
          if (
            ...
            errors.push(
              `Invalid itemId at index ${itemIndex} in component "${component.name}"`
            );
          }
        }
      }
      ...
      for (
        ...
        if (!component.componentId) {
          errors.push(
            `Invalid ServiceComponent "${component.name}": componentId is missing`
          );
        ...
        if (
          ...
          errors.push(
            `Missing required component: ${component.name}`
          );
        }
      }
      ...
      if (
        ...
        errors.push(
          "No components selected for service"
        );
      }
      ...
      for (
        ...
        if (
          ...
          errors.push(
            `Invalid addon component at index ${index}: componentId is missing`
          );
        ...
        if (
          ...
          errors.push(
            `Invalid addon componentId at index ${index}`
          );
        ...
        for (
          ...
          if (!item?.itemId) {
            errors.push(
              `Invalid item at index ${itemIndex} in addon component "${component.name}": itemId is missing`
            );
          ...
          if (
            ...
            errors.push(
              `Invalid itemId at index ${itemIndex} in addon component "${component.name}"`
            );
          }
        }
      }
    ...
    if (cart.packageId) {
      ...
      try {
        const repaired =
          await this.rebuildPackageServiceComponents(
            cart,
            session
          );
        ...
        if (repaired) {
          changes.push(
            "Package service component snapshots were refreshed"
          );
        }
        ...
        errors.push(
          error.message ||
          "Failed to refresh package components"
        );
      }
      ...
      if (session) {
        packageQuery.session(
          session
        );
      }

      const pkg =
        await packageQuery.lean();
      ...
      if (!pkg) {
        ...
        if (!tierExists) {
          errors.push(
            "Selected package tier is no longer available"
          );
        }
        ...
        if (
          ...
          errors.push(
            "Selected package location is no longer available"
          );
        }
        ...
        if (session) {
          mappingQuery.session(
            session
          );
        }

        const packageTierMap =
          await mappingQuery.lean();

        if (!packageTierMap) {
          errors.push(
            "Package tier mapping no longer exists"
          );
        } else {
          ...
          for (
            ...
            if (
              ...
              errors.push(
                `Invalid package service at index ${serviceIndex}: serviceId is missing`
              );
            ...
            if (
              ...
              errors.push(
                `Invalid package serviceId at index ${serviceIndex}`
              );
            ...
            if (!mappedService) {
              errors.push(
                `Service "${service.name}" is no longer available in this package`
              );
            } else if (
              mappedService.isRelated
            ) {
              errors.push(
                `Service "${service.name}" is an addon service and cannot be selected as an included service`
              );
            }
            ...
            for (
              ...
              if (
                ...
                errors.push(
                  `Invalid component at index ${componentIndex} for package service "${service.name}": componentId is missing`
                );
              ...
              if (
                ...
                errors.push(
                  `Invalid componentId at index ${componentIndex} for package service "${service.name}"`
                );
              ...
              for (
                ...
                if (!item?.itemId) {
                  errors.push(
                    `Invalid item at index ${itemIndex} in component "${component.name}" for package service "${service.name}": itemId is missing`
                  );
                ...
                if (
                  ...
                  errors.push(
                    `Invalid itemId at index ${itemIndex} in component "${component.name}" for package service "${service.name}"`
                  );
                }
              }
            }
            ...
            for (
              ...
              if (
                ...
                errors.push(
                  `Invalid sub-service at index ${subServiceIndex} for package service "${service.name}": subServiceId is missing`
                );
              ...
              if (
                ...
                errors.push(
                  `Invalid subServiceId at index ${subServiceIndex} for package service "${service.name}"`
                );
              }
            }
          }
          ...
          for (
            ...
            if (
              ...
              errors.push(
                `Missing required package service: ${mappedService.name}`
              );
            }
          }
          ...
          for (
            ...
            if (
              ...
              errors.push(
                `Invalid addon service at index ${serviceIndex}: serviceId is missing`
              );
            ...
            if (
              ...
              errors.push(
                `Invalid addon serviceId at index ${serviceIndex}`
              );
            ...
            if (!mappedService) {
              errors.push(
                `Addon service "${service.name}" is no longer available in this package`
              );
            } else if (
              mappedService.isRequired ||
              !mappedService.isRelated
              ...
              errors.push(
                `Service "${service.name}" is not a valid addon service`
              );
            }
            ...
            for (
              ...
              if (
                ...
                errors.push(
                  `Invalid component at index ${componentIndex} for addon service "${service.name}": componentId is missing`
                );
              ...
              if (
                ...
                errors.push(
                  `Invalid componentId at index ${componentIndex} for addon service "${service.name}"`
                );
              ...
              for (
                ...
                if (!item?.itemId) {
                  errors.push(
                    `Invalid item at index ${itemIndex} in component "${component.name}" for addon service "${service.name}": itemId is missing`
                  );
                ...
                if (
                  ...
                  errors.push(
                    `Invalid itemId at index ${itemIndex} in component "${component.name}" for addon service "${service.name}"`
                  );
                }
              }
            }
            ...
            for (
              ...
              if (
                ...
                errors.push(
                  `Invalid sub-service at index ${subServiceIndex} for addon service "${service.name}": subServiceId is missing`
                );
              ...
              if (
                ...
                errors.push(
                  `Invalid subServiceId at index ${subServiceIndex} for addon service "${service.name}"`
                );
              }
            }
          }
      ...
      if (
        ...
        errors.push(
          "No services selected for package"
        );
      }
    }
  ...
  static async checkoutCart(userId: string, cartId: string) {
    ...
    try {
      const result = await session.withTransaction(async (): Promise<HydratedDocument<IBooking>> => {
        if (!userId) { throw new Error("Token missing"); }
        if (!mongoose.Types.ObjectId.isValid(cartId)) { throw new Error("Invalid cartId"); }
        ...
        if (cart.status === "CHECKOUT_PENDING" && cart.activeBookingId && cart.checkoutExpiresAt && cart.checkoutExpiresAt > new Date()) {
          ...
          if (existingBooking && existingBooking.payment.status !== "PAID") { return existingBooking; }
        }
        ...
        if (!cart.scheduledAt) { throw new Error("Scheduled date not set"); }
        ...
        const lockedCart = await Cart.findOneAndUpdate(
          { _id: cartId, userId, status: { $in: ["ACTIVE", "SCHEDULED"] } },
          { $set: { status: "CHECKOUT_PENDING", checkoutExpiresAt: expiry } },
          { new: true, session },
        );

        if (!lockedCart) { throw new Error("Checkout already initiated"); }
      ...
      await Booking.updateOne(
        { _id: finalBooking._id, "payment.status": { $ne: "PAID" } },
        {
          $set: {
  ...
  static async expireCheckoutPendingCarts() {
    ...
    await Cart.updateMany(
      { status: "CHECKOUT_PENDING", checkoutExpiresAt: { $lte: now } },
      {
        $set: { status: "ACTIVE" },
    ...
    try {
      await session.withTransaction(async () => {
        ...
        for (const cart of guestCarts) {
          ...
          if (cart.serviceId) { duplicateQuery.serviceId = cart.serviceId; }
          else if (cart.packageId) { duplicateQuery.packageId = cart.packageId; }

          const duplicateExists = await Cart.exists(duplicateQuery).session(session);
          if (duplicateExists) { duplicateGuestCartIds.push(cart._id); }
        }
        ...
        await Cart.updateMany(
          { guestId, status: { $in: ["ACTIVE", "SCHEDULED"] }, _id: { $nin: duplicateGuestCartIds } },
          {
            $set: { userId },
            $unset: { guestId: 1 },
          },
  ...
  static async applyCoupon(owner: CartOwner, cartId: string, couponCode: string) {
    if (!mongoose.Types.ObjectId.isValid(cartId)) { throw new Error("Invalid cartId"); }
    ...
    this.ensureCartEditable(cart);
    if (cart.couponId) { throw new Error("A coupon is already applied. Remove it first."); }

    if (typeof cart.subtotal !== "number" || !Number.isFinite(cart.subtotal)) { throw new Error("Cart subtotal is not calculated"); }
  ...
  static async reopenCart(owner: CartOwner, cartId: string) {
    ...
    const cart = await Cart.findOne({ _id: cartId, ...buildCartOwnerQuery(owner) });
    if (!cart) { throw new Error("Cart not found"); }
    if (cart.status !== "CHECKOUT_PENDING") { throw new Error("Only checkout pending carts can be reopened."); }
    if (cart.checkoutExpiresAt && cart.checkoutExpiresAt > new Date()) { throw new Error("Checkout is still active and cannot be reopened yet"); }
  ...
  static async exportCartsToCsv(cartIds: string[]) {
    ...
    if (invalidId) { throw new Error("One or more cart IDs are invalid"); }

    const carts = await Cart.find({ _id: { $in: uniqueCartIds } }).sort({ createdAt: -1 }).lean();
    if (carts.length === 0) { throw new Error("No carts found for export"); }
    ...
    const formatComponents = (components: Array<{ componentId?: unknown; name?: string; items?: Array<{ itemId?: unknown; name?: string; }>; priceBeforeDiscount?: number; discountAmount?: number; totalPrice?: number; }> | undefined): string => {
