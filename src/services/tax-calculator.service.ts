...
export class TaxCalculatorService {
  ...
  private static resolveJurisdiction(
    ...
    if (!/^\d{2}$/.test(supplierCode)) {
      throw new Error(
        "Supplier state code must contain exactly two digits",
      );
    }

    if (!/^\d{2}$/.test(placeOfSupplyCode)) {
      throw new Error(
        "Place of supply state code must contain exactly two digits",
      );
    }
  ...
  static calculateLineTax(
    ...
    if (jurisdiction === "INTRA_STATE") {
      ...
      const roundedGstAmount =
        this.round(rawGstAmount);

      cgstAmount = this.round(
        roundedGstAmount / 2,
      );
       ...
       */
      sgstAmount = this.round(
        roundedGstAmount - cgstAmount,
      );
    } else {
      igstRate = profile.totalRate;
      igstAmount = this.round(
        rawGstAmount,
      );
    }
