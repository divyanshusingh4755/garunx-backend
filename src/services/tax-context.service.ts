...
export class TaxContextService {
  private static normalizeStateCode(fieldName: string, value: unknown): string {
  ...
  static async resolveByLocationId(locationId: string | Types.ObjectId): Promise<ITaxContext> {
    const id = locationId.toString();
    if (!Types.ObjectId.isValid(id)) { throw new Error("Invalid locationId"); }
    if (!taxConfig.enabled) { throw new Error("GST is disabled"); }
    ...
    if (!location.stateId) { throw new Error("State is not configured for this location"); }

    const state = await State.findById(location.stateId).select("gstCode isActive name").lean();
    if (!state) { throw new Error("State not found for this location"); }
    if (!state.isActive) { throw new Error("State configured for this location is inactive"); }
