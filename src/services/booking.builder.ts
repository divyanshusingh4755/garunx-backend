...
type CartWithOptionalToObject = ICart & { toObject?: () => ICart; };

export class BookingBuilder {
  private static toPlainCart(cart: ICart): ICart {
    const possibleDocument = cart as CartWithOptionalToObject;

    if (typeof possibleDocument.toObject === "function") { return possibleDocument.toObject(); }
    return cart;
  }
  ...
  private static buildTaxSummary(taxSummary: ICart["taxSummary"]): IBookingTaxSummary {
    return {
  ...
  static async buildServiceBooking(cart: ICart): Promise<BookingBuildResult> {
    if (!cart.serviceId) { throw new Error("Service ID is required for service booking"); }
    if (cart.packageId) { throw new Error("Service booking cart cannot contain packageId"); }
    ...
    const selectedComponents = [
      ...(plainCart.selectedComponents ?? []).map((component) => ({ ...component, componentType: "DEFAULT" as const })),
  ...
  private static buildTaxSummaryFromLine(tax: ISelectedComponent["tax"] | undefined): IBookingTaxSummary {
    if (!tax) { return { taxableAmount: 0, cgstAmount: 0, sgstAmount: 0, igstAmount: 0, totalTax: 0 }; }
