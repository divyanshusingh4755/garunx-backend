...
export class BannerService {
  ...
  static async updateBanner(id: string, updateData: Partial<IBanner>) {
    ...
    if (updateData.redirect !== undefined) {
      const incomingRedirect = updateData.redirect as
        | Partial<IBanner["redirect"]>
        | undefined;

      const redirectType =
        incomingRedirect?.type ?? banner.redirect?.type ?? "NONE";

      banner.redirect = {
        type: redirectType,

        ...(["SERVICE", "PACKAGE", "CATEGORY", "PRODUCT"].includes(
          redirectType,
  ...
  static async findBanners(
    ...
    /*
     * "relevance" only makes sense
     * when using Mongo text search.
     */
    ...
    /*
     * Generate Redis key using normalized
     * values that MongoDB will actually use.
     */
    const cacheKey =
      CacheKeys.bannerList({
        searchTerm:
          normalizedSearch,
      ...
      loader:
        async () => {
          ...
          if (
            ...
            /*
             * Your original code created
             * projection but did not include
             * the score field.
             *
             * Add it so text relevance
  ...
  static async exportBannersToCsv(
    bannerIds: string[],
  ) {
    if (
      !Array.isArray(
      ...
      throw new Error(
        "At least one banner ID is required",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "A maximum of 1000 banners can be exported at once",
      );
    }

    const uniqueBannerIds = [
      ...new Set(
        bannerIds,
      ),
    ];
    ...
    const escapeCsv = (
      ...
      /*
       * Avoid spreadsheet formula execution
