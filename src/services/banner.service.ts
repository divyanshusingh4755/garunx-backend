...
export class BannerService {
  private static async invalidateBannerCache(bannerId?: string): Promise<void> {
    const operations: Promise<unknown>[] = [RedisCacheService.deleteByPattern(CacheKeys.bannerListPattern())];

    if (bannerId) { operations.push(RedisCacheService.delete(CacheKeys.bannerDetail(bannerId))); }
    await Promise.all(operations);
  }

  private static ensureValidId(id: string): void { if (!Types.ObjectId.isValid(id)) { throw new Error("Invalid banner ID"); } }

  static async createBanner(bannerData: Partial<IBanner>) {
    if (!bannerData.name?.trim()) { throw new Error("Banner name is required"); }
    if (!bannerData.description?.trim()) { throw new Error("Description is required"); }
    if (!bannerData.image?.trim()) { throw new Error("Image is required"); }
    if (!bannerData.placement) { throw new Error("Placement is required"); }
    if (!bannerData.format) { throw new Error("Format is required"); }
  ...
  static async updateBanner(id: string, updateData: Partial<IBanner>) {
    ...
    for (const [field, value] of Object.entries(updateData as Record<string, unknown>)) {
      if (protectedFields.has(field) || field === "redirect") { continue; }
      banner.set(field, value);
    }

    if (updateData.redirect !== undefined) {
      const incomingRedirect = updateData.redirect as Partial<IBanner["redirect"]> | undefined;
  ...
  static async findBanners(searchTerm?: string, placement?: string, format?: string, redirectType?: RedirectType, limit = 20, page = 1, isActive?: boolean, sortBy = "displayOrder", sortOrder: "asc" | "desc" = "asc") {
    ...
    return RedisCacheService.getOrSet({
      ...
      loader: async () => {
        ...
        if (redirectType) { query["redirect.type"] = redirectType; }
  ...
  static async exportBannersToCsv(bannerIds: string[]) {
    if (!Array.isArray(bannerIds) || bannerIds.length === 0) { throw new Error("At least one banner ID is required"); }
    if (bannerIds.length > 1000) { throw new Error("A maximum of 1000 banners can be exported at once"); }

    const uniqueBannerIds = [...new Set(bannerIds)];

    for (const bannerId of uniqueBannerIds) {
      if (!Types.ObjectId.isValid(bannerId)) { throw new Error("Invalid banner ID"); }
    }
    ...
    if (banners.length === 0) { throw new Error("No banners found for export"); }
    ...
    const orderedBanners = uniqueBannerIds.map((bannerId) => bannerMap.get(bannerId)).filter((banner): banner is NonNullable<typeof banner> => Boolean(banner));
