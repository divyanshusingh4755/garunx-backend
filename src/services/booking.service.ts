...
export class BookingService {
  private static getReassignmentManualRequestLimit(requestedByRole: ReassignmentRequestedByRole): number {
    return requestedByRole === "COORDINATOR" ? MAX_REASSIGNMENT_COORDINATOR_REQUESTS : MAX_REASSIGNMENT_USER_REQUESTS;
  }

  private static async invalidateBookingCache(bookingId?: string): Promise<void> {
    ...
    if (bookingId) {
      operations.push(
        RedisCacheService.delete(CacheKeys.bookingDetail(bookingId)),
        RedisCacheService.delete(CacheKeys.bookingInvoice(bookingId)),
      );
    }
  ...
  private static async assignReplacementCoordinatorRequest(
    params: { booking: any; coordinatorId: string | Types.ObjectId; requestedBy: string | Types.ObjectId; assignmentType: "MANUAL" | "AUTO"; session?: ClientSession; },
  ) {
    const { booking, coordinatorId, requestedBy, assignmentType, session: externalSession } = params;

    if (!booking.assignment || !booking.assignment.reassignment) { throw new Error("Active reassignment not found"); }
    ...
    if (!currentCoordinatorId) { throw new Error("Current assigned coordinator not found"); }
    ...
    if (!booking.scheduledAt) { throw new Error("Booking schedule is required"); }
    ...
    if (reassignment.mode === "AUTO" && assignmentType === "MANUAL") {
      const manualRequestCount = currentRoundRequests.filter((request: any) => request.assignmentType === "MANUAL").length;
      ...
      const automaticFallbackStarted = currentRoundRequests.some((request: any) => request.assignmentType === "AUTO");
    ...
    if (reassignment.mode === "AUTO" && assignmentType === "AUTO") {
      const autoRequestCount = currentRoundRequests.filter((request: any) => request.assignmentType === "AUTO").length;
    ...
    const alreadyRequested = currentRoundRequests.some((request: any) => request.coordinatorId?.toString() === coordinatorObjectId.toString());
    if (alreadyRequested) {
    ...
    if (externalSession) { coordinatorQuery.session(externalSession); }

    const coordinator = await coordinatorQuery;
    if (!coordinator) { throw new Error("Replacement coordinator is not available for this booking"); }
    ...
    if (externalSession) { bookedCountQuery.session(externalSession); }
    ...
    if (reassignment.mode === "NOMINATED") { reassignment.replacementCoordinatorId = coordinatorObjectId; }
  ...
  private static handleFailedReassignmentAttempt(booking: any, failureReason: string): void {
    const reassignment = booking.assignment?.reassignment;
    ...
    const currentRoundRequests = (booking.assignment.requests ?? []).filter((request: any) => (request.assignmentRound ?? 1) === currentRound);
    const hasOtherPending = currentRoundRequests.some((request: any) => request.status === "PENDING");
    ...
    if (reassignment.mode === "NOMINATED") {
      if (hasOtherPending) { reassignment.status = "REPLACEMENT_REQUESTED"; return; }
  ...
  private static clearAcceptedCoordinator(booking: any): void {
  ...
  private static async confirmSuccessfulPayment(params: { bookingId: string; orderId: string; providerPaymentId: string; amountPaid: number; paymentMethod?: string; paidAt?: Date; }): Promise<boolean> {
    ...
    try {
      await session.withTransaction(async () => {
        ...
        const paidUpdate = await Booking.updateOne(
          { _id: booking._id, "payment.providerOrderId": orderId, "payment.status": { $ne: "PAID" } },
          {
            $set: {
              "payment.status": "PAID",
        ...
        if (paidUpdate.modifiedCount === 0) { return; }
    ...
    if (paymentConfirmed) {
      ...
      if (confirmedUserId) { await ReferralRewardService.processReferralReward(confirmedUserId, bookingId); }
    }

    return paymentConfirmed;
  }

  private static generateOtp(): string { return crypto.randomInt(100000, 1000000).toString(); }

  private static generateBeneficiaryAccessToken(): { token: string; tokenHash: string; } {
    const token = crypto.randomBytes(32).toString("hex");
    const tokenHash = crypto.createHash("sha256").update(token).digest("hex");
    return { token, tokenHash };
  }

  private static hashBeneficiaryAccessToken(token: string): string { return crypto.createHash("sha256").update(token).digest("hex"); }

  private static buildServiceExecutions(booking: any) {
    const serviceExecutions: { executionId: string; serviceId: Types.ObjectId; status: "PENDING"; }[] = [];
  ...
  private static async validateBookingOtp(booking: any, otp: string): Promise<boolean> {
    const otpVerification = booking.execution?.otpVerification;

    if (!otpVerification?.otpHash) { throw new Error("Booking OTP has not been generated"); }
    if (!otpVerification.expiresAt) { throw new Error("Booking OTP expiry is missing"); }
    if (otpVerification.status === "VERIFIED") { throw new Error("Booking OTP has already been verified"); }
    if (otpVerification.attempts >= MAX_OTP_VERIFICATION_ATTEMPTS) { throw new Error("Maximum OTP verification attempts exceeded"); }
  ...
  private static getRequestedCoordinatorIds(booking: any, scheduledAt?: Date, assignmentRound?: number): Types.ObjectId[] {
    ...
    return requests.filter((request: any) => (request.assignmentRound ?? 1) === currentRound && request.scheduledAt && request.scheduledAt >= startOfDay && request.scheduledAt <= endOfDay).map((request: any) => request.coordinatorId).filter(Boolean).map((coordinatorId: Types.ObjectId | string) => new Types.ObjectId(coordinatorId.toString()));
  }

  private static async findNextAvailableCoordinator(booking: any, excludedCoordinatorIds: Types.ObjectId[] = [], scheduledAt?: Date) {
    const locationIds = this.getBookingLocationIds(booking);
    ...
    if (!targetScheduledAt) { return candidates[0]; }
    ...
    for (const coordinator of candidates) {
      ...
      if (assignedBookings < maximumBookings) { return coordinator; }
    }
  ...
  private static async assignCoordinatorRequest(params: {
    ...
    if (!targetScheduledAt) { throw new Error("Booking schedule is required"); }
    const coordinatorObjectId = new Types.ObjectId(coordinatorId.toString());
    ...
    if (isCurrentRescheduleRound) {
      const manualRescheduleRequests = booking.assignment.requests.filter((request: any) => (request.assignmentRound ?? 1) === booking.assignment.currentRound && request.assignmentType === "MANUAL");
      ...
      const automaticFallbackStarted = booking.assignment.requests.some((request: any) => (request.assignmentRound ?? 1) === booking.assignment.currentRound && request.assignmentType === "AUTO");
    ...
    const duplicateRequest = booking.assignment.requests.find((request: any) => request.coordinatorId?.toString() === coordinatorObjectId.toString() && (request.assignmentRound ?? 1) === booking.assignment.currentRound);
  ...
  private static calculateExecutionProgress(serviceExecutions: any[]): number {
    if (!serviceExecutions.length) { return 0; }

    const resolvedServices = serviceExecutions.filter(
      (service) => service.status === "COMPLETED" || service.status === "SKIPPED" || service.status === "CANCELLED",
    ).length;

    return Math.round((resolvedServices / serviceExecutions.length) * 100);
  }

  private static addMilestoneIfMissing(booking: any, code: BookingMilestone, completedBy?: string | Types.ObjectId, notes?: string) {
    booking.execution ??= { stage: "NOT_STARTED", serviceExecutions: [], milestones: [], progressPercentage: 0 };
    booking.execution.milestones ??= [];

    const alreadyExists = booking.execution.milestones.some((milestone: any) => milestone.code === code);
  ...
  static async process(req: Request) {
    ...
    if (!valid) {
      const error = new Error("Invalid webhook signature") as Error & { statusCode?: number; };
    ...
    if (!booking) { throw new Error(`Booking not found for ${orderId}`); }
    ...
    try {
      await session.withTransaction(async () => {
        if (paymentStatus === "SUCCESS") {
          ...
          if (!paymentId) { throw new Error("Missing Cashfree payment ID"); }
        ...
        if (paymentStatus === "FAILED") {
          const failedUpdate = await Booking.updateOne(
            { _id: booking._id, "payment.status": { $ne: "FAILED" } },
            { $set: { "payment.status": "FAILED", "payment.failureReason": "Payment failed" } },
            { session },
          );
  ...
  static async retryPayment(bookingId: string, userId: string) {
    ...
    if (booking.payment.status === "PAID") { throw new Error("Booking already paid"); }
    ...
    if (booking.payment.providerOrderId) {
      ...
      if (order.order_status === "PAID") { throw new Error("Payment Already done"); }
    }
  ...
  static async getPaymentStatus(cartId: string, userId: string) {
    if (!Types.ObjectId.isValid(cartId) || !Types.ObjectId.isValid(userId)) { throw new Error("Invalid cart or user ID"); }
    ...
    let syncedPaidAt: Date | undefined;
  ...
  static async findBookings(params: { searchTerm?: string; status?: string; paymentStatus?: string; userId?: string; accessibleByUserId?: string; bookingReference?: string; fromDate?: string; toDate?: string; limit?: number; page?: number; sortBy?: string; sortOrder?: "asc" | "desc"; includeCoordinatorProfile?: boolean; }) {
    ...
    if (accessibleByUserId && !Types.ObjectId.isValid(accessibleByUserId)) { throw new Error("Invalid accessible user ID"); }
        ...
        try {
          ...
          const formattedData = data.map(
            (booking: any) => {
              ...
              const coordinatorRequests = assignment?.requests?.map(
                (request: any) => {
  ...
  static async getBookingStats() {
    try {
      ...
      return {
        totalBookings: Object.values(bookingMap).reduce((sum: number, count: any) => sum + count, 0),
  ...
  static async updateBookingNotes(bookingId: string, notes: string, userId: string) {
    if (!bookingId) { throw new Error("Booking ID is required"); }

    if (typeof notes !== "string") { throw new Error("Notes must be a string"); }

    if (!Types.ObjectId.isValid(bookingId) || !Types.ObjectId.isValid(userId)) { throw new Error("Invalid booking or user ID"); }
  ...
  static async rescheduleBooking(params: { bookingId: string; scheduledAt: string; reason: string; userId: string; role: string; }) {
    const { bookingId, scheduledAt, reason, userId, role } = params;

    if (!bookingId) { throw new Error("Booking ID is required"); }

    if (!scheduledAt) { throw new Error("New scheduled date is required"); }
    ...
    if (!isOwner && !isAdmin) { throw new Error("You are not authorized to reschedule this booking"); }
    ...
    if (booking.payment.status !== "PAID") { throw new Error("Only paid bookings can be rescheduled"); }
    ...
    if (Number.isNaN(newSchedule.getTime())) { throw new Error("Invalid scheduled date"); }

    const now = new Date();
    if (newSchedule <= now) { throw new Error("Scheduled date must be in the future"); }
    ...
    if (booking.status === "ASSIGNED" && coordinatorId) {
      ...
      if (!coordinator) { throw new Error("Assigned coordinator not found"); }
    ...
    if (previousScheduledAt) { rescheduleEntry.previousScheduledAt = previousScheduledAt; }
  ...
  static async updateBookingStatus(bookingId: string, status: BookingStatus, userId: string, role: string, reason?: string) {
    const booking = await Booking.findOne({ _id: bookingId, isDeleted: false });
    if (!booking) { throw new Error("Booking not found"); }
    ...
    const ensureAssignment = () => {
      if (!booking.assignment) { booking.assignment = { status: "NOT_STARTED", currentRound: 1, requests: [] }; }
      booking.assignment.requests ??= [];
    ...
    switch (status) {
      ...
      case "ASSIGNED": {
        const assignment = ensureAssignment();
        if (!assignment.assignedCoordinatorId) { throw new Error("Coordinator must be assigned before booking can be marked as ASSIGNED"); }
        booking.status = "ASSIGNED";
  ...
  static async refundBooking(bookingId: string, amount: number, reason: string, refundedBy?: string) {
    ...
    try {
      ...
      await Booking.updateOne(
        { _id: bookingId, "payment.refundReservedAmount": { $gte: amount } },
        { $inc: { "payment.refundReservedAmount": -amount } },
      );
    ...
    let finalPaymentStatus: "PARTIAL_REFUND" | "REFUNDED" = "PARTIAL_REFUND";
    ...
    try {
      await session.withTransaction(async () => {
        ...
        if (reserved < amount) { throw new Error("Refund reservation is missing"); }
    ...
    try {
      ...
      await session.withTransaction(async () => {
        const expiredBookings = await Booking.find(
          ...
          { _id: 1, cartId: 1, userId: 1, bookingReference: 1 },
        ).session(session);
        ...
        for (const expiredBooking of expiredBookings) {
          if (!expiredBooking.userId) { continue; }
  ...
  static async getMyBookingById(bookingId: string, userId: string, role: Role) {
    if (!Types.ObjectId.isValid(bookingId)) { throw new Error("Invalid booking ID"); }
    ...
    const query: Record<string, any> = { _id: bookingObjectId, isDeleted: false };
    ...
    const assignedCoordinator = booking.assignment?.assignedCoordinatorId as any;
    ...
    const visibleRequests = role === Role.COORDINATOR ?
      assignmentRequests.filter((request: any) => {
    ...
    const coordinatorRequests = visibleRequests.map((request: any) => {
      const requestedCoordinator = request.coordinatorId;
      return {
        requestId: request._id,
        status: request.status,
  ...
  static async getMyBookings(params: { userId: string; status?: string; page?: number; limit?: number; sortBy?: string; sortOrder?: "asc" | "desc"; }) {
  ...
  static async getBookingCategory(status: BookingStatus): Promise<BookingCategory> {
    switch (status) {
  ...
  static async getAvailableCoordinators(bookingId: string, userId: string, options: CoordinatorAvailabilityOptions = {}) {
    ...
    if (!isOwner && !isRequestingCoordinator) { throw new Error("You are not authorized to view available coordinators for this booking"); }

    // Never mix rescheduling and reassignment.
    if (isActiveReassignment && options.scheduledAt) { throw new Error("Booking cannot be rescheduled while reassignment is in progress"); }
    ...
    if (booking.status === "ASSIGNED" && !options.scheduledAt && !isActiveReassignment) { throw new Error("Coordinator cannot be changed directly for an assigned booking"); }
    ...
    const locationIds = this.getBookingLocationIds(booking);
    if (locationIds.length === 0) { throw new Error("No service location found in booking"); }
    ...
    if (options.scheduledAt) {
      ...
      if (Number.isNaN(requestedDate.getTime())) { throw new Error("Invalid scheduled date"); }
      if (requestedDate <= new Date()) { throw new Error("Scheduled date must be in the future"); }
    ...
    const excludedCoordinatorIds = new Map<string, Types.ObjectId>();
    ...
    const serviceableLocationMatch: Record<string, any> = { locationId: { $in: locationIds } };
    ...
    const sortDirection: 1 | -1 = coordinatorConfig.sortOrder === "asc" ? 1 : -1;
    ...
    switch (coordinatorConfig.sortBy) {
      ...
      case "rating":
      default: sort["coordinatorProfile.averageRating"] = sortDirection;
    ...
    if (targetScheduledAt && coordinators.length > 0) {
      ...
      const bookingCounts = await Booking.aggregate<{ _id: Types.ObjectId; count: number; }>([
        {
          $match: {
            _id: { $ne: booking._id },
            isDeleted: false,
  ...
  static async selectCoordinator(params: {
    bookingId: string; coordinatorId: string; selectedBy: string; assignmentType: "MANUAL" | "AUTO";
    // Used only when selecting a coordinator as part of a reschedule flow.
    scheduledAt?: string;
    // Required when scheduledAt is supplied.
    ...
    const booking = await Booking.findOne({ _id: bookingId, isDeleted: false });
    if (!booking) { throw new Error("Booking not found"); }
    if (booking.payment.status !== "PAID") { throw new Error("Booking payment must be paid before coordinator selection"); }
    if (!["CONFIRMED", "ASSIGNMENT_PENDING", "ASSIGNED"].includes(booking.status)) { throw new Error(`Cannot select coordinator for ${booking.status} booking`); }
    ...
    if (!scheduledAt && booking.assignment?.assignmentExpiresAt && booking.assignment.assignmentExpiresAt <= new Date()) { throw new Error("Coordinator assignment window has expired"); }
    ...
    if (!isOwner && !isRequestingCoordinator) { throw new Error("Only the booking owner or the coordinator who requested reassignment can select a coordinator"); }
    if (activeReassignment && ["PENDING_REPLACEMENT", "REPLACEMENT_REQUESTED"].includes(activeReassignment.status) && scheduledAt) { throw new Error("Booking cannot be rescheduled while reassignment is in progress"); }
    ...
    if (booking.status === "ASSIGNED" && !scheduledAt && !isActiveReassignment) { throw new Error("Coordinator cannot be changed directly for an assigned booking"); }
    if (isActiveReassignment && assignmentType !== "MANUAL") { throw new Error("Reassignment coordinator selection must be manual"); }
    ...
    if (scheduledAt) {
      ...
      if (Number.isNaN(requestedSchedule.getTime())) { throw new Error("Invalid scheduled date"); }
      if (requestedSchedule <= new Date()) { throw new Error("Scheduled date must be in the future"); }
      if (!rescheduleReason?.trim()) { throw new Error("Reschedule reason is required"); }
      targetScheduledAt = requestedSchedule;
    }
    if (!targetScheduledAt) { throw new Error("Booking schedule is required before selecting a coordinator"); }
    ...
    if (isActiveReassignment) {
      const manualRequestCount = currentRoundRequests.filter((request: any) => request.assignmentType === "MANUAL").length;
      if (manualRequestCount >= reassignmentRequestLimit) { throw new Error(`You can send reassignment requests to a maximum of ${reassignmentRequestLimit} coordinators`); }

      const automaticFallbackStarted = currentRoundRequests.some((request: any) => request.assignmentType === "AUTO");
      if (automaticFallbackStarted) { throw new Error("Automatic replacement assignment has already started"); }
    }
    ...
    if (isRescheduleSelection) {
      ...
      if (sameRescheduleRound) {
        ...
        const manualRequestCount = currentRoundRequests.filter((request: any) => request.assignmentType === "MANUAL").length;
        if (manualRequestCount >= MAX_RESCHEDULE_USER_REQUESTS) { throw new Error(`You can send reschedule requests to a maximum of ${MAX_RESCHEDULE_USER_REQUESTS} coordinators`); }
    ...
    if (alreadyRequested) { throw new Error("This coordinator has already received this booking request"); }
    ...
    const maxDailyBookings = coordinator.coordinatorProfile?.maxDailyBookings ?? 5;
    if (bookedCount >= maxDailyBookings) { throw new Error("Coordinator has reached the maximum booking limit for the selected date"); }
    ...
    if (isActiveReassignment) {
      ...
      const sentCoordinatorRequests = (updatedBooking.assignment?.requests ?? []).filter((request: any) => (request.assignmentRound ?? 1) === (updatedBooking.assignment?.currentRound ?? 1) && request.assignmentType === "MANUAL").length;
    ...
    return {
      ...
      ...(isRescheduleSelection
        ? {
          ...
          sentCoordinatorRequests: (updatedBooking.assignment?.requests ?? []).filter((request: any) =>
            (request.assignmentRound ?? 1) === (updatedBooking.assignment?.currentRound ?? 1) && request.assignmentType === "MANUAL",
          ).length,
        }
        : {}),
  ...
  static async requestReassignment(params: { bookingId: string; requestedBy: string; requestedByRole: ReassignmentRequestedByRole; reason: string; }) {
    const { bookingId, requestedBy, requestedByRole, reason } = params;

    if (!Types.ObjectId.isValid(bookingId)) { throw new Error("Invalid booking ID"); }
    if (!Types.ObjectId.isValid(requestedBy)) { throw new Error("Invalid requester ID"); }
    ...
    try {
      await session.withTransaction(async () => {
        ...
        if (!booking) { throw new Error("Booking not found"); }
        if (booking.status !== "ASSIGNED") { throw new Error("Reassignment is available only for an assigned booking"); }
        if (booking.execution?.startedAt) { throw new Error("Reassignment cannot be requested after execution starts"); }
        if (!booking.assignment || booking.assignment.status !== "ACCEPTED" || !booking.assignment.assignedCoordinatorId) { throw new Error("Booking must have an accepted coordinator before reassignment"); }
        if (booking.assignment.reassignment && ["PENDING_REPLACEMENT", "REPLACEMENT_REQUESTED"].includes(booking.assignment.reassignment.status)) { throw new Error("A reassignment request is already in progress"); }
        ...
        if (requestedByRole === "USER" && !isOwner) { throw new Error("Only the booking owner can request reassignment"); }
        if (requestedByRole === "COORDINATOR" && !isAssignedCoordinator) { throw new Error("Only the assigned coordinator can request reassignment"); }
        if (!isOwner && !isAssignedCoordinator && !isAdmin && !isSystem) { throw new Error("You are not authorized to request reassignment"); }
        ...
        if (requestedByRole === "USER" && booking.scheduledAt) {
          ...
          if (timeUntilBooking <= 0) { throw new Error("Reassignment cannot be requested after the scheduled booking time"); }
          if (timeUntilBooking <= USER_REASSIGNMENT_CUTOFF_MS) { throw new Error("Reassignment cannot be requested within 2 hours of the scheduled booking"); }
        }
  ...
  static async getCoordinatorBookingList(params: { coordinatorId: string; view: CoordinatorBookingView; status?: string; page?: number; limit?: number; sortBy?: string; sortOrder?: "asc" | "desc"; }) {
    ...
    if (view === "REQUESTS") {
      ...
      else { query.status = { $in: ["ASSIGNED", "IN_PROGRESS", "COMPLETED", "CANCELLED"] }; }
    ...
    const formattedData = data.map((booking: any) => {
      ...
      const bookingEntries = booking.entries?.map((entry: any) => {
  ...
  static async processAssignmentTimeouts() {
    ...
    for (const booking of bookings) {
      ...
      if (isActiveReassignment) {
        ...
        if (reassignment.status === "PENDING_REPLACEMENT") { result.reassignmentRetry += 1; }
        else if (reassignment.status === "FAILED") { result.reassignmentFailed += 1; }
      ...
      const hasPending = booking.assignment.requests.some((request: any) => request.status === "PENDING" && (request.assignmentRound ?? 1) === currentRound);
  ...
  static async getBookingExecution(params: { bookingId: string; userId: string; role: Role; }) {
    ...
    if (role === Role.COORDINATOR) {
      const coordinator = booking.assignment?.assignedCoordinatorId as any;
  ...
  static async markCoordinatorArrived(params: { bookingId: string; coordinatorId: string; }) {
    ...
    try {
      await session.withTransaction(async () => {
        ...
        if (!initialBooking) { throw new Error("Booking not found"); }
        if (initialBooking.status !== "ASSIGNED") { throw new Error("Coordinator can arrive only for an assigned booking"); }
        if (initialBooking.assignment?.assignedCoordinatorId?.toString() !== coordinatorId) { throw new Error("Coordinator is not assigned to this booking"); }
        ...
        const arrivalClaim = await Booking.updateOne(
          ...
          { $set: { status: "IN_PROGRESS", "execution.startedAt": now } },
          { session },
        );

        if (arrivalClaim.modifiedCount === 0) { throw new Error("Booking state changed before coordinator arrival could be confirmed"); }

        const booking = await Booking.findById(initialBooking._id).session(session);
        if (!booking) { throw new Error("Booking not found after arrival claim"); }
        ...
        if (!booking.execution.serviceExecutions || booking.execution.serviceExecutions.length === 0) {
          ...
          if (serviceExecutions.length === 0) { throw new Error("Booking does not contain any executable services"); }
  ...
  static async verifyBookingOtp(params: { bookingId: string; otp: string; verifiedBy: string; }) {
    ...
    if (!booking) { throw new Error("Booking not found"); }
    if (booking.status !== "IN_PROGRESS") { throw new Error("Booking must be in progress before OTP verification"); }
    if (booking.assignment?.assignedCoordinatorId?.toString() !== verifiedBy) { throw new Error("Only the assigned coordinator can verify this OTP"); }
    if (booking.execution?.stage !== "CUSTOMER_VERIFICATION_PENDING") { throw new Error("Booking is not waiting for OTP verification"); }
  ...
  static async startBookingService(params: { bookingId: string; executionId: string; startedBy: string; }) {
    const { bookingId, executionId, startedBy } = params;

    const booking = await Booking.findOne({ _id: bookingId, isDeleted: false });
    if (!booking) { throw new Error("Booking not found"); }
    if (booking.status !== "IN_PROGRESS") { throw new Error("Booking must be in progress before starting a service"); }
    if (booking.assignment?.assignedCoordinatorId?.toString() !== startedBy) { throw new Error("Only the assigned coordinator can start this service"); }
    if (booking.execution?.otpVerification?.status !== "VERIFIED") { throw new Error("Customer OTP must be verified before starting services"); }

    const serviceExecution = booking.execution.serviceExecutions.find((service) => service.executionId === executionId);
    if (!serviceExecution) { throw new Error("Service execution not found"); }
    if (serviceExecution.status !== "PENDING") { throw new Error(`Cannot start service with status ${serviceExecution.status}`); }
  ...
  static async completeBookingService(params: { bookingId: string; executionId: string; completedBy: string; notes?: string; }) {
    const { bookingId, executionId, completedBy, notes } = params;

    const booking = await Booking.findOne({ _id: bookingId, isDeleted: false });
    if (!booking) { throw new Error("Booking not found"); }
    if (booking.status !== "IN_PROGRESS" || !booking.execution) { throw new Error("Booking execution is not active"); }
    if (booking.assignment?.assignedCoordinatorId?.toString() !== completedBy) { throw new Error("Only the assigned coordinator can complete this service"); }

    const serviceExecution = booking.execution.serviceExecutions.find((service) => service.executionId === executionId);
    if (!serviceExecution) { throw new Error("Service execution not found"); }
    if (serviceExecution.status !== "IN_PROGRESS") { throw new Error("Only an in-progress service can be completed"); }
    ...
    if (notes?.trim()) { serviceExecution.notes = notes.trim(); }
  ...
  static async skipBookingService(params: { bookingId: string; executionId: string; skippedBy: string; reason: string; }) {
    const { bookingId, executionId, skippedBy, reason } = params;

    const booking = await Booking.findOne({ _id: bookingId, isDeleted: false });
    if (!booking) { throw new Error("Booking not found"); }
    if (booking.status !== "IN_PROGRESS" || !booking.execution) { throw new Error("Booking execution is not active"); }
    if (booking.assignment?.assignedCoordinatorId?.toString() !== skippedBy) { throw new Error("Only the assigned coordinator can skip this service"); }

    const serviceExecution = booking.execution.serviceExecutions.find((service) => service.executionId === executionId);
    if (!serviceExecution) { throw new Error("Service execution not found"); }
    if (!["PENDING", "IN_PROGRESS"].includes(serviceExecution.status)) { throw new Error(`Cannot skip service with status ${serviceExecution.status}`); }
  ...
  static async addBookingMilestone(params: { bookingId: string; code: BookingMilestone; notes?: string; completedBy: string; }) {
    const { bookingId, code, notes, completedBy } = params;

    const allowedMilestones: BookingMilestone[] = ["COORDINATOR_ARRIVED", "OTP_VERIFIED", "SERVICE_STARTED", "CUSTOMER_DETAILS_VERIFIED", "DOCUMENTS_COLLECTED", "FAMILY_TREE_STARTED", "FAMILY_TREE_COMPLETED", "ALL_SERVICES_COMPLETED", "FINAL_REPORT_GENERATED"];
    if (!allowedMilestones.includes(code)) { throw new Error("Invalid milestone code"); }

    const booking = await Booking.findOne({ _id: bookingId, isDeleted: false });
    if (!booking) { throw new Error("Booking not found"); }
    if (booking.assignment?.assignedCoordinatorId?.toString() !== completedBy) { throw new Error("Only the assigned coordinator can update execution milestones"); }
    if (!["IN_PROGRESS", "ASSIGNED"].includes(booking.status)) { throw new Error("Milestones cannot be added at the current booking stage"); }

    booking.execution ??= { stage: "NOT_STARTED", serviceExecutions: [], milestones: [], progressPercentage: 0 };

    const alreadyCompleted = booking.execution.milestones.some((milestone) => milestone.code === code);
    if (alreadyCompleted) { throw new Error("Milestone has already been completed"); }
  ...
  static async completeBookingExecution(params: { bookingId: string; completedBy: string; notes?: string; proofUrls: string[]; }) {
    ...
    if (!assignedCoordinatorId) { throw new Error("Assigned coordinator not found"); }

    //CRITICAL AUTH CHECK
    if (assignedCoordinatorId.toString() !== completedBy) { throw new Error("Only the assigned coordinator can complete this booking"); }
    if (booking.assignment?.status !== "ACCEPTED") { throw new Error("Booking does not have an accepted coordinator"); }
    if (booking.status !== "IN_PROGRESS") { throw new Error("Only an in-progress booking can be completed"); }
    if (!booking.execution) { throw new Error("Booking execution details not found"); }

    //OTP should have been successfullyverified.
    if (booking.execution.otpVerification?.status !== "VERIFIED") { throw new Error("Customer OTP must be verified before completing booking"); }
    ...
    if (!allServicesResolved) { throw new Error("All services must be completed, skipped, or cancelled"); }

    const cleanProofUrls = proofUrls.filter((url) => typeof url === "string").map((url) => url.trim()).filter(Boolean);
    if (!cleanProofUrls.length) { throw new Error("At least one completion proof is required"); }
    ...
    try {
      await session.withTransaction(async () => {
        ...
        await User.updateOne(
          ...
          { $inc: { "coordinatorProfile.totalCompletedBookings": 1 } },
          { session },
        );
  ...
  static async generateBookingOtp(params: { bookingId: string; coordinatorId: string; }) {
    const { bookingId, coordinatorId } = params;

    const booking = await Booking.findOne({ _id: bookingId, isDeleted: false }).select("+execution.otpVerification.otpHash");
    if (!booking) { throw new Error("Booking not found"); }
    if (booking.status !== "IN_PROGRESS") { throw new Error("OTP can be generated only after coordinator arrival"); }
    if (booking.assignment?.assignedCoordinatorId?.toString() !== coordinatorId) { throw new Error("Only the assigned coordinator can request booking OTP"); }

    booking.execution ??= { stage: "CUSTOMER_VERIFICATION_PENDING", serviceExecutions: [], milestones: [], progressPercentage: 0 };

    const now = new Date();
    const previousOtp = booking.execution.otpVerification;
    if (previousOtp?.status === "VERIFIED") { throw new Error("Booking OTP has already been verified"); }
    ...
    if (resendCount >= MAX_OTP_RESENDS) { throw new Error("Maximum OTP resend limit reached"); }
  ...
  static async getBookingInvoice(params: { bookingId: string; requestedBy: string; requestedByRole: Role; }) {
    ...
    if (requestedByRole === Role.USER) {
      const isOwner = booking.userId?.toString() === requestedBy;
      if (!isOwner) { throw new Error("You are not authorized to view this invoice"); }
  ...
  static async createBeneficiaryAccess(bookingId: string) {
    ...
    if (phone) { identityConditions.push({ phoneNumber: phone }); }

    const existingUser = await User.findOne({ role: Role.USER, $or: identityConditions }).select("_id").lean();
    if (existingUser) { booking.beneficiaryUserId = existingUser._id; }
  ...
  static async getBeneficiaryBooking(token: string) {
    if (!token?.trim()) { throw new Error("Booking access token is required"); }
  ...
  static async linkBeneficiaryBookingsToUser(userId: string) {
    if (!Types.ObjectId.isValid(userId)) { throw new Error("Invalid user ID"); }
    ...
    const identityConditions: Record<string, string>[] = [];
    if (email) { identityConditions.push({ "customerDetails.email": email }); }
    if (phone) { identityConditions.push({ "customerDetails.phone": phone }); }

    if (identityConditions.length === 0) { return { linkedCount: 0 }; }
    ...
    if (result.modifiedCount > 0) { await this.invalidateBookingCache(); }
    ...
    for (const booking of bookings) {
      try {
        ...
        if (isAutoReassignment) {
          const currentRound = booking.assignment.currentRound ?? 1;
          const currentRoundRequests = (booking.assignment.requests ?? []).filter((request: any) => (request.assignmentRound ?? 1) === currentRound);
          const manualRequests = currentRoundRequests.filter((request: any) => request.assignmentType === "MANUAL");
          const autoRequests = currentRoundRequests.filter((request: any) => request.assignmentType === "AUTO");
          ...
          const excludedCoordinatorIds: Types.ObjectId[] = [];
          const currentCoordinatorId = booking.assignment.assignedCoordinatorId;

          if (currentCoordinatorId) { excludedCoordinatorIds.push(new Types.ObjectId(currentCoordinatorId.toString())); }

          for (const request of currentRoundRequests) {
            const id = new Types.ObjectId(request.coordinatorId.toString());
            if (!excludedCoordinatorIds.some((excludedId) => excludedId.toString() === id.toString())) { excludedCoordinatorIds.push(id); }
          }
        ...
        if (pendingReschedule && pendingReschedule.assignmentRound === currentRound) {
          ...
          if (manualRequests.length < MAX_RESCHEDULE_USER_REQUESTS && !selectionExpired) { result.waitingForUserSelection += 1; continue; }
        }

        const excludedCoordinatorIds = currentRoundRequests.map((request: any) => new Types.ObjectId(request.coordinatorId.toString()));
  ...
  static async exportBookingsToCsv(bookingIds: string[]) {
    if (!Array.isArray(bookingIds) || bookingIds.length === 0) { throw new HttpError(400, "At least one booking ID is required"); }
    ...
    for (const bookingId of uniqueBookingIds) {
      if (!Types.ObjectId.isValid(bookingId)) { throw new HttpError(400, `Invalid booking ID: ${bookingId}`); }
    }
    ...
    ).populate({ path: "assignment.assignedCoordinatorId", select: "fullName email phoneNumber userReference" }).lean();
    ...
    const getEntryDetails = (entries: any[]) => {
