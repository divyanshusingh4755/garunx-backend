...
export class BookingService {
  private static async invalidateBookingCache(
    ...
    if (bookingId) {
      operations.push(
        RedisCacheService.delete(
          CacheKeys.bookingDetail(
            bookingId,
          ),
        ),

        RedisCacheService.delete(
          CacheKeys.bookingInvoice(
            bookingId,
          ),
        ),
      );
  ...
  private static async assignReplacementCoordinatorRequest(
    ...
    if (
      ...
      if (
        ...
        throw new Error(
          "Maximum automatic reassignment attempts reached",
        );
      }
    }
    ...
    if (alreadyRequested) {
      throw new Error(
        "This coordinator has already received this reassignment request",
      );
    }
    ...
    if (!coordinator) {
      throw new Error(
        "Replacement coordinator is not available for this booking",
      );
    }
    ...
    if (externalSession) {
      bookedCountQuery.session(
        externalSession,
      );
    }

    const bookedCount =
      await bookedCountQuery;
    ...
    if (
      ...
      throw new Error(
        "Replacement coordinator has reached the maximum booking limit for the selected date",
      );
    }
    ...
    const persist =
      async (
        ...
        await OutboxService.createEvent({
          eventId:
            `BOOKING.ASSIGNMENT_REQUESTED:${booking._id.toString()}:${currentRound}:${coordinatorObjectId.toString()}`,
  ...
  private static handleFailedReassignmentAttempt(
    ...
    booking.set(
      "assignment.reassignment.replacementCoordinatorId",
      undefined,
    );
  ...
  private static clearAcceptedCoordinator(
    ...
    booking.set(
      "assignment.coordinatorAcceptedAt",
      undefined,
    );
    ...
    try {
      await session.withTransaction(
        async () => {
          ...
          if (!booking) {
            throw new Error(
              "Booking not found for payment order",
            );
          }
          ...
          if (
            ...
            await OutboxService
              .createEvent({
                eventId:
                  `BOOKING.CONFIRMED:${booking._id.toString()}`,
  ...
  private static async assignCoordinatorRequest(params: {
    ...
    const isCurrentRescheduleRound =
      assignmentType === "MANUAL" &&
      pendingReschedule &&
      pendingReschedule
        .assignmentRound ===
    ...
    if (
      isCurrentRescheduleRound
    ) {
      const manualRescheduleRequests =
        booking.assignment
          .requests
          .filter(
            (request: any) =>
      ...
      if (
        manualRescheduleRequests.length >=
        MAX_RESCHEDULE_USER_REQUESTS
      ) {
        throw new Error(
          `You can send reschedule requests to a maximum of ${MAX_RESCHEDULE_USER_REQUESTS} coordinators`,
        );
      }
      ...
      if (
        ...
        throw new Error(
          "Automatic coordinator assignment has already started for this reschedule",
        );
      }
    }
  ...
  */
    ...
    try {
      await session.withTransaction(async () => {
        ...
        await OutboxService.createEvent({
          eventId:
            `BOOKING.ASSIGNMENT_REQUESTED:${booking._id.toString()}:${booking.assignment.currentRound}:${coordinatorObjectId.toString()}`,

          eventType:
            DOMAIN_EVENTS.BOOKING_ASSIGNMENT_REQUESTED,
      ...
      await session.withTransaction(async () => {
        if (
          ...
          if (
            ...
            throw new Error(
              "Invalid payment amount received from Cashfree",
            );
          }
        ...
        if (paymentStatus === "FAILED") {
          ...
          if (
            failedUpdate.modifiedCount > 0 &&
            booking.userId
          ) {
            await OutboxService.createEvent({
              eventId:
                `PAYMENT.FAILED:${booking._id.toString()}:${paymentId ?? orderId}`,
            ...
            if (
              ...
              bookingQuery =
                bookingQuery
                  ...
                  .populate({
                    path:
                      "assignment.requests.coordinatorId",
        ...
        async () => {
          ...
          return {
            ...
            payment: {
              ...
              paymentSessionId:
                booking.payment.paymentSessionId,
  ...
  static async rescheduleBooking(params: {
    ...
    if (
      ...
      throw new Error(
        "Booking cannot be rescheduled while reassignment is in progress",
      );
    }
    ...
    if (
      ...
      const coordinator =
        await User.findById(
          ...
          .select({
            ...
            "coordinatorProfile.maxDailyBookings":
              1,
          })
          .lean();
       ...
       * to handle bookings.
       */
      const coordinatorOperationallyAvailable =
        coordinator.role ===
        "COORDINATOR" &&
       ...
       */
      let hasScheduleCapacity =
        false;

      if (
        coordinatorOperationallyAvailable
      ) {
        const startOfDay =
          new Date(
            newSchedule,
      ...
      await session.withTransaction(
        async () => {
          ...
          if (
            ...
            await OutboxService.createEvent({
              eventId:
                `BOOKING.RESCHEDULED:${booking._id.toString()}:${newSchedule.getTime()}`,

              eventType:
                DOMAIN_EVENTS.BOOKING_RESCHEDULED,
      ...
      await session.withTransaction(async () => {
        ...
        if (booking.userId) {
          ...
          if (status === "CANCELLED") {
            await OutboxService.createEvent({
              eventId:
                `BOOKING.CANCELLED:${booking._id.toString()}`,
              ...
              payload: {
                ...
                coordinatorId:
                  booking.assignment?.assignedCoordinatorId?.toString() ?? null,

                reason:
                  booking.cancellation?.reason ?? reason ?? "Booking cancelled",
              },
          ...
          if (status === "IN_PROGRESS") {
            await OutboxService.createEvent({
              eventId:
                `BOOKING.STARTED:${booking._id.toString()}`,

              eventType:
                DOMAIN_EVENTS.BOOKING_STARTED,
          ...
          if (status === "COMPLETED") {
            await OutboxService.createEvent({
              eventId:
                `BOOKING.COMPLETED:${booking._id.toString()}`,

              eventType:
                DOMAIN_EVENTS.BOOKING_COMPLETED,
  ...
  static async refundBooking(
    ...
    if (
      ...
      throw new Error(
        `Maximum currently refundable amount is ₹${available}`,
      );
    }

    const orderId =
      reservedBooking.payment
        .providerOrderId!;
    ...
    if (
      ...
      throw new Error(
        `Refund was not accepted by Cashfree. Status: ${providerStatus}`,
      );
    }
        ...
        async () => {
          ...
          if (
            ...
            await OutboxService
              .createEvent({
                eventId:
                  `PAYMENT.REFUNDED:${booking._id.toString()}:${refundReference}`,
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        for (const expiredBooking of expiredBookings) {
          ...
          await OutboxService.createEvent({
            eventId:
              `PAYMENT.FAILED:${expiredBooking._id.toString()}:EXPIRED`,
            ...
            payload: {
              ...
              bookingReference:
                expiredBooking.bookingReference,

              userId:
                expiredBooking.userId.toString(),
    ...
    const visibleRequests =
      role === Role.COORDINATOR
        ? assignmentRequests.filter(
          (request: any) => {
            const populatedCoordinator =
              request.coordinatorId;

            const requestCoordinatorId =
  ...
  static async getAvailableCoordinators(
    bookingId: string,
    userId: string,
    options: CoordinatorAvailabilityOptions = {},
  ) {
    /*
     * =========================================================
    ...
    const coordinatorConfig =
      await CoordinatorSelectionConfigService.getEffectiveConfig();
    ...
    if (coordinatorConfig.matchCaste) {
      ...
      if (!bookingCaste) {
        throw new Error(
          "Caste matching is enabled, but booking caste is missing",
        );
      }
    ...
    if (coordinatorConfig.matchGotra) {
      ...
      if (!bookingGotra) {
        throw new Error(
          "Gotra matching is enabled, but booking gotra is missing",
        );
      }
    ...
    if (
      coordinatorConfig.minRating > 0
    ) {
      query[
        "coordinatorProfile.averageRating"
      ] = {
        $gte:
          coordinatorConfig.minRating,
      };
    }
  ...
  static async selectCoordinator(params: {
    ...
    /*
     * Used only when selecting a coordinator
     * as part of a reschedule flow.
     */
    scheduledAt?: string;
    ...
    if (
      ...
      return {
        ...
        message:
          "Reassignment request sent successfully. The current coordinator remains assigned until one replacement coordinator accepts.",
      };
    }
      ...
      await session.withTransaction(
        async () => {
          ...
          if (
            ...
            throw new Error(
              `Cannot respond to assignment for ${booking.status} booking`,
            );
          }
          ...
          if (
            ...
            throw new Error(
              "Booking is no longer available for reassignment",
            );
          }
          ...
          if (
            ...
            throw new Error(
              "Pending assignment request not found",
            );
          }
          ...
          if (
            ...
            postCommitError =
              new Error(
                "Booking execution has already started and reassignment is no longer available",
              );
          ...
          if (
            ...
            if (
              ...
              this
                .handleFailedReassignmentAttempt(
                  booking,
                  "Replacement coordinator response deadline expired",
                );
            } else {
              const hasOtherPending =
                booking.assignment
            ...
            postCommitError =
              new Error(
                "Coordinator response deadline has expired",
              );
          ...
          if (
            ...
            if (
              ...
              throw new Error(
                "Previous coordinator not found for reassignment",
              );
            }
              ...
              await Booking.updateOne(
                {
                  ...
                  "assignment.reassignment.status":
                    "REPLACEMENT_REQUESTED",

                  "assignment.reassignment.assignmentRound":
                    currentRound,
...

                {
                  $set: {
                    ...
                    "assignment.reassignment.completedAt":
                      now,
            ...
            if (
              ...
              throw new Error(
                "Reassignment could not be completed because the booking state changed",
              );
            }
            ...
            await User.updateOne(
              ...
              {
                $inc: {
                  "coordinatorProfile.totalAssignedBookings":
                    1,
                },
              },
            ...
            if (
              ...
              await OutboxService
                .createEvent({
                  eventId:
                    `BOOKING.ASSIGNED:${booking._id.toString()}:${currentRound}`,
          ...
          const isPendingRescheduleForCurrentRound =
            Boolean(
              pendingReschedule &&
              pendingReschedule
                .assignmentRound ===
          ...
          if (
            ...
            if (
              ...
              await OutboxService
                .createEvent({
                  eventId:
                    `BOOKING.RESCHEDULED:${booking._id.toString()}:${pendingReschedule.requestedScheduledAt.getTime()}`,
    ...
    try {
      await session.withTransaction(async () => {
        ...
        if (booking.status !== "ASSIGNED") {
          throw new Error(
            "Reassignment is available only for an assigned booking",
          );
        }
        ...
        if (
          ...
          throw new Error(
            "Booking must have an accepted coordinator before reassignment",
          );
        }

        if (
          booking.assignment.reassignment &&
          ["PENDING_REPLACEMENT", "REPLACEMENT_REQUESTED"].includes(
        ...
        const isAssignedCoordinator =
          currentCoordinatorId.toString() === requestedBy;
        ...
        booking.assignment.reassignment = {
          ...
          reason: reason.trim(),
          requestedAt: now,
          previousCoordinatorId: new Types.ObjectId(
            currentCoordinatorId.toString(),
          ),
  ...
  static async markCoordinatorArrived(params: {
    ...
    try {
      await session.withTransaction(async () => {
        ...
        if (
          initialBooking.assignment?.assignedCoordinatorId?.toString() !==
          coordinatorId
        ) {
        ...
        if (arrivalClaim.modifiedCount === 0) {
          throw new Error(
            "Booking state changed before coordinator arrival could be confirmed",
          );
        }

        const booking = await Booking.findById(initialBooking._id).session(
          session,
        );
    ...
    await ChatConversationService.createForBooking({
      ...
      coordinatorId:
        updatedBooking.assignment.assignedCoordinatorId.toString(),
    });
  ...
  static async exportBookingsToCsv(
    bookingIds: string[],
  ) {
    if (
      !Array.isArray(
    ...
    const bookings =
      await Booking.find({
        ...
        .populate({
          ...
          select:
            "fullName email phoneNumber userReference",
        })
        .lean();
