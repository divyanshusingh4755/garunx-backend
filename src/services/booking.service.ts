...
export class BookingService {
  ...
  private static getUnavailableDateRange(scheduledAt: Date | string): { startOfDay: Date, endOfDay: Date } {
  ...
  private static buildCoordinatorUnavailableDateFilter(scheduledAt: Date | string) {
      ...
      await session.withTransaction(async () => {
        ...
        switch (status) {
          ...
          case "COMPLETED": {
            ...
            await User.updateOne({ _id: assignedCoordinatorId, },
          ...
          case "CANCELLED": {
            ...
            if (!normalizedReason) { throw new Error("Cancellation reason required",); }
  ...
  static async refundBooking(bookingId: string, amount: number, reason: string, refundedBy?: string) {
    ...
    try {
      await session.withTransaction(async () => {
        ...
        if (!booking.userId) { throw new Error("Booking user not found",); }
        ...
        const finalPaymentStatus: "PARTIAL_REFUND" | "REFUNDED" = finalTotalRefunded >= booking.pricing.grandTotal ? "REFUNDED" : "PARTIAL_REFUND";
  ...
  static async completeBookingExecution(params: { bookingId: string; completedBy: string; notes?: string; proofUrls: string[]; }) {
    ...
    const walletTransaction =
      await WalletService.creditCoordinatorEarning({
        coordinatorId: assignedCoordinatorId,
        bookingId: booking._id,
