...
export class WalletService {
    ...
    static async getMyTransactions(
        ...
        const bookingIds = [...new Set(transactions.map((transaction) => transaction.bookingId?.toString()).filter((bookingId): bookingId is string => Boolean(bookingId))),];
        ...
        const data = transactions.map((transaction) => {
            ...
            if (!booking) { return { ...transaction, bookingSnapshot: null, }; }
