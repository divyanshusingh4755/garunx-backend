...
export class UserQueryService {
  private static async invalidateUserQueryCache(
    queryId?: string,
  ): Promise<void> {
    const operations: Promise<unknown>[] = [
      RedisCacheService.deleteByPattern(
        CacheKeys.userQueryMyListPattern(),
      ),

      RedisCacheService.deleteByPattern(
        CacheKeys.userQueryAdminListPattern(),
      ),
    ];
    ...
    if (queryId) {
      operations.push(
        RedisCacheService.deleteByPattern(
          `${CACHE_PREFIX}:user-query:user-detail:${queryId}:*`,
        ),

        RedisCacheService.delete(
          CacheKeys.userQueryAdminDetail(
            queryId,
          ),
        ),
      );
  ...
  private static validateObjectId(id: string, fieldName: string): void {
  ...
  private static generateQueryReference(queryId: Types.ObjectId): string {
    return `QRY-${queryId.toString().slice(-8).toUpperCase()}`;
  }

  private static async createActivity(input: {
    ...
    if (Object.prototype.hasOwnProperty.call(input, "newValue")) {
      activity.newValue = input.newValue;
    }
  ...
  static async createUserQueryService(input: CreateUserQueryInput) {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        const [messageDocument] = await UserQueryMessage.create(
          [messagePayload],
          { session },
        );
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        const messagePayload: {
          ...
          queryId: queryDocument._id,
          senderId: requesterObjectId,
          senderType: this.resolveMessageSenderType(
            queryDocument.requesterType,
          ),
          imageUrls,
        };
        ...
        queryDocument.latestMessage = this.latestMessageText(
          message,
          imageUrls,
        );
        ...
        if (queryDocument.assignedAdminId) {
          await OutboxService.createEvent({
            eventId:
              `QUERY.REQUESTER_REPLIED:${messageDocument._id.toString()}`,

            eventType:
              DOMAIN_EVENTS.QUERY_REQUESTER_REPLIED,

            aggregateType:
              "USER_QUERY",

            aggregateId:
              queryDocument._id.toString(),
            ...
            payload: {
              ...
              queryReference:
                queryDocument.queryReference,

              subject:
                queryDocument.subject,

              requesterId:
                queryDocument.requesterId.toString(),

              requesterType:
                queryDocument.requesterType,

              assignedAdminId:
                queryDocument.assignedAdminId.toString(),
            },
  ...
  static async markUserQueryAsRead(
    ...
    const requesterQuery =
      await UserQuery.findOneAndUpdate(
        ...
        .select(
          "requesterUnreadCount adminUnreadCount",
        )
        .lean();
  ...
  static async sendAdminQueryReply(input: AdminReplyInput) {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        await OutboxService.createEvent({
          eventId:
            `QUERY.ADMIN_REPLIED:${messageDocument._id.toString()}`,

          eventType:
            DOMAIN_EVENTS.QUERY_ADMIN_REPLIED,
  ...
  static async updateUserQueryStatus(input: UpdateStatusInput) {
    ...
    const allowedTransitions: Record<UserQueryStatus, UserQueryStatus[]> = {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        let statusEventType:
          | typeof DOMAIN_EVENTS.QUERY_RESOLVED
          | typeof DOMAIN_EVENTS.QUERY_REJECTED
          | typeof DOMAIN_EVENTS.QUERY_REOPENED
          | undefined;
        ...
        if (statusEventType) {
          await OutboxService.createEvent({
            eventId:
              `${statusEventType}:${queryDocument._id.toString()}:${queryDocument.updatedAt.getTime()}`,
  ...
  static async updateUserQueryPriority(input: UpdatePriorityInput) {
  ...
  static async updateUserQueryCategory(input: UpdateCategoryInput) {
    return this.updateSimpleField({
      queryId: input.queryId,
      adminId: input.adminId,
      field: "category",
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        const activityInput: {
          queryId: Types.ObjectId;
          performedBy: Types.ObjectId;
          type: "PRIORITY_CHANGED" | "CATEGORY_CHANGED";
  ...
  static async assignUserQuery(input: AssignQueryInput) {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        await OutboxService.createEvent({
          eventId:
            `QUERY.ASSIGNED:${queryDocument._id.toString()}:${adminObjectId.toString()}:${queryDocument.updatedAt.getTime()}`,

          eventType:
            DOMAIN_EVENTS.QUERY_ASSIGNED,
  ...
  static async deleteUserQuery(input: DeleteQueryInput) {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        const queryDocument =
          await UserQuery.findById(queryObjectId).session(session);
        ...
        await OutboxService.createEvent({
          eventId:
            `QUERY.DELETED:${queryDocument._id.toString()}:${queryDocument.updatedAt.getTime()}`,

          eventType:
            DOMAIN_EVENTS.QUERY_DELETED,
  ...
  static async exportUserQueriesToCsv(
    queryIds: string[],
  ) {
    if (
      !Array.isArray(
      ...
      throw new Error(
        "At least one query ID is required",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "A maximum of 1000 queries can be exported at once",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "User queries not found for export",
      );
    }
