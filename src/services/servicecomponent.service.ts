...
import {
  ServiceCascadingEngine,
} from "./cascading-engine.service.js";
...
export class ServiceComponentService {
  private static async invalidateServiceComponentCache(
    serviceId: string,
  ): Promise<void> {
    await Promise.all([
  ...
  static async bulkUpsertComponents(
    ...
    try {
       ...
       */
      await this.invalidateServiceComponentCache(
        payload.serviceId,
      );
