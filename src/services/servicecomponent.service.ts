...
export class ServiceComponentService {
  private static normalizeItemIds(
    ...
    return [
      ...new Set(
        items.map((item) => (typeof item === "string" ? item : item.itemId)),
      ),
    ];
  }
  ...
  private static async prepareComponents(
    ...
    const componentMap = new Map(
      dbComponents.map((component) => [component._id.toString(), component]),
    );
    ...
    return components.map((component) => {
      ...
      const formattedItems: IServiceComponentItem[] = this.normalizeItemIds(
        component.items,
      ).map((itemId) => {
  ...
  static async bulkUpsertComponents(payload: ComponentPayload) {
    ...
    try {
      ...
      if (payload.components.length === 0) {
        ...
        await ServiceCascadingEngine.run(payload.serviceId);
      ...
      const preparedComponents = await this.prepareComponents(
        payload.components,
        session,
      );
      ...
      const selectedComponentIds = preparedComponents.map(
  ...
  static async patchComponent(payload: PatchPayload) {
    await this.validateServiceTier(payload.serviceId, payload.tierId);
    ...
    const serviceComponent = await ServiceComponent.findOne(filter);
    ...
    const updatedComponent = await ServiceComponent.findOneAndUpdate(
      filter,
      {
        $set: updateData,
      },
