...
import { ServiceComponent, type IServiceComponentItem, } from "../models/servicecomponent.model.js";
...
export class ServiceComponentService {
  private static async invalidateServiceComponentCache(serviceId: string,): Promise<void> {
    await Promise.all([
  ...
  private static normalizeItemIds(items: Array<string | { itemId: string; }> = [],): string[] {
    return [...new Set(items.map((item) => typeof item === "string" ? item : item.itemId,),),];
  }

  private static async validateServiceTier(serviceId: string, tierId: string, session?: ClientSession,) {
    ...
    if (!tierExists) { throw createHttpError("Tier does not belong to service", 400,); }
    return service;
  }

  private static async prepareComponents(components: ComponentInput[], session?: ClientSession,) {
    const componentIds = [...new Set(components.map((component) => component.componentId,),),];
    if (componentIds.length !== components.length) { throw createHttpError("Duplicate componentId values are not allowed", 400,); }
    ...
    if (dbComponents.length !== objectIds.length) { throw createHttpError("One or more components are invalid or inactive", 400,); }
    ...
    for (const component of components) {
      ...
      if (!dbComponent) { throw createHttpError("Invalid component", 400,); }

      const itemIds = this.normalizeItemIds(component.items,);
      if (dbComponent.isBundled && itemIds.length === 0) { throw createHttpError(`Component ${dbComponent.name} requires items`, 400,); }
      if (!dbComponent.isBundled && itemIds.length > 0) { throw createHttpError(`Component ${dbComponent.name} is not bundled`, 400,); }
    ...
    if (dbItems.length !== itemObjectIds.length) { throw createHttpError("One or more component items are invalid or inactive", 400,); }

    const itemMap = new Map(dbItems.map((item) => [item._id.toString(), item,],),);

    return components.map((component) => {
      const dbComponent = componentMap.get(component.componentId,)!;
      const formattedItems: IServiceComponentItem[] = this.normalizeItemIds(component.items,).map((itemId) => {
  ...
  static async bulkUpsertComponents(payload: ComponentPayload,) {
  ...
  static async replaceComponents(payload: ComponentPayload,) {
    const session = await mongoose.startSession();
  ...
  static async getComponentsByServiceAndTier(serviceId: string, tierId: string,) {
  ...
  static async patchComponent(payload: PatchPayload,) {
    const session = await mongoose.startSession();
    ...
    try {
      ...
      if (!serviceComponent) { throw createHttpError("Component not found in this service tier", 404,); }

      const updateData: Partial<{ isRequired: boolean; items: IServiceComponentItem[]; }> = {};
      ...
      if (payload.items !== undefined) {
        ...
        if (!baseComponent || !baseComponent.isActive) { throw createHttpError("Invalid or inactive base component", 400,); }

        const itemIds = this.normalizeItemIds(payload.items,);
        if (baseComponent.isBundled && itemIds.length === 0) { throw createHttpError(`Component ${baseComponent.name} requires items`, 400,); }
        if (!baseComponent.isBundled && itemIds.length > 0) { throw createHttpError("Non-bundled component cannot have items", 400,); }
        ...
        if (dbItems.length !== objectIds.length) { throw createHttpError("One or more component items are invalid or inactive", 400,); }
      ...
      if (!updatedComponent) { throw createHttpError("Component not found in this service tier", 404,); }

      await ServiceCascadingEngine.run(payload.serviceId, session,);
      await session.commitTransaction(); await this.invalidateServiceComponentCache(payload.serviceId,);
