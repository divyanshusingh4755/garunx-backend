...
export class ServicePricingService {
  private static async invalidatePricingCache(serviceId: string): Promise<void> {
    await Promise.all([
  ...
  static async bulkUpsertTierPricing(payload: BulkTierPricingPayload) {
    ...
    for (const locationPricing of pricing) {
      ...
      if (locationIdsSeen.has(locationId)) { throw createHttpError(`Duplicate location in pricing: ${locationId}`, 400); }

      locationIdsSeen.add(locationId);
      if (!serviceLocationIds.has(locationId)) { throw createHttpError(`Location ${locationId} does not belong to this service or is inactive`, 400); }
      ...
      for (const componentPricing of locationPricing.components) {
        ...
        if (componentIdsSeen.has(componentId)) { throw createHttpError(`Duplicate component ${componentId} for location ${locationId}`, 400); }

        componentIdsSeen.add(componentId);
        allComponentIds.add(componentId);

        if (typeof componentPricing.price !== "number" || !Number.isFinite(componentPricing.price) || componentPricing.price < 0) { throw createHttpError(`Invalid price for component ${componentId}`, 400); }

        if (componentPricing.taxProfileId) { allTaxProfileIds.add(componentPricing.taxProfileId); }
    ...
    const validComponentSet = new Set(validComponents.filter((record) => record.componentId).map((record) => { const component = record.componentId as unknown as { _id: Types.ObjectId; }; return component._id.toString(); }));
    const invalidComponentIds = [...allComponentIds].filter((componentId) => !validComponentSet.has(componentId));

    if (invalidComponentIds.length > 0) { throw createHttpError(`Components are invalid, inactive, or do not belong to this service tier: ${invalidComponentIds.join(", ")}`, 400); }
    ...
    try {
      await session.withTransaction(async () => {
        ...
        for (const taxProfileId of taxProfileIds) {
          ...
          if (lockResult.matchedCount !== 1) { throw createHttpError(`Tax profile ${taxProfileId} is invalid or inactive`, 400); }
        }
  ...
  static async resolvePricing(serviceId: string, tierId: string, locationId: string) {
    ...
    return RedisCacheService.getOrSet({
      ...
      loader: async () => {
        ...
        if (!service) { throw createHttpError("Service not found", 404); }
        if (!service.isActive) { throw createHttpError("Service is inactive", 400); }
        if (!service.isComplete) { throw createHttpError("Service configuration is incomplete", 400); }

        const tier = service.tiers.find((item) => item.tierId.toString() === tierId);
        if (!tier) { throw createHttpError("Tier does not belong to service", 400); }

        const location = service.locations.find((item) => item.locationId.toString() === locationId);
        if (!location) { throw createHttpError("Location does not belong to service", 400); }
        if (!location.isActive) { throw createHttpError("Location is inactive for this service", 400); }
        ...
        const resolvedComponents = components.flatMap((component) => {
          ...
          const taxProfile = pricingRecord?.taxProfileId as { _id: Types.ObjectId; name?: string; code?: string; treatment?: string; totalRate?: number; isActive?: boolean; } | null | undefined;
