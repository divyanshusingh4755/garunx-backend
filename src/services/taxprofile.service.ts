...
export class TaxProfileService {
    private static async getTaxProfileUsage(
        ...
        for (const pricing of packagePricing) {
            const pkg = pricing.packageId as unknown as {
                _id: Types.ObjectId;
                name?: string;
            } | null;
    ...
    static async getTaxProfileById(
        ...
        const taxProfile = await TaxProfile.findById(
            taxProfileId,
        )
            .populate(
                "createdBy",
    ...
    static async updateTaxProfileStatus(
        ...
        if (!isActive) {
            ...
            if (!isActive) {
                const usage = await this.getTaxProfileUsage(
                    taxProfile._id,
                );
