...
export class TaxProfileService {
  private static async invalidateTaxProfileCache(
    taxProfileId?: string,
  ): Promise<void> {
    ...
    if (taxProfileId) { operations.push(RedisCacheService.delete(CacheKeys.taxProfileDetail(taxProfileId))); }

    await Promise.all(operations);
  }

  private static async invalidateDependentPricingCaches(): Promise<void> {
    await Promise.all([
  ...
  private static validateObjectId(id: string, fieldName = "taxProfileId"): void { if (!Types.ObjectId.isValid(id)) { throw new Error(`Invalid ${fieldName}`); } }

  private static toObjectId(value: string | Types.ObjectId, fieldName: string): Types.ObjectId {
  ...
  private static validateTreatmentRate(treatment: TaxTreatment, totalRate: number): void {
    if (treatment === "TAXABLE" && totalRate <= 0) { throw new Error("Taxable tax profile must have a rate greater than zero"); }
    if (treatment !== "TAXABLE" && totalRate !== 0) { throw new Error("Non-taxable tax profiles must have a rate equal to zero"); }
  }

  static async createTaxProfile(payload: CreateTaxProfilePayload): Promise<ITaxProfile> {
    ...
    if (existingProfile) { throw new Error(`Tax profile with code ${code} already exists`); }

    const description = this.normalizeOptionalString(payload.description);
    const createPayload: { name: string; code: string; treatment: TaxTreatment; totalRate: number; isActive: true; description?: string; createdBy?: Types.ObjectId; updatedBy?: Types.ObjectId; } = { name, code, treatment: payload.treatment, totalRate: payload.totalRate, isActive: true };
  ...
  static async getTaxProfiles(filters: TaxProfileFilters = {}) {
    ...
    return RedisCacheService.getOrSet({
      ...
      loader: async () => {
        ...
        if (filters.treatment) { query.treatment = filters.treatment; }
  ...
  static async updateTaxProfile(taxProfileId: string, payload: UpdateTaxProfilePayload) {
    this.validateObjectId(taxProfileId);
    const taxProfile = await TaxProfile.findById(taxProfileId);
    if (!taxProfile) { throw new Error("Tax profile not found"); }
    ...
    if (payload.name !== undefined) { taxProfile.name = payload.name.trim(); }
    if (payload.treatment !== undefined) { taxProfile.treatment = payload.treatment; }
    if (payload.totalRate !== undefined) { taxProfile.totalRate = payload.totalRate; }
    if (Object.prototype.hasOwnProperty.call(payload, "description")) {
      const description = this.normalizeOptionalString(payload.description);
      if (description) { taxProfile.description = description; }
      else { taxProfile.set("description", undefined); }
    }

    if (payload.updatedBy) { taxProfile.updatedBy = this.toObjectId(payload.updatedBy, "updatedBy"); }
  ...
  static async updateTaxProfileStatus(taxProfileId: string, isActive: boolean, updatedBy?: string | Types.ObjectId) {
    this.validateObjectId(taxProfileId);

    const objectId = new Types.ObjectId(taxProfileId);
    const session = await mongoose.startSession();
    let updatedTaxProfile: ITaxProfile | null = null;

    try {
      await session.withTransaction(async () => {
        const taxProfile = await TaxProfile.findById(objectId).session(session);
        if (!taxProfile) { throw new Error("Tax profile not found"); }
        if (taxProfile.isActive === isActive) { updatedTaxProfile = taxProfile; return; }
        ...
        if (!isActive) {
          ...
          if (servicePricingCount > 0 || packagePricingCount > 0) {
            const error = new Error("Cannot deactivate this tax profile because it is used by active service or package pricing. Reassign those pricing records first.") as Error & { statusCode?: number; };
        ...
        if (updatedBy) { taxProfile.updatedBy = this.toObjectId(updatedBy, "updatedBy"); }
    ...
    if (!updatedTaxProfile) { throw new Error("Unable to update tax profile status"); }
  ...
  static async exportTaxProfilesToCsv(taxProfileIds: string[]): Promise<{ csv: string; total: number; }> {
    ...
    if (taxProfiles.length === 0) { throw new HttpError(404, "No tax profiles found for export"); }
    ...
    const escapeCsv = (value: unknown): string => {
      ...
      if (/^[=+\-@]/.test(stringValue.trimStart())) { stringValue = `'${stringValue}`; }
