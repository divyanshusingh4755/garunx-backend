...
import { NotificationDevice, type DevicePlatform } from "../models/notification-device.model.js";
...
export class NotificationDeviceService {
    static async registerDevice(params: { userId: string; token: string; platform: DevicePlatform; deviceId?: string; }) {
    ...
    static async deactivateDevice(params: { userId: string; token: string; }) {
        ...
        if (!device) { throw new Error("Notification device not found"); }
        return device;
    }

    static async sendToUser(params: SendPushToUserParams) {
        ...
        response.responses.forEach((item, index) => {
            ...
            if (code === "messaging/registration-token-not-registered" || code === "messaging/invalid-registration-token") {
                ...
                if (device) { invalidDeviceIds.push(device._id); }
            }
        },
        );
