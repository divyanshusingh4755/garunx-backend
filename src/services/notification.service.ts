...
export interface NotificationChannels { email?: boolean; push?: boolean; }

export class NotificationService {
    private static isCategoryEnabled(params: {
        category: NotificationCategory;
        preferences: {
            categories: { booking: boolean; payment: boolean; query: boolean; review: boolean; promotional: boolean; appUpdate: boolean; newFeature: boolean; };
        };
    }): boolean {
    ...
    private static async emitUnreadCount(userId: string | Types.ObjectId) {
    ...
    static async createNotification(params: { recipientId: string | Types.ObjectId; recipientRole: Role; title: string; message: string; type: NotificationType; referenceId?: string | Types.ObjectId; dedupeKey?: string; emailRequested?: boolean; pushRequested?: boolean; templateCode?: string; emailSubject?: string; emailBody?: string; pushTitle?: string; pushMessage?: string; showInApp?: boolean; }) {
        ...
        if (!Types.ObjectId.isValid(recipientId)) { throw new Error("Invalid recipient ID"); }
        ...
        try {
            ...
            if (dedupeKey && error instanceof mongo.MongoServerError && error.code === 11000) {
                ...
                if (existingNotification) { return { notification: existingNotification, created: false }; }
            }
    ...
    static async getMyNotifications(params: { userId: string; page?: number; limit?: number; isRead?: boolean; type?: NotificationType; }) {
        ...
        const query: Record<string, any> = { recipientId, isDeleted: false, showInApp: true };
    ...
    static async markAsRead(params: { notificationId: string; userId: string; }) {
        const { notificationId, userId } = params;

        if (!Types.ObjectId.isValid(notificationId)) { throw new Error("Invalid notification ID"); }
        if (!Types.ObjectId.isValid(userId)) { throw new Error("Invalid user ID"); }

        const notification = await Notification.findOneAndUpdate(
            { _id: notificationId, recipientId: userId, isDeleted: false, showInApp: true },
    ...
    static async markAllAsRead(userId: string) {
        ...
        const result = await Notification.updateMany(
            { recipientId: userId, isRead: false, isDeleted: false, showInApp: true },
    ...
    static async deleteNotification(params: { notificationId: string; userId: string; }) {
        const { notificationId, userId } = params;
    ...
    static async sendAdminNotification(params: { title: string; message: string; type: NotificationType; audience: "ALL" | Role; referenceId?: string; }) {
    ...
    static async createFromTemplate(params: { recipientId: string | Types.ObjectId; recipientRole: Role; templateCode: string; variables?: Record<string, string | number | boolean | Date | null | undefined>; referenceId?: string | Types.ObjectId; dedupeKey?: string; channels?: NotificationChannels; }) {
        ...
        if (emailRequested) { await NotificationQueueService.enqueueEmail(notification._id.toString()); }
        if (pushRequested) { await NotificationQueueService.enqueuePush(notification._id.toString()); }
    ...
    static async deliverEmail(notificationId: string) {
        if (!Types.ObjectId.isValid(notificationId)) { throw new Error("Invalid notification ID"); }
    ...
    static async deliverPush(notificationId: string) {
        if (!Types.ObjectId.isValid(notificationId)) { throw new Error("Invalid notification ID"); }
    ...
    static async markEmailDeliveryFailed(notificationId: string, error: unknown) {
    ...
    static async markPushDeliveryFailed(notificationId: string, error: unknown) {
    ...
    static async retryEmail(notificationId: string) {
        if (!Types.ObjectId.isValid(notificationId)) { throw new Error("Invalid notification ID"); }

        // Atomic claim. Only ONE retry request can change FAILED -> RETRYING.
        const notification = await Notification.findOneAndUpdate(
            { _id: notificationId, isDeleted: false, "emailDelivery.status": "FAILED" },
            {
        ...
        if (!notification) { throw new Error("Notification email is not available for retry"); }
        ...
        try {
            ...
            await Notification.updateOne(
                { _id: notification._id, "emailDelivery.status": "RETRYING" },
                {
                    $set: {
                        "emailDelivery.status": "FAILED",
                        "emailDelivery.failedAt": new Date(),
    ...
    static async retryPush(notificationId: string) {
        if (!Types.ObjectId.isValid(notificationId)) { throw new Error("Invalid notification ID"); }
