...
type ComponentUpdate = Partial<
  Pick<IComponent, "name" | "description" | "imageUrl" | "isBundled" | "isRemovable"> & { categoryId?: string; }
>;

const createHttpError = (message: string, statusCode: number) => { const error = new Error(message) as Error & { statusCode: number; }; error.statusCode = statusCode; return error; };

export class ComponentService {
  private static async invalidateComponentCache(componentId?: string): Promise<void> {
    const operations: Promise<unknown>[] = [RedisCacheService.deleteByPattern(CacheKeys.componentListPattern())];
    if (componentId) { operations.push(RedisCacheService.delete(CacheKeys.componentDetail(componentId))); }
    await Promise.all(operations);
  }

  private static async invalidateAffectedServiceCaches(componentId: string): Promise<void> {
  ...
  static async updateComponent(componentId: string, updateData: ComponentUpdate) {
    ...
    try {
      ...
      if (!component) { throw createHttpError("Component not found", 404); }
      ...
      if (error?.code === 11000) { throw createHttpError("Component already exists", 409); }
      throw error;
    }
  }
  ...
  static async toggleComponentStatus(componentId: string, isActive: boolean, confirmed = false) {
    const component = await Component.findById(componentId).select("_id isActive").lean();
    if (!component) { throw createHttpError("Component not found", 404); }

    if (component.isActive === isActive) { return { success: true, unchanged: true, component }; }
    ...
    try {
      ...
      if (!updatedComponent) { throw createHttpError("Component not found", 404); }
      if (!isActive) {
  ...
  static async findComponents(params: { searchTerm?: string; categoryId?: string; limit?: number; page?: number; isRemovable?: boolean; isActive?: boolean; isBundled?: boolean; sortBy?: string; sortOrder?: "asc" | "desc"; }) {
    ...
    if (categoryId && !Types.ObjectId.isValid(categoryId)) { throw createHttpError("Invalid categoryId", 400); }
  ...
  static async exportComponentsToCsv(componentIds: string[]) {
    ...
    if (uniqueComponentIds.some((id) => !Types.ObjectId.isValid(id))) { throw createHttpError("One or more component IDs are invalid", 400); }
    ...
    if (components.length === 0) { throw createHttpError("No components found for export", 404); }
