...
export class ReviewService {
    private static async adjustRatingAggregate(
        ...
        const reviewee = await User.findById(
            revieweeId,
        ).session(session);
        ...
        if (
            ...
            const currentRatingSum =
                reviewee.coordinatorProfile.ratingSum ?? 0;

            const currentTotalRatings =
                reviewee.coordinatorProfile.totalRatings ?? 0;
        ...
        if (
            ...
            const currentRatingSum =
                reviewee.ratingSummary?.ratingSum ?? 0;

            const currentTotalRatings =
                reviewee.ratingSummary?.totalRatings ?? 0;
    ...
    static async getAllReviews(params: {
        ...
        try {
            ...
            throw new Error(
                `Review fetch failed: ${error.message}`,
            );
        }
    }
    ...
    static async editReviewService(
        ...
        try {
            ...
            await session.withTransaction(async () => {
                ...
                if (
                    !reviewDocument.reviewerId.equals(
                        reviewerObjectId,
                    )
                ) {
                    throw new Error(
                ...
                switch (action) {
                    ...
                    case "PUBLISH": {
                        ...
                        if (reviewDocument.isDeleted) {
                            throw new Error(
                                "Deleted review cannot be published",
                            );
                        }
    ...
    static async getMyBookingReview(
        ...
        const isCustomer =
            customerId.equals(userObjectId);

        const isCoordinator =
            coordinatorId.equals(userObjectId);
    ...
    static async getMyReviews(params: {
        ...
        try {
            ...
            throw new Error(
                `My reviews fetch failed: ${error.message}`,
            );
        }
    }
    ...
    static async getCoordinatorReviews(params: {
        ...
        try {
            ...
            throw new Error(
                `Coordinator reviews fetch failed: ${error.message}`,
            );
        }
    }
}