...
export class CartPricingEngine {
  ...
  private static validateTaxPriceMode(value: unknown): TaxPriceMode {
    if (value === TaxPriceMode.EXCLUSIVE) {
      return TaxPriceMode.EXCLUSIVE;
    }

    if (value === TaxPriceMode.INCLUSIVE) {
  ...
  private static ensureUniqueIds<T>(
    items: readonly T[],
    getId: (item: T) => Types.ObjectId,
    label: string,
  ): void {
  ...
  private static createUniqueMap<T>(
    items: readonly T[],
  ...
  private static async loadTaxProfiles(
  ...
  ): Promise<Map<string, TaxProfileLookup>> {
    const uniqueIds = [
      ...new Set(
        taxProfileIds
    ...
    if (uniqueIds.length === 0) {
      return new Map<string, TaxProfileLookup>();
    }
  ...
  private static calculatePricingLine(params: {
    ...
    taxProfileMap: Map<string, TaxProfileLookup>;
    ...
    const discountAmount = this.validateMoney(
      "Line discount",
  ...
  static async calculateServiceCart(cart: ICart): Promise<CartTotals> {
    ...
    const selectedIds = new Set(
      selectedComponents.map((item) => item.componentId.toString()),
    );
    ...
    const pricingMap = this.createUniqueMap(
      pricingRows,
      (pricing) => pricing.componentId.toString(),
      "service pricing row for component",
    );

    const serviceComponentMap = this.createUniqueMap(
      serviceComponents,
      (component) => component.componentId.toString(),
      "service component",
    );
    ...
    if (taxConfig.enabled) {
      const taxContext = await TaxContextService.resolveByLocationId(
        cart.locationId,
      );
    ...
    const duplicateRequiredIds = requiredComponentIds.filter(
      (id, index) => requiredComponentIds.indexOf(id) !== index,
    );

    if (duplicateRequiredIds.length > 0) {
      throw new Error(
        `Duplicate required service component: ${duplicateRequiredIds[0]}`,
      );
    }

    const requiredComponentIdSet = new Set(requiredComponentIds);

    const selectedComponentMap = new Map<string, CartComponentInput>(
    ...
    if (placeOfSupplyStateCode) {
      result.taxSummary.placeOfSupplyStateCode = placeOfSupplyStateCode;
    }
  ...
  static async calculatePackageCart(cart: ICart): Promise<CartTotals> {
    ...
    const selectedIds = new Set(
      selectedServices.map((item) => item.serviceId.toString()),
    );
    ...
    this.ensureUniqueIds(
      ...
      "package tier service",
    );

    const pricingMap = this.createUniqueMap(
      pricingRows,
      (pricing) => pricing.serviceId.toString(),
      "package pricing row for service",
    );
    ...
    const selectedServiceMap = new Map<string, CartServiceInput>(
    ...
    const addonServiceMap = new Map<string, CartServiceInput>(
      addonServices.map((service) => [service.serviceId.toString(), service]),
    );

    const mappedServiceIds = new Set(
      packageServices.map((service) => service.serviceId.toString()),
    );
    ...
    for (const selectedService of selectedServices) {
      ...
      if (!mappedServiceIds.has(id)) {
        throw new Error(
          `Selected service is not part of the package tier: ${id}`,
        );
      }
    }
    ...
    for (const service of packageServices) {
      ...
      if (selectedService && service.isRelated) {
        throw new Error(
          `Related service must be selected as addon: ${serviceId}`,
        );
      }

      if (addonService && !service.isRelated) {
        throw new Error(
          `Non-related package service cannot be selected as addon: ${serviceId}`,
        );
      }

      if (!selectedService && !addonService) {
        continue;
      }

      if (!pricing) {
        throw new Error(
          `Pricing not found for selected package service: ${serviceId}`,
        );
      }
    ...
    const result: CartTotals = {
      ...
      componentLines: new Map<string, CartPriceLine>(),
      serviceLines,
      componentItems: [],
      serviceItems,
    };
