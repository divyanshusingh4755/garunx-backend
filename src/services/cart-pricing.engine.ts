...
} from "./tax-calculator.service.js";

import {
  TaxContextService,
} from "./tax-context.service.js";
...
export class CartPricingEngine {
  ...
  private static validateTaxPriceMode(
    ...
    throw new Error(
      `Invalid tax price mode: ${String(value)}`,
    );
  }
  ...
  private static calculatePricingLine(
    ...
    const taxProfileId =
      pricing.taxProfileId ??
      null;

    const taxPriceMode =
      this.validateTaxPriceMode(
        pricing.taxPriceMode,
      );
  ...
  static async calculateServiceCart(
    ...
    if (!cart.serviceId) {
      throw new Error(
        "serviceId is required for service cart pricing",
      );
    }

    if (cart.packageId) {
      throw new Error(
        "Service cart cannot contain packageId",
      );
    }
    ...
    for (
      ...
      if (selectedIds.has(id)) {
        throw new Error(
          `Component cannot be both selected and addon: ${id}`,
        );
      }
    }
    ...
    const duplicateRequiredIds =
      requiredComponentIds.filter(
        (id, index) =>
    ...
    for (
      ...
      const cartComponent =
        selectedComponentMap.get(
          componentId,
        );

      const line =
        this.calculatePricingLine({
          amount:
            pricing.price,
          discountAmount:
            cartComponent
  ...
  static async calculatePackageCart(
    ...
    if (!cart.packageId) {
      throw new Error(
        "packageId is required for package cart pricing",
      );
    }

    if (cart.serviceId) {
      throw new Error(
        "Package cart cannot contain serviceId",
      );
    }
    ...
    for (
      ...
      if (selectedIds.has(id)) {
        throw new Error(
          `Service cannot be both selected and addon: ${id}`,
        );
      }
    }
    ...
    for (
      ...
      if (
        ...
        throw new Error(
          `Addon service is not part of the package tier: ${id}`,
        );
      }
    }
  ...
  static async calculateCartTotals(
    ...
    const hasService =
      Boolean(cart.serviceId);

    const hasPackage =
      Boolean(cart.packageId);

    if (
      hasService === hasPackage
    ) {
      throw new Error(
        hasService
    ...
    return hasService
      ? this.calculateServiceCart(
        cart,
      )
      : this.calculatePackageCart(
        cart,
      );
  }
}