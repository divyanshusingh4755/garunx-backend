...
export class RbacService {
    private static async invalidatePermissionCaches(permissionId?: string): Promise<void> {
    ...
    private static async invalidateRoleCaches(roleId?: string): Promise<void> {
        const operations: Promise<unknown>[] = [
            RedisCacheService.deleteByPattern(CacheKeys.rbacRoleListPattern()),
    ...
    private static async invalidateUserAccessCache(userId: string): Promise<void> {
        await RedisCacheService.delete(CacheKeys.rbacUserAccess(userId));
    }

    static async createPermission(params: CreatePermissionParams) {
        ...
        if (keyModule !== normalizedModule) { throw new Error("Permission key module must match the module field"); }

        const existingPermission = await Permission.findOne({ key: normalizedKey }).lean();
        if (existingPermission) { throw new Error("Permission with this key already exists"); }
    ...
    static async getPermissions(params: GetPermissionsParams) {
        ...
        return RedisCacheService.getOrSet({
            ...
            loader: async () => {
                ...
                if (normalizedModule) { query.module = normalizedModule; }
    ...
    static async getPermissionById(permissionId: string) {
        if (!Types.ObjectId.isValid(permissionId)) { throw new Error("Invalid permission ID"); }
    ...
    static async updatePermission(params: UpdatePermissionParams) {
    ...
    static async updatePermissionStatus(permissionId: string, isActive: boolean) {
        if (!Types.ObjectId.isValid(permissionId)) { throw new Error("Invalid permission ID"); }
    ...
    static async createRole(params: CreateRoleParams) {
        ...
        if (uniquePermissionIds.length > 0) {
            ...
            if (foundPermissions.length !== uniquePermissionIds.length) { throw new Error("One or more permissions are invalid or inactive"); }
        }
    ...
    static async getRoles(params: GetRolesParams) {
    ...
    static async updateRole(params: UpdateRoleParams) {
        ...
        if (!role) { throw new Error("Role not found"); }
        if (role.isSystem) { throw new Error("System role cannot be modified"); }
    ...
    static async updateRoleStatus(roleId: string, isActive: boolean) {
        if (!Types.ObjectId.isValid(roleId)) { throw new Error("Invalid role ID"); }

        const role = await RbacRole.findById(roleId);
        if (!role) { throw new Error("Role not found"); }
        if (role.isSystem) { throw new Error("System role status cannot be changed"); }
    ...
    static async addRolePermissions(params: AddRolePermissionsParams) {
    ...
    static async removeRolePermission(roleId: string, permissionId: string) {
        if (!Types.ObjectId.isValid(roleId)) { throw new Error("Invalid role ID"); }
        if (!Types.ObjectId.isValid(permissionId)) { throw new Error("Invalid permission ID"); }
        ...
        if (!hasPermission) { throw new Error("Permission is not assigned to this role"); }
    ...
    static async assignRolesToUser(userId: string, roleIds: string[]) {
        if (!Types.ObjectId.isValid(userId)) { throw new Error("Invalid user ID"); }

        const user = await User.findById(userId);
        if (!user) { throw new Error("User not found"); }
        if (user.role !== Role.ADMIN) { throw new Error("RBAC roles can only be assigned to admin users"); }

        const uniqueRoleIds = [...new Set(roleIds)];
        const roles = await RbacRole.find({ _id: { $in: uniqueRoleIds }, isActive: true, isSystem: false }).select("_id").lean();
        if (roles.length !== uniqueRoleIds.length) { throw new Error("One or more roles are invalid, inactive, or cannot be assigned"); }
    ...
    static async removeAllRolesFromUser(userId: string) {
        if (!Types.ObjectId.isValid(userId)) { throw new Error("Invalid user ID"); }
    ...
    static async removeRoleFromUser(userId: string, roleId: string) {
        ...
        if (!hasRole) { throw new Error("Role is not assigned to this user"); }
    ...
    static async exportRolesToCsv(roleIds: string[]) {
        if (!Array.isArray(roleIds) || roleIds.length === 0) { throw new Error("At least one role ID is required"); }

        const uniqueRoleIds = [...new Set(roleIds)];

        // Defensive validation because service methods should not rely only on the route.
        if (uniqueRoleIds.some((roleId) => !Types.ObjectId.isValid(roleId))) { throw new Error("One or more role IDs are invalid"); }
        ...
        if (roles.length === 0) { throw new Error("No roles found for export"); }
        ...
        const orderedRoles = uniqueRoleIds.map((roleId) => roleMap.get(roleId)).filter((role): role is NonNullable<typeof role> => role !== undefined);
        ...
        const rows = orderedRoles.map((role) => {
            ...
            const permissionNames = permissions.map((permission: any) => permission.name).filter(Boolean).join(" | ");
            const permissionKeys = permissions.map((permission: any) => permission.key).filter(Boolean).join(" | ");
            const permissionModules = [...new Set(permissions.map((permission: any) => permission.module).filter(Boolean))].join(" | ");
