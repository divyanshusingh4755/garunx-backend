...
export class PackageDiagnosticsEngine {
  ...
  static async analyze(packageId: string): Promise<PackageDiagnosticResult> {
    ...
    const packageDocument =
      await Package.findById(packageId).lean<PackageDiagnosticDocument>();

    if (!packageDocument) {
      throw new Error("Package not found");
    }

    const packageTierIds = packageDocument.tiers
    ...
    const [mappings, pricing, locations, tiers] = await Promise.all([
      ...
      packageLocationIds.length > 0
        ...
        : Promise.resolve([] as NamedDocument[]),
    ...
    const tierMap = new Map(
      tiers.map((tier) => [tier._id.toString(), tier.name]),
    );
    ...
    if (!packageDocument.isActive) {
      issues.push({
        ...
        message: `Package "${packageDocument.name}" is inactive`,
        severity: "info",
      });
    }
    ...
    const malformedTiers = packageDocument.tiers.filter(
      (tier) => this.safeObjectIdString(tier.tierId) === null,
    );

    if (malformedTiers.length > 0) {
      issues.push({
        code: "MALFORMED_TIERS",
        message: "Some package tiers contain invalid IDs",
        severity: "blocking",
        meta: {
    ...
    const missingTierIds = packageTierIds.filter(
      (tierId) => !tierMap.has(tierId),
    );
    ...
    const malformedLocations = packageDocument.locations.filter(
      (location) => this.safeObjectIdString(location.locationId) === null,
    );
    ...
    const missingLocationIds = packageLocationIds.filter(
      (locationId) => !locationMap.has(locationId),
    );
    ...
    if (missingLocationIds.length > 0) {
      issues.push({
        ...
        severity: "blocking",
        meta: missingLocationIds.map((locationId) => ({
          locationId,
    ...
    const activeLocations = packageDocument.locations.filter(
      (location) =>
        location.isActive &&
    ...
    const inactiveMasterLocations = activeLocations
      .map((location) => this.safeObjectIdString(location.locationId))
      .filter((id): id is string => id !== null)
      .filter((locationId) => {
    ...
    const orphanMappings = mappings.filter(
      (mapping) => !packageTierIds.includes(mapping.tierId.toString()),
    );
    ...
    const validMappings = mappings.filter((mapping) =>
      packageTierIds.includes(mapping.tierId.toString()),
    );

    const mappingTierIds = validMappings.map((mapping) =>
    ...
    const allMappedServices = validMappings.flatMap((mapping) =>
      (mapping.services ?? []).map((service) => ({
    ...
    const malformedServices = allMappedServices.filter(
      (service) => this.safeObjectIdString(service.serviceId) === null,
    );
    ...
    if (malformedServices.length > 0) {
      issues.push({
        ...
        severity: "blocking",
        meta: {
          count: malformedServices.length,
        },
      });
    }
    ...
    const inactiveServices = allMappedServices
      .map((service) => this.safeObjectIdString(service.serviceId))
      .filter((id): id is string => id !== null)
      .filter((serviceId) => {
    ...
    if (duplicateMappedServices.length > 0) {
      issues.push({
        ...
        severity: "blocking",
        meta: {
          count: duplicateMappedServices.length,
          samples: duplicateMappedServices.slice(0, 10),
        },
      });
    }
