...
export class PackageTierMapService {
  private static async invalidatePackageTierMapCache(
    packageId: string,
  ): Promise<void> {
    await Promise.all([
      /*
  ...
  private static async validateAndFormatServices(
    ...
    if (
      ...
      throw new Error(
        "Services field must be an array",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Duplicate serviceId is not allowed",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "One or more services are invalid, inactive, incomplete, or do not support this tier",
      );
    }
  ...
  static async bulkUpsertMappings(
    ...
    try {
      await session.withTransaction(
        async () => {
          await this.validatePackageTier(
            packageId,
            tierId,
            session,
          );
          ...
          const formattedServices =
            await this.validateAndFormatServices(
              services,
              tierId,
              session,
            );
          ...
          if (
            ...
            if (
              ...
              throw new Error(
                "Unable to create package tier mapping",
              );
            }
     ...
     */
    await this.invalidatePackageTierMapCache(
      packageId,
    );
    ...
    try {
      await session.withTransaction(
        async () => {
          ...
          const mapping =
            await PackageTierMap.findOneAndUpdate(
              {
                packageId:
                  new Types.ObjectId(
                    packageId,
          ...
          if (
            ...
            throw new Error(
              "Unable to replace package tier mapping",
            );
          }
  ...
  static async getServicesByPackageAndTier(
    packageId: string,
    tierId: string,
  ) {
    /*
  ...
  static async patchService(
      ...
      await session.withTransaction(
        async () => {
          ...
          if (
            ...
            throw new Error(
              "Service is invalid, inactive, or incomplete",
            );
          }
          ...
          if (
            ...
            throw new Error(
              "A service cannot be both required and related",
            );
          }
    ...
    return {
      ...
      message:
        "Service mapping updated successfully",
