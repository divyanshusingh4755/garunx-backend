...
export class ServiceDiagnosticsEngine {
  ...
  static async analyze(serviceId: string): Promise<ServiceDiagnosticResult> {
    ...
    const service =
      await Service.findById(serviceId).lean<ServiceDiagnosticDocument>();
    ...
    const malformedLocations = service.locations.filter(
    ...
    const missingLocations = serviceLocationIds.filter(
    ...
    const malformedTiers = service.tiers.filter(
    ...
    const missingTierIds = serviceTierIds.filter(
    ...
    const orphanComponents = components.filter(
      (component) => !serviceTierIds.includes(component.tierId.toString()),
    );
    ...
    if (orphanComponents.length > 0) {
      issues.push({
        ...
        meta: orphanComponents.slice(0, 10).map((component) => ({
          ...
          componentName:
            componentMap.get(component.componentId.toString()) ?? "UNKNOWN",
    ...
    const tiersWithoutRequiredComponents = serviceTierIds.filter(
      (tierId) =>
