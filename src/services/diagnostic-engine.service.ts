...
export class ServiceDiagnosticsEngine {
  ...
  static async analyze(serviceId: string): Promise<ServiceDiagnosticResult> {
    if (!Types.ObjectId.isValid(serviceId)) { throw new Error("Invalid serviceId"); }

    const service = await Service.findById(serviceId).lean<ServiceDiagnosticDocument>();
    if (!service) { throw new Error("Service not found"); }

    const serviceTierIds = service.tiers.map((tier) => this.safeObjectIdString(tier.tierId)).filter((id): id is string => id !== null);
    const serviceLocationIds = service.locations.map((location) => this.safeObjectIdString(location.locationId)).filter((id): id is string => id !== null);

    const [category, components, pricing, tiers, locations] = await Promise.all(
      [
        Category.findById(service.categoryId).lean<CategoryDocument>(),
        ServiceComponent.find({ serviceId: service._id }).lean<ComponentDocument[]>(),
        ServicePricing.find({ serviceId: service._id }).lean<PricingDocument[]>(),

        serviceTierIds.length > 0 ? Tier.find({ _id: { $in: serviceTierIds } }).lean<NamedDocument[]>() : Promise.resolve([] as NamedDocument[]),
        serviceLocationIds.length > 0 ? Location.find({ _id: { $in: serviceLocationIds } }).lean<NamedDocument[]>() : Promise.resolve([] as NamedDocument[]),
      ],
    );
    ...
    const inactiveConfiguredLocations = service.locations.filter((location) => location.isActive).map((location) => this.safeObjectIdString(location.locationId)).filter((id): id is string => id !== null).filter((locationId) => {
    ...
    if (orphanComponents.length > 0) {
      issues.push({
        ...
        severity: "warning",
        meta: orphanComponents.slice(0, 10).map((component) => ({
          componentId: component.componentId.toString(),
          componentName: componentMap.get(component.componentId.toString()) ?? "UNKNOWN",
          tierId: component.tierId.toString(),
          tierName: tierMap.get(component.tierId.toString()) ?? "UNKNOWN",
        })),
      });
    }
    ...
    const missingPricing: MissingServicePricing[] = [];
    ...
    for (const tier of service.tiers) {
      ...
      const tierComponents = requiredComponents.filter((component) => component.tierId.toString() === tierId);

      for (const location of activeServiceLocations) {
