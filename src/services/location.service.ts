...
export class LocationService {
  private static async invalidateLocationCache(
    locationId?:
      string,
  ): Promise<void> {
    const operations:
  ...
  private static async validateHierarchy(params: {
    country: string;
    stateId: string;
    cityId: string;
  }): Promise<void> {
    ...
    if (!city) {
      throw createHttpError(
        "City does not belong to state/country",
        400,
      );
    }
  }
  ...
  static async exportLocationsToCsv(
    locationIds: string[],
  ) {
    ...
    const rows =
      locations.map(
        (location) => {
          const state =
            location.stateId as unknown as {
              _id?: Types.ObjectId;
              name?: string;
              gstCode?: string;
            } | null;
