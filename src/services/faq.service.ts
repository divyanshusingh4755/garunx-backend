...
export class FAQService {
  private static async invalidateFaqCache(faqId?: string): Promise<void> {
    const operations: Promise<unknown>[] = [RedisCacheService.deleteByPattern(CacheKeys.faqListPattern()),];

    if (faqId) { operations.push(RedisCacheService.delete(CacheKeys.faqDetail(faqId))); }

    await Promise.all(operations);
  }

  private static ensureValidId(id: string): void {
    if (!Types.ObjectId.isValid(id)) { throw new Error("Invalid FAQ ID"); }
  }

  static async createFaq(faqData: Partial<IFAQ>) {
    if (!faqData.name?.trim()) { throw new Error("Name is required"); }
    if (!faqData.question?.trim()) { throw new Error("Question is required"); }
    if (!faqData.answer?.trim()) { throw new Error("Answer is required"); }
  ...
  static async updateFaq(id: string, updateData: FaqUpdateData) {
    ...
    for (
      const [field, value,] of Object.entries(updateData)) {
      faq.set(field, value);
    }
  ...
  static async findFaqs(searchTerm?: string, faqType?: string, limit = 20, page = 1, isActive?: boolean, sortBy = "displayOrder", sortOrder: "asc" | "desc" = "asc") {
    ...
    return RedisCacheService.getOrSet({
      ...
      loader: async () => {
        ...
        if (faqType) { query.faqType = faqType as FaqType; }
  ...
  static async exportFaqsToCsv(faqIds: string[]) {
    if (!Array.isArray(faqIds) || faqIds.length === 0) { throw new Error("At least one FAQ ID is required"); }
    if (faqIds.length > 1000) { throw new Error("A maximum of 1000 FAQs can be exported at once"); }
    ...
    if (faqs.length === 0) { throw new Error("No FAQs found for export"); }

    // Preserve the same order in which IDs were received from frontend.
    const faqMap = new Map(faqs.map((faq) => [faq._id.toString(), faq,]));
    const orderedFaqs = uniqueFaqIds.map((faqId) => faqMap.get(faqId)).filter((faq): faq is NonNullable<typeof faq> => Boolean(faq));
