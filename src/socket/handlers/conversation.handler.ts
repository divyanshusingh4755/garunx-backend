...
export const registerReadHandlers = (io: ChatSocketServer, socket: ChatSocket): void => {
  socket.on("conversation:read", async (payload, callback) => {
    try {
      ...
      if (otherParticipant) { io.to(getUserRoom(otherParticipant.userId.toString())).emit("conversation:read", readEvent) }
...
export const registerMessageHandlers = (io: ChatSocketServer, socket: ChatSocket): void => {
  socket.on("message:send", async (payload, callback) => {
    ...
    try {
      ...
      if (!socket.rooms.has(room)) { throw new Error("Join the conversation before sending messages") }
      ...
      if (message.replyToMessageId) { replyMessage = await ChatMessage.findOne({ _id: message.replyToMessageId, conversationId: message.conversationId, }); }

      const messageDto = { ...toChatMessageSocketDto(message, { replyMessage }), deliveryStatus: "SENT" as const }
...
export const registerConversationHandlers = (_io: ChatSocketServer, socket: ChatSocket): void => {
  ...
  socket.on("conversation:leave", async ({ conversationId }: { conversationId: string }) => {
    try {
      if (!conversationId || !Types.ObjectId.isValid(conversationId)) { throw new Error("Invalid conversation ID") }
...
export const registerPresenceHandlers = (_io: ChatSocketServer, socket: ChatSocket): void => {
  socket.on("presence:get", async (payload) => {
    try {
      ...
      if (!otherParticipant) { throw new Error("Other chat participant not found") }
...
export const registerTypingHandlers = (_io: ChatSocketServer, socket: ChatSocket): void => {
  socket.on("typing:start", async (payload) => {
    try {
      ...
      if (!socket.rooms.has(room)) { throw new Error("Join the conversation before typing") }
  ...
  socket.on("typing:stop", async (payload) => {
    try {
      if (!payload || typeof payload !== "object") { throw new Error("Invalid typing payload") }
