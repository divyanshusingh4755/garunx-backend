...
const createComponentValidation = [
  body("name").isString().withMessage("Component name must be a string").trim().notEmpty().withMessage("Component name is required"),
  ...
  body("isRemovable").optional().isBoolean().withMessage("isRemovable must be boolean").toBoolean(),
  body("isBundled").optional().isBoolean().withMessage("isBundled must be boolean").toBoolean(),
...
const updateComponentValidation = [
  ...
  body().custom((value) => {
    const allowedFields = ["name", "categoryId", "description", "imageUrl", "isRemovable", "isBundled",];
  ...
  body("name").optional().isString().withMessage("name must be string").trim().notEmpty().withMessage("name cannot be empty"),
  body("categoryId").optional().isMongoId().withMessage("Invalid category ID"),
  body("description").optional().isString().withMessage("description must be string").trim().notEmpty().withMessage("description cannot be empty"),
  body("imageUrl").optional({ values: "falsy" }).isURL().withMessage("Invalid image URL"),
  body("isRemovable").optional().isBoolean().withMessage("isRemovable must be boolean").toBoolean(),
  body("isBundled").optional().isBoolean().withMessage("isBundled must be boolean").toBoolean(),
  validate,
...
const publicListValidation = [
  ...
  query("searchTerm").optional().isString().withMessage("searchTerm must be a string").trim().isLength({ max: 100 }).withMessage("searchTerm cannot exceed 100 characters"),
  query("limit").optional().isInt({ min: 1, max: 100 }).withMessage("limit must be between 1 and 100"),
  query("page").optional().isInt({ min: 1 }).withMessage("page must be at least 1"),
  query("isRemovable").optional().isBoolean().withMessage("isRemovable must be true or false"),
  query("isBundled").optional().isBoolean().withMessage("isBundled must be true or false"),
  query("sortBy").optional().isIn(["name", "createdAt", "updatedAt", "isActive", "isRemovable", "isBundled", "relevance"]).withMessage("Invalid sortBy value"),
...
const exportComponentsValidation = [
  body("componentIds").isArray({ min: 1, max: 1000, }).withMessage("componentIds must contain between 1 and 1000 component IDs"),
  body("componentIds.*").isMongoId().withMessage("Each componentId must be a valid MongoDB ID"),
  validate,
];
