    ...
    .custom((value) => {
      ...
      return values.every((item) =>
        APPLICABLE_TYPES.includes(
          String(item)
            .trim()
            .toUpperCase() as (typeof APPLICABLE_TYPES)[number],
        ),
      );
    })
    .withMessage("Invalid applicable type"),
...
const exportCouponsValidation = [
  body("couponIds")
    ...
    .withMessage(
      "couponIds must contain between 1 and 1000 coupon IDs",
    ),

  body("couponIds.*")
    .isMongoId()
    .withMessage(
      "Each couponId must be a valid MongoDB ID",
    ),
...
router.post(
  ...
  authenticate,
  authorizeRoles(Role.ADMIN),
  requirePermission("coupon.export"),
  exportCouponsValidation,
  exportCouponsCsv,
);
