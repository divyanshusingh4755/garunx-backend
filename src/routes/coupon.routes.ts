...
const APPLICABLE_TYPES = ["ALL", "SERVICE", "PACKAGE", "REFERRAL"] as const;
const DISCOUNT_TYPES = ["PERCENTAGE", "FIXED"] as const;
const SORT_FIELDS = ["createdAt", "updatedAt", "name", "couponCode", "applicableOn", "discount", "usageLimit", "usedCount", "validFrom", "validTill", "isActive", "relevance"] as const;
...
const validateCouponCombination = body().custom((_, { req }) => {
  ...
  if (applicableOn === "SERVICE") {
    if (!Array.isArray(services) || services.length === 0) { throw new Error("At least one service is required for SERVICE coupons"); }
    if (Array.isArray(packages) && packages.length > 0) { throw new Error("Packages are not allowed for SERVICE coupons"); }
  }

  if (applicableOn === "PACKAGE") {
    if (!Array.isArray(packages) || packages.length === 0) { throw new Error("At least one package is required for PACKAGE coupons"); }
    if (Array.isArray(services) && services.length > 0) { throw new Error("Services are not allowed for PACKAGE coupons"); }
  }
  ...
  if (applicableOn === "REFERRAL" && !assignedUserId) { throw new Error("assignedUserId is required for REFERRAL coupons"); }
...
const listCouponValidation = [
  ...
  query("applicableOn").optional().custom((value) => {
    ...
    return values.every((item) => APPLICABLE_TYPES.includes(String(item).trim().toUpperCase() as (typeof APPLICABLE_TYPES)[number]));
