...
const registerValidation = [
  ...
  body().custom(({ userEmail, phoneNumber }) => {
    if (!userEmail && !phoneNumber) { throw new Error("Email or phone number is required"); }
    return true;
  }),
  validate,
];
...
export const serviceableLocationsValidation = [
  ...
  body("serviceableLocations").custom((locations) => {
    const locationIds = locations.map((location: { locationId: string }) => location.locationId);
...
const verifyOtpValidation = [
  ...
  body().custom(({ userId, email }) => {
    ...
    if (userId && email) throw new Error("Provide either User ID or email, not both");
    return true;
  }),
...
const resendOtpValidation = [
  body().custom(({ userId, email, role }) => {
    if (!userId && !email) throw new Error("User ID or email is required");
    if (userId && email) throw new Error("Provide either User ID or email, not both");
    if (email && !role) throw new Error("Role is required for password reset OTP");
    return true;
  }),
  body("userId").optional().isMongoId().withMessage("Invalid User ID"),
  body("email").optional().isEmail().normalizeEmail(),
...
const coordinatorUnavailableDatesValidation = [
  ...
  body("unavailableDates.*").isString().withMessage("Each unavailable date must be string").matches(/^\d{4}-\d{2}-\d{2}$/).withMessage("Each unavailable date must be in YYYY-MM-DD format").custom((value: string) => {
  ...
  body("unavailableDates").custom((dates: string[]) => {
