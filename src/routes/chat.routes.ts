...
import {
  ...
  getById,
  markAsRead,
} from "../controllers/chatconversation.controllers.js";
...
import {
  chatImageUpload,
} from "../middleware/chatImageUpload.js";

import {
  requireActiveChatParticipant,
} from "../middleware/chatParticipant.js";
...
const sendMessageValidation = [
  ...
  body("clientMessageId")
    ...
    .isString()
    .withMessage(
      "Client message ID must be string",
    )
    .bail()
    .trim()
    .notEmpty()
    .withMessage(
      "Client message ID cannot be empty",
    )
    .isLength({
      max: 128,
    })
  ...
  body("images")
    ...
    .withMessage(
      "Images must be an array with maximum 5 images",
    ),
  ...
  body("images.*")
    ...
    .withMessage(
      "Each image URL must be between 1 and 2000 characters",
    )
    .bail()
    .isURL({
      protocols: [
