...
const TAX_TREATMENTS = ["TAXABLE", "EXEMPT", "NIL_RATED", "NON_GST"] as const;
...
const createTaxProfileValidation = [
  body("name")
    ...
    .withMessage("Tax profile name must be between 2 and 100 characters"),
  ...
  body("code")
    ...
    .withMessage("Tax profile code must be between 2 and 50 characters")
    .matches(/^[A-Z0-9_]+$/)
    .withMessage(
  ...
  body("treatment")
    ...
    .bail()
    .isIn(TAX_TREATMENTS)
    .withMessage("Tax treatment must be TAXABLE, EXEMPT, NIL_RATED or NON_GST"),
  ...
  body("totalRate")
    ...
    .withMessage("totalRate must be between 0 and 100")
    .toFloat(),
...
const listTaxProfilesValidation = [
  ...
  query("treatment")
    .optional()
    .isIn(TAX_TREATMENTS)
    .withMessage("Invalid treatment filter"),
...
const exportTaxProfilesValidation = [
  body("taxProfileIds")
    ...
    .withMessage(
      "taxProfileIds must contain between 1 and 1000 tax profile IDs",
    ),

  body("taxProfileIds.*")
    .isMongoId()
    .withMessage(
      "Each taxProfileId must be a valid MongoDB ID",
    ),
...
router.post(
  "/export",
  requirePermission("tax_profile.export"),
  exportTaxProfilesValidation,
  TaxProfileController.exportCsv,
);
