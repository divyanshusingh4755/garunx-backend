...
const coordinatesValidation = [
  ...
  body("location.coordinates").optional().isArray({ min: 2, max: 2 }).withMessage("Coordinates must be [longitude, latitude]").custom((coordinates: unknown[]) => {
    if (coordinates.length !== 2 || !coordinates.every((coordinate) => typeof coordinate === "number" && Number.isFinite(coordinate))
...
const createLocationValidation = [
  ...
  body("stateId").notEmpty().withMessage("stateId is required").isMongoId().withMessage("Invalid stateId"),
  body("cityId").notEmpty().withMessage("cityId is required").isMongoId().withMessage("Invalid cityId"),
  body("fullAddress").isString().withMessage("fullAddress must be a string").trim().isLength({ min: 10 }).withMessage("Address is too short"),
  body("pincode").isString().withMessage("pincode must be a string").trim().isPostalCode("IN").withMessage("Invalid Indian Pincode"),
...
const updateLocationValidation = [
  ...
  body().custom((value) => {
    const allowedFields = ["name", "country", "stateId", "cityId", "fullAddress", "pincode", "image", "description", "location"];
  ...
  body("country").optional().isString().withMessage("country must be a string").trim().notEmpty().withMessage("country cannot be empty"),
  body("stateId").optional().isMongoId().withMessage("Invalid stateId"),
  body("cityId").optional().isMongoId().withMessage("Invalid cityId"),
  body("fullAddress").optional().isString().withMessage("fullAddress must be a string").trim().isLength({ min: 10 }).withMessage("Address is too short"),
  body("pincode").optional().isString().withMessage("pincode must be a string").trim().isPostalCode("IN").withMessage("Invalid Indian Pincode"),
  body("image").optional({ values: "falsy" }).isURL().withMessage("Image must be a valid URL"),
  body("description").optional().isString().withMessage("description must be a string").trim(),
  ...coordinatesValidation,
  validate,
...
const statusValidation = [
  param("id").isMongoId().withMessage("Invalid location ID"),
  body("status").exists({ checkNull: true }).withMessage("status is required").isBoolean().withMessage("status must be a boolean").toBoolean(),
  body("confirmed").optional().isBoolean().withMessage("confirmed must be a boolean").toBoolean(),
  validate,
];

const locationIdsValidation = [
  body("locationIds").isArray({ min: 1 }).withMessage("locationIds must be a non-empty array"),
  body("locationIds.*").isMongoId().withMessage("Each location ID must be valid"),
  validate,
];
...
const publicListValidation = [
  ...
  query("sortBy").optional().isIn(["name", "country", "pincode", "createdAt", "updatedAt", "relevance"]).withMessage("Invalid sortBy value"),
  validate,
];
...
const exportLocationsValidation = [
  body("locationIds").isArray({ min: 1, max: 1000 }).withMessage("locationIds must contain between 1 and 1000 location IDs"),
  body("locationIds.*").isMongoId().withMessage("Each locationId must be a valid MongoDB ID"),
  validate,
];
