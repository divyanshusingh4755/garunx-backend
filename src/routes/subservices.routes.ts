...
const updateSubServiceComponentValidation = [
  ...
  body().custom((value) => {
    const allowedFields = ["name", "description", "serviceId", "image",];
...
const statusValidation = [
  param("id").isMongoId().withMessage("Invalid ID"),
  body("status").exists({ checkNull: true }).withMessage("status is required").isBoolean().withMessage("status must be boolean").toBoolean(),
  validate,
];

const publicListValidation = [
  query("serviceId").optional().custom((value) => {
  ...
  query("sortBy").optional().isIn(["name", "createdAt", "updatedAt", "relevance",]).withMessage("Invalid sortBy value"),
