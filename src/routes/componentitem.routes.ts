...
const publicListValidation = [
  ...
  query("sortBy").optional().isIn(["name", "price", "createdAt", "updatedAt", "relevance"]).withMessage("Invalid sortBy value"),
...
const exportComponentItemsValidation = [
  body("componentItemIds").isArray({ min: 1, max: 1000 }).withMessage("componentItemIds must contain between 1 and 1000 component item IDs"),
  body("componentItemIds.*").isMongoId().withMessage("Each componentItemId must be a valid MongoDB ID"),
  validate,
];
