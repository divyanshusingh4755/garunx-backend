...
const FAQ_TYPES = ["User", "Coordinator", "User_Query", "Coordinator_Query"] as const;
const SORT_FIELDS = ["displayOrder", "createdAt", "updatedAt", "name", "faqType", "isActive", "relevance"] as const;
...
const exportFaqsValidation = [
  ...
  body("faqIds").custom((faqIds) => {
    if (!Array.isArray(faqIds)) { return true; }
