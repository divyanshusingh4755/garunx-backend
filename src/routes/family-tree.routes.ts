...
const FAMILY_TREE_ACTIVITY_ACTIONS = ["MEMBER_ADDED", "MEMBER_UPDATED", "MEMBER_DELETED", "MEMBER_RESTORED", "RELATIONSHIP_LINKED", "RELATIONSHIP_UNLINKED"] as const;

const addFamilyMemberValidation = [
  body("fullName").notEmpty().withMessage("Full name is required").isString().withMessage("Full name must be a string").trim().isLength({ min: 2, max: 120 }).withMessage("Full name must be between 2 and 120 characters"),
  body("relation").notEmpty().withMessage("Family relation is required").isIn(Object.values(FamilyRelation)).withMessage("Invalid family relation"),
  ...
  body("caste").optional({ nullable: true, checkFalsy: true, }).isIn(Object.values(Caste)).withMessage("Invalid caste"),
  body("gotra").optional({ nullable: true, checkFalsy: true, }).isIn(Object.values(Gotra)).withMessage("Invalid gotra"),
  body("designatedPandit").optional().isString().withMessage("Designated pandit must be a string").trim().isLength({ max: 120, }).withMessage("Designated pandit cannot exceed 120 characters"),
  body("visitors").optional().isArray().withMessage("Visitors must be an array"),
  body("visitors.*").optional().isString().withMessage("Visitor must be a string").trim().isLength({ min: 1, max: 120, }).withMessage("Visitor must be between 1 and 120 characters"),
  body("profileImage").optional({ nullable: true, checkFalsy: true, }).isString().withMessage("Profile image must be a string"),
  body("notes").optional().isString().withMessage("Notes must be a string").trim().isLength({ max: 1000, }).withMessage("Notes cannot exceed 1000 characters"),
  body().custom((value) => {
    const dob = value.dob ? new Date(value.dob) : null;
    const dateOfDeath = value.dateOfDeath ? new Date(value.dateOfDeath) : null;

    if (dob && dob.getTime() > Date.now()) { throw new Error("DOB cannot be in the future"); }
    if (dob && dateOfDeath && dateOfDeath < dob) { throw new Error("Date of death cannot be before DOB"); }
    return true;
  }),
...
const deleteFamilyMemberValidation = [
  param("id").notEmpty().withMessage("Family member ID is required").isMongoId().withMessage("Invalid family member ID"),
  body("reason").notEmpty().withMessage("Deletion reason is required").isString().withMessage("Deletion reason must be a string").trim().isLength({ min: 3, max: 500 }).withMessage("Deletion reason must be between 3 and 500 characters"),
  validate,
];

const getFamilyMembersValidation = [
  query("search").optional().isString().withMessage("Search must be a string").trim().isLength({ max: 120 }).withMessage("Search cannot exceed 120 characters"),
  query("relation").optional().isIn(Object.values(FamilyRelation)).withMessage("Invalid family relation"),
  query("gender").optional().isIn(Object.values(Gender)).withMessage("Invalid gender"),
  query("lifeStatus").optional().isIn(Object.values(MemberLifeStatus)).withMessage("Invalid life status"),
...
const getFamilyTreeActivitiesValidation = [
  query("action").optional().isIn(FAMILY_TREE_ACTIVITY_ACTIONS).withMessage("Invalid family tree activity action"),
  query("familyMemberId").optional().isMongoId().withMessage("Invalid family member ID"),
  query("performedBy").optional().isMongoId().withMessage("Invalid performed-by user ID"),
...
const familyTreeOwnerIdValidation = [
  param("ownerId").notEmpty().withMessage("Family tree owner ID is required").isMongoId().withMessage("Invalid family tree owner ID"),
  validate,
];

const restoreFamilyMemberValidation = [
  param("id").notEmpty().withMessage("Family member ID is required").isMongoId().withMessage("Invalid family member ID"),
  body("reason").optional().isString().withMessage("Restore reason must be a string").trim().isLength({ min: 3, max: 500 }).withMessage("Restore reason must be between 3 and 500 characters"),
  validate,
];

const exportFamilyMembersValidation = [
  body("memberIds").isArray({ min: 1, max: 1000 }).withMessage("memberIds must contain between 1 and 1000 family member IDs"),
  body("memberIds.*").isMongoId().withMessage("Each memberId must be a valid MongoDB ID"),
  body("memberIds").custom((memberIds) => {
    if (!Array.isArray(memberIds)) { return true; }
...
// Keep the more-specific activity route before the generic member detail route.
