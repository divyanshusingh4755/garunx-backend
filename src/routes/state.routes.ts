...
const createStateValidation = [
  ...
  body("gstCode").isString().withMessage("gstCode must be a string").trim().matches(/^\d{2}$/).withMessage("gstCode must contain exactly 2 digits"),
...
const updateStateValidation = [
  ...
  body().custom((value) => {
    const allowedFields = ["country", "name", "gstCode", "image", "description", "location",];
  ...
  body("gstCode").optional().isString().withMessage("gstCode must be a string").trim().matches(/^\d{2}$/).withMessage("gstCode must contain exactly 2 digits"),
  body("image").optional({ values: "falsy" }).isURL().withMessage("Image must be a valid URL"),
  body("description").optional().isString().withMessage("description must be a string").trim(),
  ...locationValidation,
  validate,
...
const publicStateListValidation = [
  ...
  query("sortBy").optional().isIn(["name", "country", "gstCode", "createdAt", "updatedAt", "relevance",]).withMessage("Invalid sortBy value"),
  ...
  query("stateFilter").optional().isString().trim().isLength({ max: 500 }).withMessage("stateFilter cannot exceed 500 characters"),
  validate,
];

const exportStatesValidation = [
  body("exportAll").optional().isBoolean().withMessage("exportAll must be a boolean").toBoolean(),
  body("stateIds").optional().isArray({ min: 1, max: 1000 }).withMessage("stateIds must contain between 1 and 1000 state IDs"),
  body("stateIds.*").optional().isMongoId().withMessage("Each stateId must be a valid MongoDB ID"),
  body().custom((value) => {
    const exportAll = value?.exportAll === true;
    const stateIds = value?.stateIds;

    if (exportAll && Array.isArray(stateIds) && stateIds.length > 0) {
      throw new Error(
        "Provide either exportAll=true or stateIds, not both",
      );
    }

    if (!exportAll && (!Array.isArray(stateIds) || stateIds.length === 0)) { throw new Error("Provide stateIds or set exportAll to true"); }
    return true;
  }),
  validate,
];
