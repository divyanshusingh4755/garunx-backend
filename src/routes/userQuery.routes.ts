...
const QUERY_CATEGORIES = ["BOOKING", "PAYMENT", "REFUND", "SERVICE", "PACKAGE", "ACCOUNT", "TECHNICAL", "OTHER"] as const;
const QUERY_PRIORITIES = ["LOW", "NORMAL", "HIGH", "URGENT"] as const;
const REQUESTER_TYPES = ["USER", "COORDINATOR"] as const;
const SORT_FIELDS = ["createdAt", "updatedAt", "latestMessageAt", "lastActionAt"] as const;
...
const exportUserQueriesValidation = [
  ...
  body("queryIds").custom((queryIds) => {
    if (!Array.isArray(queryIds)) { return true; }

    const uniqueIds = new Set(queryIds);
    if (uniqueIds.size !== queryIds.length) { throw new Error("Duplicate query IDs are not allowed"); }
    return true;
  },
  ),
  validate,
