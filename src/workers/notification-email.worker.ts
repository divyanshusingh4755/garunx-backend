...
export const notificationEmailWorker = new Worker<NotificationEmailJobData>(
    NOTIFICATION_QUEUE_NAMES.EMAIL, async (job) => {
        try {
            ...
            if (currentAttempt >= maxAttempts) { await NotificationService.markEmailDeliveryFailed(job.data.notificationId, error); }
